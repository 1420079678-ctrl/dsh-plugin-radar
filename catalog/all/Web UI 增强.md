...
- 🟩 `[可用]` [amruthpillai-reactive-resume](https://github.com/amruthpillai/reactive-resume) ★42436 — A one-of-a-kind resume builder that keeps your privacy in mind〔📦〕
- 🟩 `[可用]` [huangruiteng-loopx](https://github.com/huangruiteng/loopx) ★5890 — Long-horizon agent control plane for durable, governed work across Codex, Claude Code, and other harnesses.
...
- 🟩 `[可用]` [bowenliang123-dsh-context](https://github.com/bowenliang123/dsh-context) ★1424 — The best DeepSeek Harness plugin for context insight and management, with context dashboard / browser / sideba
- ⬜ `[待定]` [xmanrui-dsh-im](https://github.com/xmanrui/dsh-im) ★1374 — 通过扫码或机器人凭据把IM机器人接入DeepSeek Harness（支持飞书、微信、钉钉、企业微信、QQ、Slack、Telegram、Discord和WhatsApp）
...
- 🟩 `[可用]` [paean-ai-deeptide](https://github.com/paean-ai/deeptide) ★1103 — Built by DeepSeek, for DeepSeek — a Swift-native macOS coding agent
...
- 🟩 `[可用]` [vostride-agent-qa](https://github.com/vostride/agent-qa) ★912 — Open-source self-improving QA agent for software teams
- 🟩 `[可用]` [DSH-better-sidebar](https://github.com/omdsh-dev/DSH-better-sidebar) ★846 — 一个侧边栏的完整工作台，支持三方拓展注册新侧边栏页面
- 🟩 `[可用]` [vshulcz-deja-vu](https://github.com/vshulcz/deja-vu) ★826 — One memory shared by Claude Code, Codex, Cursor, Copilot CLI, OpenClaw and 20 more coding agents, built from t
...
- 🟩 `[可用]` [shanliuling-dsh-image-gen](https://github.com/shanliuling/dsh-image-gen) ★436 — AI image studio for DeepSeek Harness — generate, edit & compare images in chat, with 500+ prompts, gallery, mu
- 🟩 `[可用]` [liangmianya-dsh-synapse](https://github.com/liangmianya/dsh-synapse) ★402 — A visual, non-linear conversation workspace plugin for DeepSeek Harness ; A canvas-based session explorer and
...
- 🟩 `[可用]` [yjh051108-dsh-router-standard](https://github.com/yjh051108/dsh-router-standard) ★358 — 已并入 dsh-routing-suite（单仓库化）；本仓库为历史镜像/归档 —— 注意力工程主线 v1.19.1/v34 研发线未发布
- 🟩 `[可用]` [alaliqing-claude-paper](https://github.com/alaliqing/claude-paper) ★337 — 📖 Cross-agent research paper toolkit for Claude Code, Codex, OpenCode, and DeepSeek Harness—quick summaries, d
- 🟩 `[可用]` [whale-girl](https://github.com/vlln/whale-girl) ★325 — DSH Web GUI 桌面宠物插件（QQ 宠物形态）：右下角悬浮、可拖拽/投喂/玩耍的积累型伙伴
- 🟩 `[可用]` [elysia395-dsh-wallpaper-engine](https://github.com/elysia395/dsh-wallpaper-engine) ★300 — 把本机 Wallpaper Engine 的壁纸变成 DSH 网页界面的背景：Video 动态播放、Web 以 iframe 加载、Scene 壁纸提取主纹理作为静态帧；iOS 液态玻璃设置窗口（配色 / 玻璃颜色 /
- ⬜ `[待定]` [dsh-tianshu-tui](https://github.com/huiliyi37/dsh-tianshu-tui) ★279 — 官方 DeepSeek Harness 的交互式终端 UI 插件：自研 ANSI 极简交互渲染、流式 Markdown/工具卡、16+ 主题、slash 命令与选择器、输入历史与本地偏好持久化、LSP 诊断、memory
- ⬜ `[待定]` [saya-ch-dsh-mobile](https://github.com/saya-ch/dsh-mobile) ★272 — DeepSeek Harness 的 Android App 与安全远程访问插件，支持局域网/远程连接和高度自定义的移动界面与扩展能力
- ⬜ `[待定]` [libukai-awesome-deepseek-harness](https://github.com/libukai/awesome-deepseek-harness) ★266 — DeepSeek Harness 终极指南：快速入门、资源推荐、精选插件与实用工具 ｜The Ultimate Guide to DeepSeek Harness: QuickStart, Resources, Plug
...
- 🟩 `[可用]` [xing-shuyin-pi-web-ui](https://github.com/xing-shuyin/pi-web-ui) ★189 — dsh / pi-agent web ui
- 🟩 `[可用]` [cocode-agency-cocode](https://github.com/cocode-agency/cocode) ★162 — Best ready-to-run DeepSeek Harness distribution: DSH desktop GUI, DSH terminal TUI, and harness integration.
- 🟩 `[可用]` [lire1131-dsh-undo-savepoint](https://github.com/lire1131/dsh-undo-savepoint) ★158 — DSH crash-rescue plugin: undo config & plugin-code changes, secret-safe snapshots, one-click SAFE MODE, plus o
- 🟩 `[可用]` [deepseek-harness-for-vscode](https://github.com/skymecode/deepseek-harness-for-vscode) ★147 — deepseek-harness for vscode .A native VS Code coding-agent extension for DeepSeek Harness
- 🟩 `[可用]` [modusensus-dsh-mneme](https://github.com/modusensus/dsh-mneme) ★111 — 🧠 The memory that dreams — cross-session memory for DeepSeek Harness
...
- 🟩 `[可用]` [ymh0000123-dsh-theme-endfield](https://github.com/ymh0000123/dsh-theme-endfield) ★92 — 终末地官网风格的 DSH Web 主题：奶油纸底、墨黑文字、信号黄强调、全直角工业编辑风
- 🟩 `[可用]` [mexiaosqwq-dsh-web-mobile](https://github.com/mexiaosqwq/dsh-web-mobile) ★89 — DSH Web UI 移动端适配：窄屏好用，宽屏适用
...
- ⬜ `[待定]` [dsh-undo-plugin](https://github.com/lire1131/dsh-undo-plugin) ★86 — DSH crash-rescue plugin: undo config & plugin-code changes, secret-safe snapshots, one-click SAFE MODE, plus o
- 🟩 `[可用]` [dickpy-dsh-imagegen](https://github.com/dickpy/dsh-imagegen) ★77 — DSH (DeepSeek Harness) Web GUI AI image generation plugin: text-to-image & image-to-image via OpenAI-compatibl
- 🟩 `[可用]` [fandc520-dsh-comfyui](https://github.com/fandc520/dsh-comfyui) ★71 — 一个基于DeepSeek-Harness的ComfyUI插件
- 🟩 `[可用]` [amlyczz-dsh-agy-link](https://github.com/amlyczz/dsh-agy-link) ★70 — Google Antigravity (agy CLI) models for DeepSeek Harness — streaming chat, thinking, tool activity, usage, in-
- 🟩 `[可用]` [plolpl789-dsh-raw-html](https://github.com/plolpl789/dsh-raw-html) ★65 — VCP visual-synesthesia protocol plugin for DeepSeek Harness: render agent HTML output as real UI (cards / KaTe
- 🟩 `[可用]` [niuhuoshan-launch-wechat-miniprogram](https://github.com/niuhuoshan/launch-wechat-miniprogram) ★61 — 面向完全新手的微信小程序 Agent Skill，从需求确认、原生 UI 和高保真原型，到 AppID、备案、开发测试、腾讯云后台、体验版、提审、发布及版本更新
...
- 🟩 `[可用]` [plolpl789-dsh-raw-html-v2](https://github.com/plolpl789/dsh-raw-html-v2) ★48 — VCP visual-synesthesia plugin v2 for DeepSeek Harness (official Slot API): raw-HTML vcp-root cards, streaming
...
- ⬜ `[待定]` [kejixiaoliang-awesome-dsh-plugins](https://github.com/kejixiaoliang/awesome-dsh-plugins) ★42 — DeepSeek Harness (DSH) 插件精选目录 — 14 类 280+ 个社区插件，覆盖 MCP / Skill / TUI / 多 Agent / 上下文记忆 / UI 皮肤，点链接直达仓库
- 🟩 `[可用]` [dsh-file-review](https://github.com/left0ver/dsh-file-review) ★41 — dsh插件 - 立刻审查agent对文件的修改，查看diff，支持在dsh-better-sidebar中使用
- 🟩 `[可用]` [erichare-skillroute](https://github.com/erichare/skillroute) ★40 — Local-first skill routing for agent builders — index SKILL.md bundles, get ranked skill plans with confidence
- 🟩 `[可用]` [skyf0xx-hedgehog](https://github.com/skyf0xx/hedgehog) ★39 — HEDGEHOG codes Cleaner, Faster and with Fewer Tokens
...
- 🟩 `[可用]` [dsh-lark-link](https://github.com/amlyczz/dsh-lark-link) ★38 — High-reliability Feishu/Lark bridge for DeepSeek Harness — QR one-click auth, multi-mode agents, card-based co
- 🟩 `[可用]` [webkubor-dsh-bloom-theme](https://github.com/webkubor/dsh-bloom-theme) ★37 — DeepSeek Harness (DSH) 主题插件：Bloom 莫兰迪配色 10 套中国风命名（黛蓝/朱砂/桃夭/天青/竹青/赭石/青金/琥珀/落霞/青莲），OKLCH 调色，明暗双主题，顶栏一键切换，20 组配色实
- ⬜ `[待定]` [techysy-deepseek-harness-fnos](https://github.com/techysy/deepseek-harness-fnos) ★36 — DeepSeek Harness (DeepSeek 官方 agent 浏览器 UI) fnOS 应用 — 本地常驻服务
- 🟩 `[可用]` [yoli-mi-dsh-client-ui-custom](https://github.com/yoli-mi/dsh-client-ui-custom) ★35 — Configurable DSH web-surface plugin: wallpaper & frosted-glass themes, accent colors, custom keyboard shortcut
- ⬜ `[待定]` [dsh-ui-whale](https://github.com/lhh010/dsh-ui-whale) ★34 — 【求⭐】🐋DSH Web UI 全手绘像素鲸鱼伙伴插件：会话标题栏常驻，平时眨眼/偶尔摆尾/动胸鳍，思考运行时持续动起来，回合完成头顶喷水，点击还会冒爱心，不工作时还会偷懒睡觉，零核心改动 〔PR〕
- 🟩 `[可用]` [niiang-dsh-kimino-theme](https://github.com/niiang/dsh-kimino-theme) ★34 — Kimi no Na wa (Your Name) theme for DeepSeek Harness Web GUI
- 🟩 `[可用]` [988hj7tczd-oss-dsh-computer-use](https://github.com/988hj7tczd-oss/dsh-computer-use) ★33 — Cross-platform Computer Use plugin for DeepSeek Harness: observable desktop automation with an isolated virtua
- ⬜ `[待定]` [dsh-plugin-session-delete](https://github.com/lsz-asd/dsh-plugin-session-delete) ★33 — Delete DeepSeek Harness sessions from the UI: header danger button + sidebar session-row menu item (no convers
- ⬜ `[待定]` [dsh-minigames](https://github.com/lhh010/dsh-minigames) ★32 — DSH Web UI 右侧小游戏面板：18 款离线小游戏（恐龙跳一跳 / 俄罗斯方块 / 坦克大战 / 扫雷 / 2048 / 数独 / 吃豆人 / 跟枪练习等），可扩展游戏注册表，等待模型回复或修 bug 时的摸鱼神器
...
- 🟩 `[可用]` [quqxui-dsh-memgas](https://github.com/quqxui/dsh-memgas) ★25 — DeepSeek Harness 长期记忆插件：四通道检索融合 + 记忆演化闭环，记忆构建与检索方法来自 ICLR 2026 论文 MemGAS
...
- 🟩 `[可用]` [loongsuite-dsh-plugin](https://github.com/loongsuite/dsh-plugin) ★23 — OpenTelemetry tracing for DeepSeek Harness (dsh): turns each agent turn into a GenAI span tree — steps, LLM ca
- 🟩 `[可用]` [chen731215-dev-dsh-tavern-v2](https://github.com/chen731215-dev/dsh-tavern-v2) ★22 — DeepSeek Harness Tavern Plugin - character card roleplay, worldbook management, preset switching, dark theme,
- ⬜ `[待定]` [liangdabiao-dsh-plugin-developer-skill](https://github.com/liangdabiao/dsh-plugin-developer-skill) ★22 — dsh-plugin-developer — DeepSeek Harness 插件开发 Skill  > 指导 AI Agent 从 0 到 1 开发、构建、安装、测试 DeepSeek Harness（dsh）插件
- 🟩 `[可用]` [renat3u-tonghuashun-webui](https://github.com/renat3u/tonghuashun-webui) ★22 — 仿同花顺的webui插件
...
- 🟩 `[可用]` [chumingjun-dsh-harness-one](https://github.com/chumingjun/dsh-harness-one) ★20 — Visual AI workflow orchestrator for DeepSeek Harness (dsh): multi-agent DAGs, live execution, recovery, and Fe
- 🟩 `[可用]` [chyra-moon-deepseek-harness-desktop](https://github.com/chyra-moon/deepseek-harness-desktop) ★20 — DeepSeek Harness desktop shell: 1:1 replica of the official web UI as a Windows desktop app (community project
- 🟩 `[可用]` [iuikj-dsh-desktop](https://github.com/iuikj/dsh-desktop) ★20 — 一个微调美观的DeepSeek harness客户端（欢迎插件加入）
- 🟩 `[可用]` [huey1in-reef](https://github.com/huey1in/reef) ★20 — DSH 插件全家桶:浏览器自动化 + MCP Server + GitHub/GitLab 自动评审 + 原生嵌入面板 \| One install, five modules for DeepSeek Harness:
- 🟩 `[可用]` [cheshireez-dsh-skill-hub](https://github.com/cheshireez/dsh-skill-hub) ★19 — DeepSeek Harness（dsh）Web GUI 技能中枢：浏览/搜索完整本地技能目录、启用/禁用、查看正文、排查诊断、新建技能，基于官方 ctx.skills 注册表
- 🟩 `[可用]` [dskin](https://github.com/dancingmemory/dskin) ★19 — DSKIN · DeepSeek Harness（DSH）卡通像素皮肤插件 / Cartoon pixel skin plugin for DSH Web GUI — 原始界面不动，像素宠物会散步、眨眼、跳跃 / liv
...
- ⬜ `[待定]` [thness-dsh-mobile](https://github.com/thness/dsh-mobile) ★19 — DeepSeek Harness Android — 把 DeepSeek Harness 完整装进安卓手机：内嵌 Node.js 运行时 + 官方 Web UI，开箱即用的独立 App
- 🟩 `[可用]` [xiake595-touhou-hakurei](https://github.com/xiake595/touhou-hakurei) ★19 — 灵梦（Reimu）·博丽神社（东方Project）美化版皮肤：神社昼夜实景背景、灵梦立绘、画框侧边栏与输入框、纸白透明界面 — DeepSeek Harness Web GUI skin
- 🟩 `[可用]` [dsh-outline](https://github.com/urzeye/dsh-outline) ★19 — DeepSeek Harness（DSH）Web GUI 的实时大纲插件，移植自 Ophel Atlas
- 🟩 `[可用]` [ht426-deepseek-harness-tui](https://github.com/ht426/deepseek-harness-tui) ★18 — 为deepseek harness做了一个tui界面，方便开发者直接在终端使用
- 🟩 `[可用]` [naodeng-dsh-qa](https://github.com/naodeng/dsh-qa) ★18 — dsh-qa · QA Workbench — A local software testing workbench plugin for DeepSeek Harness
- 🟩 `[可用]` [tpmoonchefryan-dsh-joi-channel-theme](https://github.com/tpmoonchefryan/dsh-joi-channel-theme) ★18 — 轴伊 Joi 双衣装主题 for DeepSeek Harness — unofficial, non-commercial fan theme plugin 🍊
...
- 🟩 `[可用]` [dsh-hdc-bridge](https://github.com/1na-ko/dsh-hdc-bridge) ★17 — DSH 原生鸿蒙开发助手：hdc 设备闭环调试 + 设备面板（官方 client 插件形态）+ 离线官方知识层（Tier-1 随包）+ DevEco CLI 构建/签名/模拟器控制 / DSH-native Harmon
- 🟩 `[可用]` [dsh-side-panel](https://github.com/ccq1/dsh-side-panel) ★17 — DSH 侧边栏，集成文件浏览器、终端和 Git 审查，方便预览文件
- 🟩 `[可用]` [liqichen-dsh-plugin-manager](https://github.com/liqichen/dsh-plugin-manager) ★17 — DSH 插件管理器:在 DeepSeek Harness 设置面板内嵌 GUI,管理 MCP 服务 / Skills / 内置插件包,改动热生效无需重启
...
- 🟩 `[可用]` [starslittle-dsh-queue-plus](https://github.com/starslittle/dsh-queue-plus) ★15 — DSH 排队消息增强面板：编辑、删除、插话、排序与批量删除功能
- 🟩 `[可用]` [xtxo-dsh-ui](https://github.com/xtxo/dsh-ui) ★15 — DeepSeek Harness desktop
- ⬜ `[待定]` [chenproton-dsh-history](https://github.com/chenproton/dsh-history) ★15 — Quickly view, search, and jump to all the messages you sent in a long conversation.
...
- 🟩 `[可用]` [liyown-dshx](https://github.com/liyown/dshx) ★14 — Build, test, and ship typed DeepSeek Harness plugins — framework, scaffolding, and a verified community Hub.
...
- ⬜ `[待定]` [heartmove-dsh-side-chat](https://github.com/heartmove/dsh-side-chat) ★12 — 一个 DSH 网页插件，Codex 式侧边聊天的强化版本： 在右侧面板提供按主会话隔离的独立聊天，具备 Codex 式的智能体能力——继承主会话的 工具集、模型、思考难度与权限预设，能感知所在工作目录；选中对话内容即可提
...
- ⬜ `[待定]` [qishuilalala-dsh-voice-mode](https://github.com/qishuilalala/dsh-voice-mode) ★12 — Full-duplex voice plugin for DeepSeek Harness: local zipformer2 streaming ASR (no API key) → editable draft; E
...
- 🟩 `[可用]` [tsrigo-dsh-from-scratch](https://github.com/tsrigo/dsh-from-scratch) ★12 — A runnable TypeScript tutorial that builds a minimal DeepSeek-style agent harness from scratch.
...
- ⬜ `[待定]` [dsh-search-mcp](https://github.com/gxpppp/dsh-search-mcp) ★11 — Replace dsh's built-in web search with search MCP servers (Tavily/Brave/Exa/Perplexity/DuckDuckGo/custom), con
...
- 🟩 `[可用]` [baiyuscc13724-max-deepseek-harness-desktop](https://github.com/baiyuscc13724-max/deepseek-harness-desktop) ★10 — DeepSeek Harness 中文 Windows 桌面版：女仆鲸桌宠、主题、插件市场、模型路由与安全更新
...
- 🟩 `[可用]` [hellosky983-dsh-mc-agent](https://github.com/hellosky983/dsh-mc-agent) ★10 — AI-assisted Minecraft for DeepSeek Harness: drive an autonomous Mineflayer agent (survival, mining, chat, live
- ⬜ `[待定]` [hkkz9522-dsh-session-manager](https://github.com/hkkz9522/dsh-session-manager) ★10 — Session manager for the DeepSeek Harness Web UI: delete sessions, move sessions across workspaces, and migrate
...
- 🟩 `[可用]` [linenxi-ctrl-dsh-vision](https://github.com/linenxi-ctrl/dsh-vision) ★10 — 为 DeepSeek Harness 增加外挂识图模型：圆形鲸鱼按钮、发送图片识图自动回传、模型自主截图+识图工具、多协议自动适配、小白一键安装（未装 Node.js 自动下载）
...
- 🟩 `[可用]` [133563825as-ai-dsh-api-dashboard](https://github.com/133563825as-ai/dsh-api-dashboard) ★9 — 多平台 API 余额看板插件 for DeepSeek Harness Web GUI
- 🟩 `[可用]` [boxeryao-dsh-mini-tui](https://github.com/boxeryao/dsh-mini-tui) ★9 — DSH-TUI: a lightweight and fast terminal plugin connected directly to the DSH runtime.
...
- ⬜ `[待定]` [dsh-better-archive](https://github.com/huahai0202/dsh-better-archive) ★9 — DeepSeek Harness (DSH) web-GUI plugin: archived-session panel with unarchive & delete
...
- ⬜ `[待定]` [dsh-paste-input](https://github.com/lhh010/dsh-paste-input) ★9 — DSH WebUI 文件输入增强：Ctrl+V 粘贴（带首次告知弹窗）+ 拖拽 + 选择文件，发送时复制进会话工作区临时目录
...
- 🟩 `[可用]` [weibaohui-skills-management](https://github.com/weibaohui/skills-management) ★9 — DeepSeek Harness 插件：技能市场——安装/删除/详情 API + 管理界面
- 🟩 `[可用]` [yu-wenchao-dsh-free-models-hub](https://github.com/yu-wenchao/dsh-free-models-hub) ★9 — 免费模型排行榜 · DeepSeek Harness 社区插件，在 DeepSeek Harness (DSH) Web UI 左侧边栏提供「免费模型榜」：分页浏览（每页 20 条、页码窗口、首页/末页）、 点击标题展开
...
- 🟩 `[可用]` [asukasec-dsh-message-preview](https://github.com/asukasec/dsh-message-preview) ★8 — Right-side user-message navigator for the DeepSeek Harness Web UI.
...
- 🟩 `[可用]` [cryjkd-dsh-clawbot](https://github.com/cryjkd/dsh-clawbot) ★8 — 在 DeepSeek Harness（DSH）里接入微信官方 ilink 机器人网关：扫码绑定后，AI 能主动给自己发微信通知；每次回复后自动推送任务完成摘要；目标完成/阻断时自动通知；需要批准/选择时主动微信提醒
...
- ⬜ `[待定]` [fengyungithub-dsh-short-video-studio](https://github.com/fengyungithub/dsh-short-video-studio) ★8 — 基于deepseek harness和ComfyUI的AI视频创作工作台
...
- 🟩 `[可用]` [moguiyu-dsh-tavily](https://github.com/moguiyu/dsh-tavily) ★8 — Tavily-powered optional search tool for DeepSeek Harness (rc.7 plugin management): multi-key rotation/failover
...
- 🟩 `[可用]` [wbin0001-dsh-comfyui-canvas](https://github.com/wbin0001/dsh-comfyui-canvas) ★8 — DSH+ComfyUI画布插件dsh-comfyui-canvas embeds your ComfyUI instance as a split-screen canvas inside DeepSeek Harnes
- 🟩 `[可用]` [yanzwzz-dsh-whale-girl-pet](https://github.com/yanzwzz/dsh-whale-girl-pet) ★8 — 🐋 DeepSeek 娘桌宠：住进 DeepSeek Harness Web 界面的蓝发鲸鱼女仆
- 🟩 `[可用]` [yefeng7531-dsh-whale-maid-mascot](https://github.com/yefeng7531/dsh-whale-maid-mascot) ★8 — DeepSeek 娘 (whale maid) in-page mascot plugin for DeepSeek Harness web UI
...
- ⬜ `[待定]` [compforge-baton](https://github.com/compforge/baton) ★7 — A terminal-native workspace for Codex, Claude Code, and DeepSeek Harness, built on durable cross-harness sessi
...
- ⬜ `[待定]` [rayafriandion-dsh-oc-tui](https://github.com/rayafriandion/dsh-oc-tui) ★7 — An opencode-inspired terminal UI (TUI) for DeepSeek Harness, shipped as a dsh profile app plugin
...
- 🟩 `[可用]` [xiaozhe7772222-dsh-api-key-pool](https://github.com/xiaozhe7772222/dsh-api-key-pool) ★7 — API Key rotation pool for DeepSeek Harness: multi-key round-robin, failover on 401/403/429, cooldown & recover
...
- 🟩 `[可用]` [bilbillm-deepseek-harness-angelina-themes](https://github.com/bilbillm/deepseek-harness-angelina-themes) ★6 — Angelina light and dark glass themes with parallax for DeepSeek Harness
...
- 🟩 `[可用]` [hyzyn-dsh-plugin-kit](https://github.com/hyzyn/dsh-plugin-kit) ★6 — Plugin family for the DeepSeek Harness (DSH) Web GUI: a pnpm monorepo with a plugin template, dev kit, and one
- 🟩 `[可用]` [iimaguest-phone-tunnel-pool](https://github.com/iimaguest/phone-tunnel-pool) ★6 — Refreshable Cloudflare quick-tunnel pool for the dsh web GUI — phone access with QR + embedded login
- 🟩 `[可用]` [islibaodong-dsh-login](https://github.com/islibaodong/dsh-login) ★6 — Multi-user login gateway plugin for the DeepSeek Harness Web GUI: login wall, per-user conversation isolation,
...
- ⬜ `[待定]` [awesome-dsh-background-plugin](https://github.com/leavestring/awesome-dsh-background-plugin) ★6 — DSH Web 背景个性化插件：上传自己的图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或一键切换极光、余烬、宣纸三种预设氛围；实时预览所见即所得，支持细调图像存在感、暗色遮
...
- 🟩 `[可用]` [luweiyabo-dsh-whale-pet](https://github.com/luweiyabo/dsh-whale-pet) ★6 — DeepSeek Harness Web UI 的开源鲸鱼桌宠插件，支持 Agent 状态感知、多种透明动画、点击拖拽、屏幕漫游、自定义动作与触发规则
...
- 🟩 `[可用]` [meyaomiao-dsh-server-deck](https://github.com/meyaomiao/dsh-server-deck) ★6 — 服务器卡片仪表盘 for DeepSeek Harness — card dashboard (status/CPU/mem/disk) + one-click xterm terminal, better-sideba
- 🟩 `[可用]` [mlosun-dsh-docs-panel](https://github.com/mlosun/dsh-docs-panel) ★6 — DSH WebUI 里的「随身手册」：全局 Markdown 笔记，任何工作区随时可读
...
- 🟩 `[可用]` [shuind-dsh-codex-harness](https://github.com/shuind/dsh-codex-harness) ★6 — —
- 🟩 `[可用]` [dsh-plugin-terminal](https://github.com/siberiah2o/dsh-plugin-terminal) ★6 — Bottom terminal panel plugin for DeepSeek Harness (DSH Web GUI)
...
- 🟩 `[可用]` [starslittle-dsh-blue-whale](https://github.com/starslittle/dsh-blue-whale) ★6 — 复刻 DeepSeek Chat 蓝鲸配色的皮肤，亮色/深色跟随系统外观
...
- 🟩 `[可用]` [wycto-dsh-dock](https://github.com/wycto/dsh-dock) ★6 — dsh-dock · DeepSeek Harness 功能坞插件：一张面板统一注册/开关所有小功能——用量记账（自定义单价·分时价）、模型设置与余额、19 种任务动画、任务通知（钉钉/飞书）、运行状态、图片理解代理、账
...
- 🟩 `[可用]` [zaimokuza-yoshiteru-dsh-acp-adapter](https://github.com/zaimokuza-yoshiteru/dsh-acp-adapter) ★6 — Use AI agents from the DSH session UI.
...
- ⬜ `[待定]` [reshuibuduo-tmcra-memory](https://github.com/reshuibuduo/tmcra-memory) ★5 — TMCRA Core — local-first, scope-isolated long-term memory runtime for AI agents.
- 🟩 `[可用]` [rice-awa-dsh-lan-gateway](https://github.com/rice-awa/dsh-lan-gateway) ★5 — 把 DeepSeek Harness 的 Web GUI 安全地开放到局域网或公网，自带密码鉴权，支持可选的TLS
...
- 🟩 `[可用]` [iluluyu-dsh-ui-outline](https://github.com/iluluyu/dsh-ui-outline) ★4 — ChatGPT-style right-edge turn navigation plugin for DeepSeek Harness (dsh) web
...
- 🟩 `[可用]` [runfali-dsh-login-gateway](https://github.com/runfali/dsh-login-gateway) ★4 — dsh 登录门卫插件：让只监听本机的 dsh Web UI 安全地对外开放——登录墙 + 全量反代，零依赖、零侵入
...
- 🟩 `[可用]` [tuojc-dsh-browser-firefox](https://github.com/tuojc/dsh-browser-firefox) ★4 — Firefox browser-control plugin for DeepSeek Harness: one DSH plugin + one Firefox extension, driving your own
- 🟩 `[可用]` [wangxilhy23-dsh-wx-skin](https://github.com/wangxilhy23/dsh-wx-skin) ★4 — deepseek harness 皮肤插件
- 🟩 `[可用]` [weibaohui-dsh-sync](https://github.com/weibaohui/dsh-sync) ★4 — DeepSeek Harness 插件：会话同步与冲突解决（apiproxy、token 内联）
...
- ⬜ `[待定]` [yu-wenchao-dsh-opencode-free-models](https://github.com/yu-wenchao/dsh-opencode-free-models) ★4 — 无限免费额度的deepseek harnes 免费模型插件-免费api-DeepSeek Harness (DSH) 插件 — 在聊天界面里实时展示 OpenCode Zen 的最新免费模型，无需登录、无需密钥，直接在模
...
- 🟩 `[可用]` [yuioi666-dsh-plugin-model-capability](https://github.com/yuioi666/dsh-plugin-model-capability) ★4 — Model Capability Manager for DSH Web: thinking levels, context window, output caps, input modalities, gateway
...
- 🟩 `[可用]` [263311487-ux-dsh-verify](https://github.com/263311487-ux/dsh-verify) ★3 — The quality gate for agent-built web apps
...
- 🟩 `[可用]` [lhbsaa-dsh-visibridge](https://github.com/lhbsaa/dsh-visibridge) ★3 — DeepSeek Harness vision plugin: analyze_image (structured OCR evidence) + capture_image (USB camera visual loo
...
- 🟩 `[可用]` [mantonlove-dsh-prism-plugin](https://github.com/mantonlove/dsh-prism-plugin) ★3 — Prism · 棱镜 — a deeply customizable glassmorphism theme plugin for the DeepSeek Harness Web GUI
...
- 🟩 `[可用]` [mienfong-dsh-session-mgr](https://github.com/mienfong/dsh-session-mgr) ★3 — Session manager for the DeepSeek Harness web UI: move, archive, restore, backup/export and import conversation
...
- 🟩 `[可用]` [mokuyoaxis-dsh-iris](https://github.com/mokuyoaxis/dsh-iris) ★3 — Media generation and visual understanding for DeepSeek Harness, with multi-provider routing and the integrated
...
- 🟩 `[可用]` [oceanxuikun-dsh-eva-theme-plugin](https://github.com/oceanxuikun/dsh-eva-theme-plugin) ★3 — Evangelion-inspired theme plugin for DSH WebUI, featuring Unit-00, Unit-01, and Unit-02 themes with immersive
...
- 🟩 `[可用]` [orriduck-dsh-tui](https://github.com/orriduck/dsh-tui) ★3 — A small, session-aware terminal UI for DeepSeek Harness
...
- 🟩 `[可用]` [qwert702-dsh-commander](https://github.com/qwert702/dsh-commander) ★3 — Commander for the DeepSeek Harness Web GUI: one conversation orchestrates others via <dsh-dispatch> protocol b
...
- 🟩 `[可用]` [sixtysevenlf-dsh-save-balance-monitor](https://github.com/sixtysevenlf/dsh-save-balance-monitor) ★3 — 省钱+余额监控悬浮窗（DSH Web UI 插件）：DeepSeek / OpenCode Go 双标签余额、限额监控（金额/tok/小时）、语音提醒、超限断点截断
- 🟩 `[可用]` [smanx-dsh-conversation-indicator](https://github.com/smanx/dsh-conversation-indicator) ★3 — Conversation indicator plugin for the DeepSeek Harness web GUI: a compact rail beside the scrollbar marks each
...
- 🟩 `[可用]` [tianjiqx-dsh-wigolo](https://github.com/tianjiqx/dsh-wigolo) ★3 — DSH 联网搜索私有化方案，代理 web_search/web_fetch 至 Wigolo，覆盖搜索/抓取/提取/深度研究/缓存/监控全链路，数据不出域，零查询成本
...
- 🟩 `[可用]` [zmm863-commits-dsh-dice-game](https://github.com/zmm863-commits/dsh-dice-game) ★3 — 🎲 Dice Battle (骰子大作战) — classic dice game collection for DSH web GUI: Liar's Dice, Red Dots, Red/Blue, Big/Sma
...
- 🟩 `[可用]` [1010n111-dsh-about](https://github.com/1010n111/dsh-about) ★2 — 为 DeepSeek Harness Web 设置面板添加“关于”页，展示当前运行的 DSH 版本、插件版本与项目主页
...
- 🟩 `[可用]` [aaronlei-dsh-width-tiers](https://github.com/aaronlei/dsh-width-tiers) ★2 — A client plugin for the DeepSeek Harness Web GUI that adds chat content width tiers with a floating picker but
...
- 🟩 `[可用]` [baiyun200-dsh-dashboard](https://github.com/baiyun200/dsh-dashboard) ★2 — DSH 插件看板 · DeepSeek Harness 插件生态可视化（shadcn/ui，每日自动构建部署）
- 🟩 `[可用]` [beihzb-dsh-notebook](https://github.com/beihzb/dsh-notebook) ★2 — Native Jupyter-style notebook for DeepSeek Harness: real ipykernel sidecar + VS Code-aligned cell UI, tqdm pro
...
- 🟩 `[可用]` [djs326-dsh-plugin-width-slider](https://github.com/djs326/dsh-plugin-width-slider) ★2 — 对话宽度滑块插件：DSH Desktop 设置面板滑块，按下即全屏预览实时调节对话宽度，自动隐藏原生拖拽手柄
...
- 🟩 `[可用]` [lucasx001-dsh-skin-claude-code](https://github.com/lucasx001/dsh-skin-claude-code) ★2 — Claude Code-inspired skin for the DeepSeek Harness web GUI
...
- 🟩 `[可用]` [naniwet-dsh-themes](https://github.com/naniwet/dsh-themes) ★2 — A configurable skin for the DeepSeek Harness web UI: palette, display font and every image slot are configurat
...
- 🟩 `[可用]` [robbywang25-dsh-codex-pins](https://github.com/robbywang25/dsh-codex-pins) ★2 — Codex-style pinned sessions for DeepSeek Harness — always visible above the sidebar list
...
- 🟩 `[可用]` [warment-deepseek-harness-locale-ru](https://github.com/warment/deepseek-harness-locale-ru) ★2 — Русский язык для DeepSeek Harness web UI — one-command install, 1061 строка, 100% покрытие
...
- ⬜ `[待定]` [dsh-review-loop](https://github.com/wuxiangru915/dsh-review-loop) ★2 — Incremental diff reviewer for DeepSeek Harness — Web UI review panel + /review command.
...
- ⬜ `[待定]` [xingyingyuzhui-dsh-claw-suite](https://github.com/xingyingyuzhui/dsh-claw-suite) ★2 — Claw Agent governance suite for DeepSeek Harness
...
- 🟩 `[可用]` [yolorouter-dsh-llmasking](https://github.com/yolorouter/dsh-llmasking) ★2 — Transport-layer data masking for deepseek-harness (dsh): the model sees placeholders, you see real values rest
- 🟩 `[可用]` [yu502950715yang-dsh-use-wallpaper](https://github.com/yu502950715yang/dsh-use-wallpaper) ★2 — 把 Wallpaper Engine 动态壁纸搬进 DeepSeek Harness 的 Web GUI：浏览器内实时渲染 scene 壁纸（three.js + Rust/WASM 粒子模拟），支持视频与 web 壁纸
...
- 🟩 `[可用]` [yuhui-sama-dsh-agentsoul](https://github.com/yuhui-sama/dsh-agentsoul) ★2 — Local personality, memory and distillation layer for DeepSeek Harness — SOUL/IDENTITY/USER/STATE persona files
...
- 🟩 `[可用]` [zhang66633-dsh-plugin-installer](https://github.com/zhang66633/dsh-plugin-installer) ★2 — DeepSeek Harness（dsh）的插件商店 + 安装助手：在 Web GUI 里逛插件目录，一键确认安装，agent 替你装好
...
- ⬜ `[待定]` [dsh-miku-skin](https://github.com/stushansusu/dsh-miku-skin) ★2 — 初音未来主题皮肤，用于 DeepSeek Harness (DSH) Web GUI —— 蓝紫洋红渐变、毛玻璃面板、可自定义背景图、亮暗双主题
...
- ⬜ `[待定]` [a792883583-dsh-git-panel](https://github.com/a792883583/dsh-git-panel) ★1 — DSH web GUI git panel: branch management + GitLens-style commit graph
...
- 🟩 `[可用]` [bblike-dsh-plugin-odette](https://github.com/bblike/dsh-plugin-odette) ★1 — Odette / Snezhnaya theme skin for DeepSeek Harness Web: winter backgrounds, ballet cursors, ballerina thinking
...
- ⬜ `[待定]` [guiyi-labs-kubemd](https://github.com/guiyi-labs/kubemd) ★1 — Evidence-first Kubernetes runtime diagnosis with case memory — a DSH skill (+ aiops CLI twin).
...
- ⬜ `[待定]` [hanwuji1-dsh-web-launcher](https://github.com/hanwuji1/dsh-web-launcher) ★1 — 🐋 One-click desktop launcher for the DeepSeek Harness Web UI — a whale-icon app that boots dsh web and opens y
...
- 🟩 `[可用]` [iluluyu-dsh-ui-deepdiving](https://github.com/iluluyu/dsh-ui-deepdiving) ★1 — Continuous water-flow light for the dsh web 'Deep diving...' turn status — three parallax gradient currents ov
...
- 🟩 `[可用]` [jsoncode-dsh-jenkins](https://github.com/jsoncode/dsh-jenkins) ★1 — 可以在DeepSeek Harness中，快捷配置和管理多台 Jenkins 服务器与 Token， 支持设置页配置、模型工具触发构建、工作区级「执行 Jenkins Job」入口
...
- 🟩 `[可用]` [lkdxzhxi-dsh-glass-ui-theme](https://github.com/lkdxzhxi/dsh-glass-ui-theme) ★1 — 为 DeepSeek Harness 打造的液态玻璃主题插件：磨砂玻璃、可调色调、动态壁纸，让 DSH 界面焕然一新
...
- 🟩 `[可用]` [longyu065-dsh-theme-ti](https://github.com/longyu065/dsh-theme-ti) ★1 — —
...
- ⬜ `[待定]` [math-lrz-dsh-plugin-meta](https://github.com/math-lrz/dsh-plugin-meta) ★1 — DSH meta-plugin suite: plugin-forge + plugin-auditor + skill-forge — the meta capability layer where everythin
...
- ⬜ `[待定]` [nlqh7-dsh-beautify](https://github.com/nlqh7/dsh-beautify) ★1 — DeepSeek Harness Dream Skin，DSH theme plugin: Dream Skin color presets with a settings-page switcher.
- 🟩 `[可用]` [notload-dsh-session-toc](https://github.com/notload/dsh-session-toc) ★1 — 为 DeepSeek Harness Web UI 每个会话页右侧加一个类似deepseek网页端的常驻、可折叠的目录栏：每条用户提问对应一个条目，点击即可滚动定位到对应消息并高亮当前条目
...
- 🟩 `[可用]` [omdsh-plugins-omdsh-basemode](https://github.com/omdsh-plugins/omdsh-basemode) ★1 — The session-mode system for the DeepSeek Harness web GUI: the segment registry every mode plugin registers int
...
- 🟩 `[可用]` [penguin-oo-dsh-pathlink](https://github.com/penguin-oo/dsh-pathlink) ★1 — Ctrl+click file paths and links in DeepSeek Harness chat: paths open their folder in the OS file manager, link
- ⬜ `[待定]` [pgmi-builds-corti](https://github.com/pgmi-builds/corti) ★1 — Persistent memory layer for AI agent swarms
- ⬜ `[待定]` [philmingdao-dsh-meter](https://github.com/philmingdao/dsh-meter) ★1 — DSH plugin (#dsh-plugin): DeepSeek usage & balance meter for the DSH settings panel — account balance, today's
...
- 🟩 `[可用]` [ptonlix-dsh-forge](https://github.com/ptonlix/dsh-forge) ★1 — building an auditable desktop distribution around DeepSeek Harness (DSH).
...
- 🟩 `[可用]` [suimi8-dsh-cost-ledger](https://github.com/suimi8/dsh-cost-ledger) ★1 — Cross-session persistent cost ledger for DeepSeek Harness: logs every LLM token usage to SQLite and exposes re
...
- 🟩 `[可用]` [weibaohui-dsh-continue](https://github.com/weibaohui/dsh-continue) ★1 — 自动续跑插件 for DeepSeek Harness — 有序规则表：按失败类型路由 继续续跑 / 换模型 / 压缩后继续 / 停止
...
- 🟩 `[可用]` [weibaohui-dsh-smart-title](https://github.com/weibaohui/dsh-smart-title) ★1 — dsh 插件 · 会话智能标题：LLM 总结改写会话标题，每轮对话后自动更新
...
- 🟩 `[可用]` [xavier711-dsh-plugin-deepseek-usage](https://github.com/xavier711/dsh-plugin-deepseek-usage) ★1 — A DeepSeek usage panel plugin for the DeepSeek Harness Web GUI
...
- ⬜ `[待定]` [yangshen830-eng-dsh-editor](https://github.com/yangshen830-eng/dsh-editor) ★1 — DSH 代码编辑器插件：VS Code 风格界面，内置文件树、Monaco 编辑器、跨文件搜索/替换、Markdown 预览与 Git diff
...
- 🟩 `[可用]` [caoqinnan-web-organize-workspace-sessions](https://github.com/caoqinnan-web/organize-workspace-sessions) ★1 — DSH Skill for organizing DeepSeek Harness workspace sessions as 类别｜主题, with archive/rename/judgment suggestion
...
- 🟩 `[可用]` [xzyonline-dsh-chat-files](https://github.com/xzyonline/dsh-chat-files) ★1 — Session-bound file attachments for DeepSeek Harness: drag, paste, or select files into the chat; the agent rea
...
- 🟩 `[可用]` [djh2203-dsh-refined](https://github.com/djh2203/dsh-refined) ★1 — DeepSeek-Refined 的 DeepSeek Harness 移植版 为 DeepSeek Harness（DSH）前端注入 Obsidian Border 风格的 Markdown 美化与多主题配色
...
- ⬜ `[待定]` [swaylq-dsh-genie](https://github.com/swaylq/dsh-genie) ★1 — Wishes that outlive the session — turn a DeepSeek Harness agent's runtime plugins into permanently installed o
- 🟩 `[可用]` [liveqte-dsh-lan-proxy](https://github.com/liveqte/dsh-lan-proxy) ★1 — DSH 插件：把 dsh 的回环 Web UI 通过 0.0.0.0 反代暴露到局域网，开关/状态/日志嵌入设置页
...
- 🟩 `[可用]` [omdsh-dev-dsh-ui-progress](https://github.com/omdsh-dev/dsh-ui-progress) ★1 — DSH Web UI 会话进度插件：输入框停靠区常驻会话进度条（todos 真实进度 / 实时 token 生成速率 / 中断橘红态 / 待办提醒），零核心改动
...
- 🟩 `[可用]` [baikangwang-dsh-ide-ui](https://github.com/baikangwang/dsh-ide-ui) ★1 — —
...
- `[未定位]` **0nt-one-dsh-neo-skin** — 占位待复核，判定暂不展示
- `[未定位]` **0nt-one-dsh-voice-input** — 占位待复核，判定暂不展示
...
- `[未定位]` **147228-dsh-xiaoyao-skins** — 占位待复核，判定暂不展示
- `[未定位]` **1m01m0-dsh-chat-skin** — 占位待复核，判定暂不展示
- `[未定位]` **241793-dsh-win-gui** — 占位待复核，判定暂不展示
- `[未定位]` **2795178962-dsh-ui-thinking-title** — 占位待复核，判定暂不展示
- `[未定位]` **384961890-ui-pawin-brain-deepseek-harness** — 占位待复核，判定暂不展示
- `[未定位]` **394804078-pixel-dsh-liquid-glass** — 占位待复核，判定暂不展示
- `[未定位]` **4sa1ary9-dockyard-dsh** — 占位待复核，判定暂不展示
- `[未定位]` **54088lp-dsh-kafka-ui** — 占位待复核，判定暂不展示
...
- `[未定位]` **a1swg1159-pixel-dsh-arcaea-theme** — 占位待复核，判定暂不展示
...
- `[未定位]` **acebang0303-dsh-quick-launch** — 占位待复核，判定暂不展示
- `[未定位]` **adamaik-dsh-git-sidebar** — 占位待复核，判定暂不展示
- `[未定位]` **aefuimn-dsh-agent-preset-switcher** — 占位待复核，判定暂不展示
- `[未定位]` **afrel1024-dsh-client-ui-aqua-unofficial** — 占位待复核，判定暂不展示
- `[未定位]` **ahren112-ds-image-skin** — 占位待复核，判定暂不展示
- `[未定位]` **ai-scarlett-build-dsh-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **ai7603-dsh-cyberpunk-theme** — 占位待复核，判定暂不展示
...
- `[未定位]` **alonelypigeon-dsh-plugin-balance-panel** — 占位待复核，判定暂不展示
- `[未定位]` **andrecolin-dsh-tui** — 占位待复核，判定暂不展示
- `[未定位]` **andrepontesmelo-dsh-suite** — 占位待复核，判定暂不展示
- `[未定位]` **andy294753951-dsh-plugin-gouden-leeuw-theme** — 占位待复核，判定暂不展示
- `[未定位]` **annanren-dsh-usage-suite** — 占位待复核，判定暂不展示
- `[未定位]` **anweat-dsh-plugin-dev-guide** — 占位待复核，判定暂不展示
- `[未定位]` **anyuinlara-bit-dsh-bff-pet** — 占位待复核，判定暂不展示
- `[未定位]` **aorucshiea-dsh-safe-tui** — 占位待复核，判定暂不展示
- `[未定位]` **april-jk-dsh-mobile-suite** — 占位待复核，判定暂不展示
...
- `[未定位]` **auran-lu-dsh-client-ui-monitor** — 占位待复核，判定暂不展示
- `[未定位]` **auraxm-dsh-plugin-ui-turnav** — 占位待复核，判定暂不展示
- `[未定位]` **autumnshiu009-ship-it-dsh-mobile-ui** — 占位待复核，判定暂不展示
...
- `[未定位]` **baisama-cloud-dsh-stt-input** — 占位待复核，判定暂不展示
- `[未定位]` **baka-world-dsh-sidebar-modes** — 占位待复核，判定暂不展示
- `[未定位]` **baobao1996121-dsh-restart-ui** — 占位待复核，判定暂不展示
- `[未定位]` **baroncyrus-dsh-ugui-preset** — 占位待复核，判定暂不展示
- `[未定位]` **beizi6-dsh-theme-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **big0lives-dsh-win-quick-launcher** — 占位待复核，判定暂不展示
- `[未定位]` **bilbillm-deepseek-harness-angelina-themes** — 占位待复核，判定暂不展示
- `[未定位]` **bill9109-dsh-web-ui-notify** — 占位待复核，判定暂不展示
...
- `[未定位]` **breaker505-dsh-aurora-skin** — 占位待复核，判定暂不展示
- `[未定位]` **buhuikongpan-dsh-pluginmanager** — 占位待复核，判定暂不展示
- `[未定位]` **buhuikongpan-dsh-project-context** — 占位待复核，判定暂不展示
- `[未定位]` **buildbeforewepitch-agentscars** — 占位待复核，判定暂不展示
- `[未定位]` **byh819-png-dsh-client-ui-notify** — 占位待复核，判定暂不展示
- `[未定位]` **caisiyang123-dsh-theme-dodger-17** — 占位待复核，判定暂不展示
- `[未定位]` **caoyiwei850-dsh-client-ui-skins** — 占位待复核，判定暂不展示
- `[未定位]` **captain1275-dsh-ui-web** — 占位待复核，判定暂不展示
- `[未定位]` **carlown-majia7-dsh-skin** — 占位待复核，判定暂不展示
- `[未定位]` **carpon39038-dsh-image-theme** — 占位待复核，判定暂不展示
- `[未定位]` **cass67-deepseek-tui** — 占位待复核，判定暂不展示
- `[未定位]` **caxson-dsh-gui** — 占位待复核，判定暂不展示
- `[未定位]` **ccch1mneyyy-dsh-tui** — 占位待复核，判定暂不展示
...
- `[未定位]` **changlianxiya-139-dsh-ambiguity-handling** — 占位待复核，判定暂不展示
- `[未定位]` **chen-001-dsh-chat-width** — 占位待复核，判定暂不展示
- `[未定位]` **chencmb2077-ui-dsh-token-monitor** — 占位待复核，判定暂不展示
...
- `[未定位]` **chentao326-dsh-gui** — 占位待复核，判定暂不展示
...
- `[未定位]` **cnskycn-shuimo-skin** — 占位待复核，判定暂不展示
- `[未定位]` **cnsmaple-dsh-client-ui-skill-fuzzy** — 占位待复核，判定暂不展示
- `[未定位]` **cocofhu-dsh-testsuite** — 占位待复核，判定暂不展示
- `[未定位]` **cocowwy-dsh-plugin-eyecare-theme** — 占位待复核，判定暂不展示
- `[未定位]` **collapsey-xiaokui-pet** — 占位待复核，判定暂不展示
- `[未定位]` **condathinker-dsh-left-sidebar-collapse** — 占位待复核，判定暂不展示
- `[未定位]` **congyaqwq-dsh-quick-prompts** — 占位待复核，判定暂不展示
- `[未定位]` **cooltea001-dsh-cool-theme** — 占位待复核，判定暂不展示
- `[未定位]` **couioly-plugin-audio-alert** — 占位待复核，判定暂不展示
- `[未定位]` **crtnt-dsh-plugin-suite** — 占位待复核，判定暂不展示
- `[未定位]` **cslawyer1985-trajectory-zh-guide** — 占位待复核，判定暂不展示
- `[未定位]` **cungphammanh590-star-dsh-quiz** — 占位待复核，判定暂不展示
- `[未定位]` **cw1999mm-dsh-annotator** — 占位待复核，判定暂不展示
- `[未定位]` **czhzz-dsh-input-history** — 占位待复核，判定暂不展示
- `[未定位]` **d-dev0101-open-sea-skin** — 占位待复核，判定暂不展示
- `[未定位]` **dac114514-dsh-theme-center** — 占位待复核，判定暂不展示
- `[未定位]` **dakeshui123-dsh-pet-frieren** — 占位待复核，判定暂不展示
- `[未定位]` **damonbao-dsh-codex-suite** — 占位待复核，判定暂不展示
- `[未定位]` **dancingmemory-dskin** — 占位待复核，判定暂不展示
...
- `[未定位]` **ddbj-hub-dsh-wallpaper-skin** — 占位待复核，判定暂不展示
- `[未定位]` **dddfxyqiming-dsh-annotation-patched** — 占位待复核，判定暂不展示
- `[未定位]` **dddzzz123-dz-dsh-voice-input-plugin** — 占位待复核，判定暂不展示
...
- `[未定位]` **dlpufan-dsh-theme-cyberpunk** — 占位待复核，判定暂不展示
...
- `[未定位]` **dragonbaba-dsh-routing-suite** — 占位待复核，判定暂不展示
- `[未定位]` **drfccv-dsh-theme-neko** — 占位待复核，判定暂不展示
- `[未定位]` **drscrewdriver-dsh-input-traffic** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-mixxed-dsh-client-ui-filesystem-manager** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-mixxed-dsh-client-ui-git-branch** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-mixxed-dsh-client-ui-settings-mcp** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-niao-dsh-niao-quick-open** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-tui-ecosystem-dsh-code-memory** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-tui-ecosystem-plugin-template** — 占位待复核，判定暂不展示
...
- `[未定位]` **e2mcc-dsh-popout-sidebar** — 占位待复核，判定暂不展示
...
- `[未定位]` **edabchann-dsh-neotui** — 占位待复核，判定暂不展示
...
- `[未定位]` **edwardyang0011-dsh-ui-skins** — 占位待复核，判定暂不展示
- `[未定位]` **elohia-dsh-plugin-image-input** — 占位待复核，判定暂不展示
- `[未定位]` **emeryxu1-blip-dsh-matrix-skin** — 占位待复核，判定暂不展示
- `[未定位]` **enchangcui340-cloud-dsh-whale-balance** — 占位待复核，判定暂不展示
...
- `[未定位]` **fendouai-deepseek-harness-skin** — 占位待复核，判定暂不展示
- `[未定位]` **fengb3-dsh-theme-aurum** — 占位待复核，判定暂不展示
- `[未定位]` **fengb3-dsh-theme-macintosh** — 占位待复核，判定暂不展示
- `[未定位]` **fengzhiyushui-dsh-desktop-window** — 占位待复核，判定暂不展示
...
- `[未定位]` **forrestahha-dsh-voice-input** — 占位待复核，判定暂不展示
- `[未定位]` **founder987-dsh-dev-ui** — 占位待复核，判定暂不展示
- `[未定位]` **fourtow-dsh-theme-win98** — 占位待复核，判定暂不展示
- `[未定位]` **foxi-ui-deepseek-harness-desktop** — 占位待复核，判定暂不展示
...
- `[未定位]` **frostgao-dsh-theme-blackgold** — 占位待复核，判定暂不展示
...
- `[未定位]` **ginuim-multi-screen-wireframe** — 占位待复核，判定暂不展示
- `[未定位]` **gitnohup-macaron-theme** — 占位待复核，判定暂不展示
- `[未定位]` **goodpostidea-tech-deepseek-harness-skin** — 占位待复核，判定暂不展示
- `[未定位]` **guionai-web** — 占位待复核，判定暂不展示
- `[未定位]` **guix4ever-dsh-point** — 占位待复核，判定暂不展示
- `[未定位]` **guiyi-labs-kubemd** — 占位待复核，判定暂不展示
- `[未定位]` **gunduziba-dsh-sidebar-open-in-ide** — 占位待复核，判定暂不展示
- `[未定位]` **guoan1tang-deepseek-ui-agent-sky** — 占位待复核，判定暂不展示
- `[未定位]` **h2o-mero-deepseek-harness-gui** — 占位待复核，判定暂不展示
- `[未定位]` **h2o-mero-dsh-git-sidebar** — 占位待复核，判定暂不展示
- `[未定位]` **hackerfish-dsh-voice-input** — 占位待复核，判定暂不展示
- `[未定位]` **haibala-aii-dsh-extensions-wallpaperskin** — 占位待复核，判定暂不展示
...
- `[未定位]` **harcochen-dsh-plugin-guide** — 占位待复核，判定暂不展示
- `[未定位]` **havingautism-dsh-ultra-ui** — 占位待复核，判定暂不展示
- `[未定位]` **haythamat-dsh-client-ui-rtl** — 占位待复核，判定暂不展示
- `[未定位]` **heigeai-deepseek-harness-skin** — 占位待复核，判定暂不展示
...
- `[未定位]` **heyflyingpig-long-draft-input** — 占位待复核，判定暂不展示
...
- `[未定位]` **hoobooyoo-deepseek-harness-tui** — 占位待复核，判定暂不展示
- `[未定位]` **hotsteel2901-dsh-client-ui-mobile-adapt** — 占位待复核，判定暂不展示
- `[未定位]` **ht426-deepseek-harness-tui** — 占位待复核，判定暂不展示
- `[未定位]` **huabai-flowerwhite-dsh-ui-skin** — 占位待复核，判定暂不展示
- `[未定位]` **huadong19970512-cyber-deepseek-tui** — 占位待复核，判定暂不展示
- `[未定位]` **huangdoup-dsh-client-ui-notifications** — 占位待复核，判定暂不展示
- `[未定位]` **huangruiteng-loopx** — 占位待复核，判定暂不展示
- `[未定位]` **huanlinoto-dsh-plugin-better-sidebar-plugin-o** — 占位待复核，判定暂不展示
- `[未定位]` **huanlinoto-dsh-plugin-input-history** — 占位待复核，判定暂不展示
- `[未定位]` **huanlinoto-dsh-plugin-sidebar-brand-text** — 占位待复核，判定暂不展示
- `[未定位]` **huanxi-drxx-dsh-client-ui-mobile** — 占位待复核，判定暂不展示
...
- `[未定位]` **huyansheng3-dsh-skin** — 占位待复核，判定暂不展示
- `[未定位]` **hyposelen1a-dsh-columbina-theme** — 占位待复核，判定暂不展示
...
- `[未定位]` **imtanhui-dsh-plugin-butler** — 占位待复核，判定暂不展示
- `[未定位]` **imzl-zl-dsh-mcp-manager-ui** — 占位待复核，判定暂不展示
- `[未定位]` **ink5897-dsh-theme-kit** — 占位待复核，判定暂不展示
...
- `[未定位]` **iuikj-dsh-desktop** — 占位待复核，判定暂不展示
...
- `[未定位]` **jasondu-dsh-ui-mobile** — 占位待复核，判定暂不展示
...
- `[未定位]` **jcaiagent7143-ui-sendpage-mcp** — 占位待复核，判定暂不展示
...
- `[未定位]` **jianghuife-dsh-okf-knowledge** — 占位待复核，判定暂不展示
- `[未定位]` **jianghuife-dsh-open-auth-plugin** — 占位待复核，判定暂不展示
...
- `[未定位]` **jiesou-dsh-webui-fix-pack** — 占位待复核，判定暂不展示
...
- `[未定位]` **jiruidai-dsh-meta-orchestrator** — 占位待复核，判定暂不展示
- `[未定位]` **jkamkk-dsh-liquid-glass-input** — 占位待复核，判定暂不展示
- `[未定位]` **jmctsh-dsh-ui-desktop-pet** — 占位待复核，判定暂不展示
...
- `[未定位]` **kipozannaki-dsh-gui** — 占位待复核，判定暂不展示
...
- `[未定位]` **kongxiangyiren-dhs-theme-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **kouyichi-dsh-tui-app** — 占位待复核，判定暂不展示
- `[未定位]` **kui123456789-dsh-codex-workflow** — 占位待复核，判定暂不展示
- `[未定位]` **kunjinkao-os-dsh-mobile-gui-agent** — 占位待复核，判定暂不展示
- `[未定位]` **l-vm2k-dsh-ayaka-theme** — 占位待复核，判定暂不展示
- `[未定位]` **lan-tina-ws-dsh-gui-customization** — 占位待复核，判定暂不展示
...
- `[未定位]` **leo-aba-dsh-skins** — 占位待复核，判定暂不展示
- `[未定位]` **leoq-bit-dsh-theme-bside-olivia-lin** — 占位待复核，判定暂不展示
- `[未定位]` **leozou320-ai-dsh-web-speech-input** — 占位待复核，判定暂不展示
...
- `[未定位]` **libinghui55-dsh-tavily-search** — 占位待复核，判定暂不展示
- `[未定位]` **licheng-ma-dsh-wechat-skin** — 占位待复核，判定暂不展示
- `[未定位]` **lichtspur-deepseek-style-theme** — 占位待复核，判定暂不展示
- `[未定位]` **lijian-ui-dsh-desktop** — 占位待复核，判定暂不展示
- `[未定位]` **lijian-ui-dsh-file-manager** — 占位待复核，判定暂不展示
- `[未定位]` **lijian-ui-dsh-session-cleaner** — 占位待复核，判定暂不展示
- `[未定位]` **lijian-ui-dsh-skill-manage** — 占位待复核，判定暂不展示
- `[未定位]` **lijian-ui-dsh-term** — 占位待复核，判定暂不展示
- `[未定位]` **liliucourier-dsh-note-sidebar** — 占位待复核，判定暂不展示
...
- `[未定位]` **lingxin-maz-dsh-profilespanel** — 占位待复核，判定暂不展示
- `[未定位]` **linshufan21-dsh-ui-font** — 占位待复核，判定暂不展示
...
- `[未定位]` **liuyun847-dsh-client-ui-peak-valley** — 占位待复核，判定暂不展示
- `[未定位]` **liuyun847-dsh-client-ui-turn-chime** — 占位待复核，判定暂不展示
...
- `[未定位]` **luoying2334-dsh-plugin-skill-manager-gui** — 占位待复核，判定暂不展示
- `[未定位]` **lvyuchuiyi-dsh-funpack** — 占位待复核，判定暂不展示
...
- `[未定位]` **lyq3-dsh-skin-nebula** — 占位待复核，判定暂不展示
...
- `[未定位]` **makuralymi-dsh-webui-glass-theme** — 占位待复核，判定暂不展示
- `[未定位]` **mannixhu-dsh-statusbar-config** — 占位待复核，判定暂不展示
- `[未定位]` **manuspurs-dsh-skin20260814** — 占位待复核，判定暂不展示
...
- `[未定位]` **mercy719-dsh-client-ui-weather** — 占位待复核，判定暂不展示
...
- `[未定位]` **mux9056-bot-dsh-theme** — 占位待复核，判定暂不展示
- `[未定位]` **mykeura-dsh-minimalist-themes** — 占位待复核，判定暂不展示
- `[未定位]` **n3kok0-dsh-md3-theme** — 占位待复核，判定暂不展示
- `[未定位]` **nailing10086-zx-dsh-custom-ui** — 占位待复核，判定暂不展示
- `[未定位]` **naniwet-dsh-themes** — 占位待复核，判定暂不展示
...
- `[未定位]` **nekogpt-dsh-ui-quote-selection** — 占位待复核，判定暂不展示
- `[未定位]` **nevertoday-dsh-theme-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **newdanew-dsh-voice-input** — 占位待复核，判定暂不展示
- `[未定位]` **ngk3pori-dsh-zh-cn-ui** — 占位待复核，判定暂不展示
- `[未定位]` **nianchen8-dsh-mcp-panel** — 占位待复核，判定暂不展示
- `[未定位]` **nicecx-dsh-matrix-skin** — 占位待复核，判定暂不展示
- `[未定位]` **niiang-dsh-kimino-theme** — 占位待复核，判定暂不展示
- `[未定位]` **nimustudio-nimuqdock-dsh** — 占位待复核，判定暂不展示
- `[未定位]` **nineandnine-9-dsh-theme-rheostat** — 占位待复核，判定暂不展示
...
- `[未定位]` **oceanxuikun-dsh-eva-theme-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **oil-oil-dsh-theme** — 占位待复核，判定暂不展示
- `[未定位]` **omdsh-dev-dsh-annotation** — 占位待复核，判定暂不展示
- `[未定位]` **omdsh-dev-dsh-genui** — 占位待复核，判定暂不展示
- `[未定位]` **omdsh-dev-dsh-web-ui-notify** — 占位待复核，判定暂不展示
- `[未定位]` **opensquad-ai-dsh-voice-input** — 占位待复核，判定暂不展示
- `[未定位]` **orangeofcarl0-sys-j-space-cognition-suite-vne** — 占位待复核，判定暂不展示
- `[未定位]` **orriduck-dsh-tui** — 占位待复核，判定暂不展示
- `[未定位]` **orxz-deepseek-harness-themes** — 占位待复核，判定暂不展示
- `[未定位]` **ossfrankfrank-dsh-dracula-theme** — 占位待复核，判定暂不展示
- `[未定位]` **ostar999-ostar-dsh-left-sidebar** — 占位待复核，判定暂不展示
...
- `[未定位]` **pang123hui-dsh-workspace-sidebar** — 占位待复核，判定暂不展示
- `[未定位]` **peng-huiyang-cronjob-dsh-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **pengls-dsh-quick-view** — 占位待复核，判定暂不展示
- `[未定位]` **penguin-oo-dsh-bookmarks** — 占位待复核，判定暂不展示
- `[未定位]` **penguin-oo-dsh-delegate-router** — 占位待复核，判定暂不展示
- `[未定位]` **penguin-oo-dsh-pathlink** — 占位待复核，判定暂不展示
...
- `[未定位]` **perrylink-dsh-mcp-panel** — 占位待复核，判定暂不展示
- `[未定位]` **perrylink-dsh-plugin-guide** — 占位待复核，判定暂不展示
...
- `[未定位]` **pineapple880066-dsh-webui-pets** — 占位待复核，判定暂不展示
...
- `[未定位]` **preflower-deepseek-harness-compaction-ui** — 占位待复核，判定暂不展示
...
- `[未定位]` **qingli-sketch-dsh-ui-kit** — 占位待复核，判定暂不展示
...
- `[未定位]` **qiuchunhuimax-dsh-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **qiushui0901-deepseek-harness-desktop** — 占位待复核，判定暂不展示
- `[未定位]` **qschen86-dsh-workspace-dock** — 占位待复核，判定暂不展示
- `[未定位]` **qt-chen-dsh-mic-input** — 占位待复核，判定暂不展示
- `[未定位]` **quan-v-dsh-mcp-ui** — 占位待复核，判定暂不展示
- `[未定位]` **quanqqq-dsh-mobile-ui** — 占位待复核，判定暂不展示
- `[未定位]` **que3sui-dsh-darwin** — 占位待复核，判定暂不展示
- `[未定位]` **quicksandznzn-dsh-session-html** — 占位待复核，判定暂不展示
- `[未定位]` **quqxui-dsh-memgas** — 占位待复核，判定暂不展示
- `[未定位]` **qweqwe12382-dsh-client-ui-pet** — 占位待复核，判定暂不展示
- `[未定位]` **qwertyuiop314-dsh-conversation-export** — 占位待复核，判定暂不展示
- `[未定位]` **qwertyuiop314-dsh-workspace-picker-enhance** — 占位待复核，判定暂不展示
- `[未定位]` **qwq-nn-dsh-client-ui-trajectory-categories** — 占位待复核，判定暂不展示
- `[未定位]` **ragnarpitla-dsh-field-guide** — 占位待复核，判定暂不展示
- `[未定位]` **rainbowdashy-dsh-theme-palettes** — 占位待复核，判定暂不展示
- `[未定位]` **rainpomelo-deepseek-harness-liquid-glass-them** — 占位待复核，判定暂不展示
- `[未定位]` **rayafriandion-dsh-oc-tui** — 占位待复核，判定暂不展示
- `[未定位]` **rayyeung1989-claude-parchment-theme** — 占位待复核，判定暂不展示
- `[未定位]` **reachforstar-dsh-client-ui-polish** — 占位待复核，判定暂不展示
- `[未定位]` **realalexandreai-dsh-atuin** — 占位待复核，判定暂不展示
...
- `[未定位]` **renat3u-tonghuashun-webui** — 占位待复核，判定暂不展示
...
- `[未定位]` **richard-yang0130-dsh-web-speech-input** — 占位待复核，判定暂不展示
- `[未定位]` **richfugui001-netizen-dsh-file-review** — 占位待复核，判定暂不展示
...
- `[未定位]` **ropz12138-dsh-ui-background** — 占位待复核，判定暂不展示
- `[未定位]` **ruijiaang-lab-dsh-wallpaper-engine** — 占位待复核，判定暂不展示
...
- `[未定位]` **shouwang37-dsh-comfyui-node-development** — 占位待复核，判定暂不展示
- `[未定位]` **shuigege2025-dev-cloud-smoke-widget** — 占位待复核，判定暂不展示
- `[未定位]` **shuihan268-dsh-qwen-mm-plugins** — 占位待复核，判定暂不展示
- `[未定位]` **shuimo07-xiaoliuren** — 占位待复核，判定暂不展示
- `[未定位]` **shuimowang-deepseek-harness-desktop** — 占位待复核，判定暂不展示
- `[未定位]` **shuind-dsh-codex-harness** — 占位待复核，判定暂不展示
- `[未定位]` **shuiqiongchuyunqishi-disk-clean-cli** — 占位待复核，判定暂不展示
- `[未定位]` **shyboy-dsh-k12-lesson-builder** — 占位待复核，判定暂不展示
...
- `[未定位]` **sutanna233-dsh-voice-input** — 占位待复核，判定暂不展示
...
- `[未定位]` **themyceliumofantan-dsh-nachoneko-theme** — 占位待复核，判定暂不展示
- `[未定位]` **theoz001-dsh-message-annotator** — 占位待复核，判定暂不展示
- `[未定位]` **thjyy-dph-endfield-theme** — 占位待复核，判定暂不展示
- `[未定位]` **thooy-dsh-client-ui-theme-manager** — 占位待复核，判定暂不展示
- `[未定位]` **tiantyu-dsh-skin-toggle** — 占位待复核，判定暂不展示
- `[未定位]` **tiger3807861189-j-space-cognition-suite-v3-7** — 占位待复核，判定暂不展示
- `[未定位]` **timsok-shit-dsh-quick-resume** — 占位待复核，判定暂不展示
- `[未定位]` **tinnyxx-dsh-xiaoxinxin-skin** — 占位待复核，判定暂不展示
...
- `[未定位]` **tpmoonchefryan-dsh-joi-channel-theme** — 占位待复核，判定暂不展示
...
- `[未定位]` **uckkk-dsh-data-visualization** — 占位待复核，判定暂不展示
...
- `[未定位]` **uckkk-dsh-skin-pack** — 占位待复核，判定暂不展示
- `[未定位]` **uckkk-dsh-skin-type** — 占位待复核，判定暂不展示
- `[未定位]` **uckkk-dsh-spirit-guide** — 占位待复核，判定暂不展示
- `[未定位]` **uckkk-dsh-tune-guide** — 占位待复核，判定暂不展示
- `[未定位]` **uigdwunm-dsh-conversation-nav** — 占位待复核，判定暂不展示
- `[未定位]` **uigdwunm-dsh-process-fold** — 占位待复核，判定暂不展示
- `[未定位]` **uigdwunm-dsh-ui-suite** — 占位待复核，判定暂不展示
- `[未定位]` **ultronen-dsh-liquid-glass** — 占位待复核，判定暂不展示
- `[未定位]` **unpain-dsh-skins** — 占位待复核，判定暂不展示
- `[未定位]` **v587d-dsh-custom-skin** — 占位待复核，判定暂不展示
- `[未定位]` **valuz-ai-dsh-valuz-genui** — 占位待复核，判定暂不展示
...
- `[未定位]` **viveksssss-deepseekharness-tui** — 占位待复核，判定暂不展示
- `[未定位]` **viy1204-recruiting-copilot** — 占位待复核，判定暂不展示
- `[未定位]` **vlln-dsh-navbar** — 占位待复核，判定暂不展示
- `[未定位]` **vuitier-dsh-sound-notify** — 占位待复核，判定暂不展示
- `[未定位]` **waldsatte-dsh-theme-taojian** — 占位待复核，判定暂不展示
- `[未定位]` **wang-bool-visual-review** — 占位待复核，判定暂不展示
- `[未定位]` **wangwei-wade-dsh-quote-annotate** — 占位待复核，判定暂不展示
- `[未定位]` **wangxilhy23-dsh-wx-skin** — 占位待复核，判定暂不展示
- `[未定位]` **wanzhiwei5-dsh-skin-amis** — 占位待复核，判定暂不展示
- `[未定位]` **warmwine-dsh-ui-font** — 占位待复核，判定暂不展示
- `[未定位]` **waynezprog-dsh-tui** — 占位待复核，判定暂不展示
- `[未定位]` **wbin0001-dsh-comfyui-canvas** — 占位待复核，判定暂不展示
- `[未定位]` **webkubor-dsh-bloom-theme** — 占位待复核，判定暂不展示
- `[未定位]` **wecury-dsh-owui-chat2api** — 占位待复核，判定暂不展示
- `[未定位]` **wedomizing-dsh-genshin-nicole-skin** — 占位待复核，判定暂不展示
- `[未定位]` **wei-806206088-dsh-skin** — 占位待复核，判定暂不展示
...
- `[未定位]` **wenliang9527-dsh-themes** — 占位待复核，判定暂不展示
- `[未定位]` **wenyixiaoqingnian-ds-mobile-skin** — 占位待复核，判定暂不展示
- `[未定位]` **wenzetan-dsh-quota-panel** — 占位待复核，判定暂不展示
...
- `[未定位]` **wj2514939573-ui-dsh-text-master** — 占位待复核，判定暂不展示
- `[未定位]` **wjinxuan-dsh-ai-ui-studio** — 占位待复核，判定暂不展示
- `[未定位]` **wjnct55555-dsh-crt-theme** — 占位待复核，判定暂不展示
- `[未定位]` **wkscc310-dsh-client-ui-cpa-quota** — 占位待复核，判定暂不展示
- `[未定位]` **wlj521-dsh-ui-tweaks** — 占位待复核，判定暂不展示
- `[未定位]` **woei66-deepseek-harness-tui** — 占位待复核，判定暂不展示
...
- `[未定位]` **wuykjl-dsh-equip-engine** — 占位待复核，判定暂不展示
- `[未定位]` **wuzhong962-alt-dsh-matrix-theme** — 占位待复核，判定暂不展示
- `[未定位]` **wwwanghua-dsh-liquid-glass** — 占位待复核，判定暂不展示
- `[未定位]` **wycto-dsh-balance-panel** — 占位待复核，判定暂不展示
- `[未定位]` **wycto-dsh-dock** — 占位待复核，判定暂不展示
- `[未定位]` **wyh66666666-dsh-transparent-ui-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **x2802490130-prog-dsh-client-ui-writing** — 占位待复核，判定暂不展示
- `[未定位]` **x9wd09ncc-dsh-x9-theme** — 占位待复核，判定暂不展示
- `[未定位]` **xain-ui-beep** — 占位待复核，判定暂不展示
- `[未定位]` **xanthanl-dsh-plugin-uisfx** — 占位待复核，判定暂不展示
- `[未定位]` **xaviermarquis93-dsh-plugin-conversation-outli** — 占位待复核，判定暂不展示
...
- `[未定位]` **ximisi-dsh-quick-attach** — 占位待复核，判定暂不展示
- `[未定位]` **xing-shuyin-ds-web-ui** — 占位待复核，判定暂不展示
- `[未定位]` **xing-shuyin-dsh-ui-tools** — 占位待复核，判定暂不展示
- `[未定位]` **xing-shuyin-pi-web-ui** — 占位待复核，判定暂不展示
- `[未定位]` **xingke-seeker-dsh-ui-notify** — 占位待复核，判定暂不展示
- `[未定位]` **xingyingyuzhui-dsh-agent-policy** — 占位待复核，判定暂不展示
- `[未定位]` **xingyingyuzhui-dsh-chat-tune** — 占位待复核，判定暂不展示
- `[未定位]` **xingyingyuzhui-dsh-claw-suite** — 占位待复核，判定暂不展示
- `[未定位]` **xingyingyuzhui-dsh-liquid-glass** — 占位待复核，判定暂不展示
- `[未定位]` **xingyingyuzhui-dsh-observability** — 占位待复核，判定暂不展示
- `[未定位]` **xingyingyuzhui-dsh-restart** — 占位待复核，判定暂不展示
- `[未定位]` **xingyingyuzhui-dsh-session-actions** — 占位待复核，判定暂不展示
- `[未定位]` **xingyingyuzhui-dsh-skill-manager** — 占位待复核，判定暂不展示
- `[未定位]` **xingyingyuzhui-dsh-updater-ui** — 占位待复核，判定暂不展示
- `[未定位]` **xiyue718-dsh-ui-external-vision** — 占位待复核，判定暂不展示
- `[未定位]` **xiyue718-dsh-ui-plugin-manager** — 占位待复核，判定暂不展示
...
- `[未定位]` **xmanrui-dsh-im** — 占位待复核，判定暂不展示
...
- `[未定位]` **xtxo-dsh-ui** — 占位待复核，判定暂不展示
- `[未定位]` **xuanyuanchumo-dsh-codegraph-visualizer** — 占位待复核，判定暂不展示
- `[未定位]` **xuboboo-dsh-gui** — 占位待复核，判定暂不展示
...
- `[未定位]` **y2zyyr-dsh-token-usage-sidebar** — 占位待复核，判定暂不展示
- `[未定位]` **yangatc-dsh-theme** — 占位待复核，判定暂不展示
- `[未定位]` **yangeyu-deepseek-harness-tui** — 占位待复核，判定暂不展示
- `[未定位]` **yanhuifair-dsh-money** — 占位待复核，判定暂不展示
- `[未定位]` **ycqaq233-dsh-unknown-theme** — 占位待复核，判定暂不展示
- `[未定位]` **yeruizhi-dsh-lark-meeting-notifier** — 占位待复核，判定暂不展示
- `[未定位]` **yhfgyyf-dsh-tui-app** — 占位待复核，判定暂不展示
- `[未定位]` **yhprime-dsh-theme-picker** — 占位待复核，判定暂不展示
- `[未定位]` **yichangyaoyue-dsh-client-ui-mermaid** — 占位待复核，判定暂不展示
...
- `[未定位]` **yjm110517-visual-to-editable-ppt-skill** — 占位待复核，判定暂不展示
- `[未定位]` **ylinghao-dsh-codeui** — 占位待复核，判定暂不展示
- `[未定位]` **ylouis8-kph** — 占位待复核，判定暂不展示
- `[未定位]` **ylqit-dsh-skin-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **ymh0000123-dsh-theme-endfield** — 占位待复核，判定暂不展示
- `[未定位]` **ymr6666-deepseekharnessui** — 占位待复核，判定暂不展示
- `[未定位]` **yoli-mi-dsh-client-ui-custom** — 占位待复核，判定暂不展示
- `[未定位]` **yolorouter-dsh-llmasking** — 占位待复核，判定暂不展示
- `[未定位]` **yonglun-deepseek-harness-themes** — 占位待复核，判定暂不展示
- `[未定位]` **young1lin-dsh-ui-gitworkbench** — 占位待复核，判定暂不展示
...
- `[未定位]` **yu2025-luo-dsh-file-panel** — 占位待复核，判定暂不展示
...
- `[未定位]` **yuhangkang-dsh-ide-sidebar** — 占位待复核，判定暂不展示
- `[未定位]` **yuhui-sama-dsh-agentsoul** — 占位待复核，判定暂不展示
...
- `[未定位]` **yumimanji-dsh-ui-spec** — 占位待复核，判定暂不展示
- `[未定位]` **yunfei07-dsh-song-ui** — 占位待复核，判定暂不展示
- `[未定位]` **yunxiiqwq-dsh-maid-whale-webui** — 占位待复核，判定暂不展示
- `[未定位]` **yuqisun-dsh-theme-machine** — 占位待复核，判定暂不展示
- `[未定位]` **yushi-xxh-dsh-homepage-skin** — 占位待复核，判定暂不展示
...
- `[未定位]` **zdjmrq-dsh-plugin-suite** — 占位待复核，判定暂不展示
- `[未定位]` **zdram-deepseek-harness-skin** — 占位待复核，判定暂不展示
- `[未定位]` **zenghuizhu69-hub-dsh-skin-blue-whale** — 占位待复核，判定暂不展示
- `[未定位]` **zeroa234-dsh-comfyui** — 占位待复核，判定暂不展示
- `[未定位]` **zhang-jiazhi-dsh-stats-panel** — 占位待复核，判定暂不展示
- `[未定位]` **zhang66633-dsh-pixel-ui** — 占位待复核，判定暂不展示
...
- `[未定位]` **zhouchengke2046-dsh-sidebar-balance** — 占位待复核，判定暂不展示
- `[未定位]` **zhoucourier-dsh-theme-whalegirl** — 占位待复核，判定暂不展示
- `[未定位]` **zhtx2024-dsh-skin-switcher** — 占位待复核，判定暂不展示
...
- `[未定位]` **zhuiyueya-dsh-voice** — 占位待复核，判定暂不展示
- `[未定位]` **zhxqc-dsh-oh-my-theme** — 占位待复核，判定暂不展示
- `[未定位]` **zink-ning-lkr-dsh-ui-state** — 占位待复核，判定暂不展示
- `[未定位]` **zjsthmjialin-commercial-ui-ux-codex-skill** — 占位待复核，判定暂不展示
- `[未定位]` **zjwzjw1018-git-dsh-cyberpunk-theme** — 占位待复核，判定暂不展示
- `[未定位]` **zjzqs-dsh-client-ui-voice-input** — 占位待复核，判定暂不展示
...
- `[未定位]` **zuishi2006-pixel-dsh-whale-pet** — 占位待复核，判定暂不展示
...
- ⬜ `[待定]` [bobostudio-deepseek-harness-desktop](https://github.com/bobostudio/deepseek-harness-desktop) ★0 — Unofficial portable Windows desktop client for DeepSeek Harness, built with Pake.
...
- 🟩 `[可用]` [cipher2026-dsh-idle-deepcompute](https://github.com/cipher2026/dsh-idle-deepcompute) ★0 — 深算 DeepCompute —— 终端风 AI 时代挂机游戏（DSH Web GUI 插件）
...
- 🟩 `[可用]` [launchmaniac-dsh-media-tools](https://github.com/launchmaniac/dsh-media-tools) ★0 — OpenRouter image, video, and speech generation as deepseek-harness tools — an out-of-tree profile bundle, no f
...
- 🟩 `[可用]` [mjn96-dsh-rhine-theme](https://github.com/mjn96/dsh-rhine-theme) ★0 — 明日方舟莱茵生命美术风格的deepseek harness皮肤主题
...
- 🟩 `[可用]` [overact-dsh-peak-indicator](https://github.com/overact/dsh-peak-indicator) ★0 — DeepSeek Peak / Off-Peak Time Indicator for DSH Web GUI (live status, timezone auto-detection, countdown, SVG
...
- 🟩 `[可用]` [siberiah2o-dsh-plugin-remote](https://github.com/siberiah2o/dsh-plugin-remote) ★0 — Remote access gateway for the DeepSeek Harness Web GUI: login auth + HTTP/WebSocket reverse proxy to the loopb
...
- 🟩 `[可用]` [weopenfire-git-dsh-plugin-pub-review](https://github.com/weopenfire-git/dsh-plugin-pub-review) ★0 — Publish-readiness review for DeepSeek Harness plugins: official docs freshness check (docs-check), 30+ static
...
- 🟩 `[可用]` [xiaheng1-dsh-turn-nav](https://github.com/xiaheng1/dsh-turn-nav) ★0 — DeepSeek Harness Web 对话轮次导航轨
...
- ⬜ `[待定]` [ismoss-dsh-harness-zh-l10n](https://github.com/ismoss/dsh-harness-zh-l10n) ★0 — DeepSeek Harness (dsh) 中文界面汉化补丁：五模式描述/权限标签/命令描述/梁神模式全部汉化，apply.bat 一键安装，拿到就能用
...
- `[未定位]` **andyahui-deepseek-harness-desktop** — 占位待复核，判定暂不展示
- `[未定位]` **boxeryao-deepseek-harness-tui** — 占位待复核，判定暂不展示
- `[未定位]` **citrusli2026-dsh-mobile-ui** — 占位待复核，判定暂不展示
- `[未定位]` **fishquito7-dsh-skill-viewer** — 占位待复核，判定暂不展示
- `[未定位]` **hilbert-beinghappy-deepseek-tui** — 占位待复核，判定暂不展示
- `[未定位]` **huiliyi37-dsh-tianshu-build** — 占位待复核，判定暂不展示
- `[未定位]` **julardepick-deepseek-harness-gui** — 占位待复核，判定暂不展示
- `[未定位]` **lehmaning-dsh-randomuuid-polyfill** — 占位待复核，判定暂不展示
- `[未定位]` **lucasn0820-dsh-skin-claude-code** — 占位待复核，判定暂不展示
- `[未定位]` **maque2333-dsh-ui-topbar-compact** — 占位待复核，判定暂不展示
- `[未定位]` **monkeydadlufy-dsh-gui** — 占位待复核，判定暂不展示
- `[未定位]` **penguinpanda-sebastian-kitchen-board** — 占位待复核，判定暂不展示
- `[未定位]` **rabbitknight-dsh-tui** — 占位待复核，判定暂不展示
- `[未定位]` **reshuibuduo-dsh-tmcra-memory** — 占位待复核，判定暂不展示
- `[未定位]` **shmilyol-galgame-skin** — 占位待复核，判定暂不展示
- `[未定位]` **sixtysevenlf-dsh-routing-suite-opencode-go** — 占位待复核，判定暂不展示
- `[未定位]` **wanchenrui-deepseek-harness-desktop** — 占位待复核，判定暂不展示
- `[未定位]` **zhishui666-dsh-vision-relay** — 占位待复核，判定暂不展示
...
- `[未定位]` **deepseekharness-claude-theme** — 占位待复核，判定暂不展示
- `[未定位]` **deepseek_harness_ui_schema_fix** — 占位待复核，判定暂不展示
- `[未定位]` **dhs-theme-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-aigc-canvas** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-anti-ads** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-atuin** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-client-ui-peak-valley** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-client-ui-side-chat** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-gui** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-miku-skin** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-mobile-ui** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-nachoneko-theme** — 占位待复核，判定暂不展示
- ⬜ `[未测]` [dsh-note-sidebar](https://github.com/liliuCourier/dsh-note-sidebar) ★0 — ?????:?????????????????,?????????,???????????(DeepSeek Harness ??)
- `[未定位]` **dsh-randomuuid-polyfill** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-theme-ti** — 占位待复核，判定暂不展示
- `[未定位]` **dshtui-by-woodwhite** — 占位待复核，判定暂不展示
- `[未定位]` **Dsh-UI-Enhance** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-ui-topbar-compact** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-web-ui-approval-notify** — 占位待复核，判定暂不展示
- `[未定位]` **uiopt** — 占位待复核，判定暂不展示
- `[未定位]` **ya-workspace-sidebar** — 占位待复核，判定暂不展示
- `[未定位]` **DeepSeekHarness-DesktopUI** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-plugin-balance-panel** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-plugin-ui-turnav** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-genshin-skin** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-skins** — 占位待复核，判定暂不展示 〔PR〕
- `[未定位]` **claude-parchment-theme** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-mic-input** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-theme-palettes** — 占位待复核，判定暂不展示
- `[未定位]` **DeepSeekHarness-Tui** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-skin-amis** — 占位待复核，判定暂不展示
...
- ⬜ `[未测]` [dsh-office](https://github.com/fayelin12/dsh-office) — 办公室工作区/会话仪表盘：悬浮 6 列精灵面板，可视化工作区、会话、token 用量与子代理，内置 Agent 邮箱、飞书消息、会议日程、妙记逐字稿与办公室日志（web bundle，v0.3.0）
- ⬜ `[未测]` [dsh-split-panes](https://github.com/dsh-external/dsh-split-panes) — 分屏面板
- ⬜ `[未测]` [dsh-docker](https://github.com/stardustlc666/dsh-docker) — 容器管理插件：docker_ps/logs/inspect/exec/manage/health 六工具，官方 subprocess 服务、argv 无 shell 注入、exec 审批门、守护进程自检、零运行时依赖；npm 包为 scoped 名 @stardustlc/dsh-docker
- ⬜ `[未测]` [dsh-sidebar-mode](https://github.com/meredith2328/dsh-sidebar-mode) — 侧边栏「新会话」按钮内嵌 Agent 预设快速切换：点击弹出预设菜单即点即用，与设置里的「Agent 预设」双向同步
- ⬜ `[未测]` [dsh-mdbox](https://github.com/chi-hong22/dsh-mdbox) — DSH Web 输入框 Markdown 编辑辅助：Shift+Enter 列表续行与空项退出、有序列表自动重编号、Tab/Shift+Tab 双向缩进；纯客户端零运行时依赖，不碰文件/网络/凭据
- ⬜ `[未测]` [dsh-web-access](https://github.com/nexusagentx/dsh-web-access) — 多提供方联网：web_search / fetch_content / source_check，注册 ctx.web 的 web-access 搜索/抓取提供方，Web 面板改配置与策展；npm `dsh-web-access`
- ⬜ `[未测]` [dsh-lens](https://github.com/nexusagentx/dsh-lens) — 写/改文件时的实时代码反馈：LSP / linter / formatter / ast-grep / symbol_search，Web chip+dock；npm `dsh-lens`
...
- ⬜ `[未测]` [dsh-task-dag](https://github.com/leemancheung/dsh-task-dag) — 由投影驱动的会话子代理与持久工作流实时 DAG：状态与节点导航、深层链路确定性布局、适应/平移画布，以及当前会话内节点拖动重排；无并行数据库或 Host 轮询
- ⬜ `[未测]` [dsh-opencodego-usage](https://github.com/beizi6/dsh-opencodego-usage) — OpenCodeGo 剩余额度监视器：输入框右下角呼吸灯（按剩余额度绿/黄/红）+ 液态玻璃面板（滚动/周/月三窗口用量与重置时间），每 30 秒自动刷新，Key 自动读取 DSH 凭据（opencode-go 提供商）也可手动覆盖
- ⬜ `[未测]` [dsh-session-hotkeys](https://github.com/yeyeyeyeshifu/dsh-session-hotkeys) — Web UI 会话快捷键：Alt+1-9 顺序切换、固定槽位三态、Alt+↑↓ 循环切换、高亮导航模式、归档/重命名/新建会话、可改键面板（键盘导航 + 内置诊断），Windows/macOS 双预设无冲突，npm 已发布
...
- ⬜ `[未测]` [dsh-feishu](https://github.com/pgzxb/dsh-feishu) — DeepSeek Harness 的飞书 UI：面板驱动控制台，卡内审批与提问，流式卡片，扫码一键配置
- 🟨 `[不兼容]` [mem0ai-mem0](https://github.com/mem0ai/mem0) ★65507 — The Memory Layer for AI Agents - Drop-in memory infrastructure for AI agents and apps
- 🟨 `[不兼容]` [yjh051108-dsh-routing-suite](https://github.com/yjh051108/dsh-routing-suite) ★7192 — dsh-routing-suite — injector + router-standard kit: install the runtime injector first, then the task-aware re
- 🟨 `[不兼容]` [chuspeeism-dashi-taskboard](https://github.com/chuspeeism/dashi-taskboard) ★3134 — 现代化可灵活嵌入的任务面板，支持 Codex、DeepSeek Harness
- 🟨 `[不兼容]` [ningbainb-deepseek-harness-desktop](https://github.com/ningbainb/deepseek-harness-desktop) ★599 — Open-source Windows desktop client and GUI for DeepSeek Harness — zero-setup installer with Codex, plugins, sk
- 🟨 `[不兼容]` [superdesigndev-superdesign-skill](https://github.com/superdesigndev/superdesign-skill) ★571 — The design skill for Claude Code, Cursor and any coding agent
- 🟨 `[不兼容]` [kelai141-dsh-mobile-apk](https://github.com/kelai141/dsh-mobile-apk) ★463 — dsh 安卓壳 APK——WebView UI + 内嵌 Termux 运行时快照（解压即跑），为dsh本地运行设计的高性能方案
- 🟨 `[不兼容]` [oh-dsh](https://github.com/hust-open-atom-club/oh-dsh) ★317 — 一套 DSH runtime，Desktop、Web 与 TUI 三种开发体验
...
- 🟨 `[不兼容]` [zhu1090093659-dsh-trading](https://github.com/zhu1090093659/dsh-trading) ★178 — Agent-native trading terminal built on DeepSeek Harness
- 🟨 `[不兼容]` [toolclub-dsh-agent-team-gui](https://github.com/toolclub/dsh-agent-team-gui) ★176 — Persistent multi-model workflow teams for DeepSeek Harness — dynamic lead planning, bounded DAGs, per-agent mo
- 🟨 `[不兼容]` [dsh-super-injector](https://github.com/yjh051108/dsh-super-injector) ★162 — 推荐组件（非必须）：DeepSeek Harness 运行时注入器；已随 dsh-routing-suite 单仓库化保留，本仓库继续维护/发布
- 🟨 `[不兼容]` [humblebanana-open-record-replay](https://github.com/humblebanana/open-record-replay) ★144 — Open-source macOS record-and-replay workflow recorder for computer use agents
...
- 🟨 `[不兼容]` [pengyue-polaron-deepseek-harness-genui](https://github.com/pengyue-polaron/deepseek-harness-genui) ★111 — Task-specific React apps for DeepSeek Harness with state carried into the next Agent turn
- 🟨 `[不兼容]` [bruc3van-dsh-desktop](https://github.com/bruc3van/dsh-desktop) ★92 — 一款主打安全且更尊重开发者的开源 DeepSeek Harness 桌面客户端
- 🟨 `[不兼容]` [runzhliu-deepseek-harness-docker](https://github.com/runzhliu/deepseek-harness-docker) ★84 — Community Docker and Kubernetes packaging for DeepSeek Harness (@deepseek-ai/dsh), with a hardened image, Comp
...
- 🟨 `[不兼容]` [rison114514-dsh-endfield-ui](https://github.com/rison114514/dsh-endfield-ui) ★66 — Endfield-inspired industrial UI shell for DeepSeek Harness (dsh) — non-official fan theme
...
- 🟨 `[不兼容]` [whyihaveyou-dsh-suite](https://github.com/whyihaveyou/dsh-suite) ★55 — The living DeepSeek Harness plugin directory — refreshed hourly, compat-tested daily, with an in-app plugin st
- 🟨 `[不兼容]` [kenz1117-dsh-ui-usage-billing](https://github.com/kenz1117/dsh-ui-usage-billing) ★53 — Usage billing dashboard plugin for DeepSeek Harness: sidebar cost metrics, real usage aggregation from session
- 🟨 `[不兼容]` [huaweicloud-huaweicloud-devkit](https://github.com/huaweicloud/huaweicloud-devkit) ★49 — Official Huawei Cloud plugin for AI agents — skills, MCP tools, safety guardrails, and cloud sandbox to help A
...
- 🟨 `[不兼容]` [zhuiyueya-dsh-im-gateway](https://github.com/zhuiyueya/dsh-im-gateway) ★46 — 把 dsh agent 接入微信、飞书等 20+ 聊天平台的聚合网关插件 \| Aggregate IM gateway for DeepSeek Harness (dsh): connect your agents to
- 🟨 `[不兼容]` [ui-status-label](https://github.com/alingalingling/ui-status-label) ★45 — 把你鲸鱼娘思考时的 deep diving 自定义成任意你想要的样子
- 🟨 `[不兼容]` [openma-ai-deepseek-harness-tui](https://github.com/openma-ai/deepseek-harness-tui) ★42 — Self-Improvement TUI Plugin of DeepSeek Harness
- 🟨 `[不兼容]` [liguobao-dsh-desktop](https://github.com/liguobao/dsh-desktop) ★38 — An independent, open-source desktop wrapper for DeepSeek Harness
...
- 🟨 `[不兼容]` [futrime-webdsh](https://github.com/futrime/webdsh) ★30 — Browser-only build of DeepSeek Harness
- 🟨 `[不兼容]` [chouxiaohuai-dsh-uiskin-theme](https://github.com/chouxiaohuai/dsh-uiskin-theme) ★27 — Blue Glass Theme (ocean fantasy) for DeepSeek Harness Web — static profile bundle
...
- 🟨 `[不兼容]` [rootkiller6788-dsh-launcher](https://github.com/rootkiller6788/dsh-launcher) ★23 — Unlock DeepSeek Harness plugin-native core capabilities, unifying Runtime, Provider, Plugin, Skill and MCP orc
- 🟨 `[不兼容]` [cofy-x-dsh-console](https://github.com/cofy-x/dsh-console) ★21 — DSH-native terminal workbench (TUI) for DeepSeek Harness with durable sessions, structured tools, plans, subag〔📦〕
- 🟨 `[不兼容]` [railgun0325-dsh-phone](https://github.com/railgun0325/dsh-phone) ★20 — 让 DeepSeek Harness 的 agent 跑在手机里，通过 Magisk root 原生操作安卓系统（截图/点击/滑动/开应用）+ 移动端布局 + WebView APK
...
- 🟨 `[不兼容]` [wingsky-1-dsh-plugin-hub](https://github.com/wingsky-1/dsh-plugin-hub) ★18 — DSH (DeepSeek Harness) web GUI plugin collection — task notifications, provider usage, LAN proxy, MCP manager,
...
- 🟨 `[不兼容]` [yxy050208-multisim-mcp](https://github.com/yxy050208/multisim-mcp) ★17 — Unofficial MCP server for AI-driven NI Multisim circuit generation, simulation, data export, and reports
...
- 🟨 `[不兼容]` [xiaoksio-dsh-solution-explorer](https://github.com/xiaoksio/dsh-solution-explorer) ★12 — DSH Web GUI right sidebar: VS Code-style file explorer plus source control (git status, stage/unstage/discard,
...
- 🟨 `[不兼容]` [dphmoblie-deepseek-harness-android](https://github.com/dphmoblie/deepseek-harness-android) ★10 — 该程序是一个独立的 Capacitor Android 应用，用于管理本机 DeepSeek Harness Ubuntu 用户空间
- 🟨 `[不兼容]` [demacia1314-dsh-airdrop](https://github.com/demacia1314/dsh-airdrop) ★9 — ⭐ 支持拖拽上传文件/文件夹到 DSH 窗口任意位置；支持远程上传，字节直传服务器会话工作区——部署在服务器上的 DSH 也能像本地一样拖了就发
...
- 🟨 `[不兼容]` [evoelsewhere-evoflux](https://github.com/evoelsewhere/evoflux) ★9 — Evoflux is an open-source, local-first workspace where AI agents build software, conduct deep research, automa
- 🟨 `[不兼容]` [fan56-dsh-tui-pi](https://github.com/fan56/dsh-tui-pi) ★9 — A fully-featured pi-style terminal UI for DeepSeek Harness (dsh) — history look-back & fork-at-turn, guided pr
...
- 🟨 `[不兼容]` [lutrodev-dsh-roleplay](https://github.com/lutrodev/dsh-roleplay) ★8 — Roleplay plugin suite for DeepSeek Harness: character cards, lorebooks, personas, presets, state, and conversa
- 🟨 `[不兼容]` [marisa-4219-deepseek-harness-quota-monitor](https://github.com/marisa-4219/deepseek-harness-quota-monitor) ★8 — DeepSeek Harness 多供应商额度监控插件：余额型 API 查询 + 限额型本地用量计量，侧边栏实时卡片 + 可视化配置
...
- 🟨 `[不兼容]` [diguike-book-deepseek-harness](https://github.com/diguike/book-deepseek-harness) ★7 — 《一切皆插件》— DeepSeek Harness 源码精读、Mini 实现与插件开发实战
...
- 🟨 `[不兼容]` [alone-tree-dsh-skill-mcp-manager](https://github.com/alone-tree/dsh-skill-mcp-manager) ★6 — 能力库 (Capability) — one-stop visual management of DSH Skills & MCP: on-demand MCP loading, in-session hot reloa
...
- 🟨 `[不兼容]` [dqsjqian-agent-guild](https://github.com/dqsjqian/agent-guild) ★5 — A protocol that lets any sufficiently intelligent AI agent join your shared memory by simply reading one file.
...
- 🟨 `[不兼容]` [jiaererw-dsh-plugin-chrome](https://github.com/jiaererw/dsh-plugin-chrome) ★5 — DeepSeek Harness browser visualization plugin: a visible Chrome window per session, 16 chrome_* agent tools, l
- 🟨 `[不兼容]` [keyiadiannao-dsh-power-button](https://github.com/keyiadiannao/dsh-power-button) ★5 — Self-contained power control for DeepSeek Harness: sidebar power button with upward restart/shutdown menu, Win
...
- 🟨 `[不兼容]` [orrinzeng-dsh-cursor-subscription](https://github.com/orrinzeng/dsh-cursor-subscription) ★5 — Log directly into your Cursor account within DeepSeek Harness and use your Cursor subscription — no API Key re
- 🟨 `[不兼容]` [rong-coder-dsh-office-tool](https://github.com/rong-coder/dsh-office-tool) ★5 — dsh plugin for DeepSeek Harness: complete Office document tools for the agent (create/read/edit .docx/.xlsx/.p
...
- 🟨 `[不兼容]` [3361805598-gif-dsh-md-annotator](https://github.com/3361805598-gif/dsh-md-annotator) ★4 — DSH Markdown sidebar viewer and editor with block- and text-range annotations for structured revision requests
- 🟨 `[不兼容]` [534119219-chicheng-quickinput](https://github.com/534119219/chicheng-quickinput) ★4 — DSH 便捷输入保险箱插件：输入栏「用量」与「发送」之间一键展开，自动收录会话中的密钥/网址/服务器/手机号等；点击填入或 Ctrl+点击直接发送；私密内容密码保护 + WebDAV 备份
...
- 🟨 `[不兼容]` [kovey-dsh-nvim-tui](https://github.com/kovey/dsh-nvim-tui) ★4 — Neovim-style TUI for DeepSeek Harness (dsh): nvim renders the terminal, a Node runner bridges DSH agent events
...
- 🟨 `[不兼容]` [squirrel20-dsh-cron](https://github.com/squirrel20/dsh-cron) ★4 — Unattended scheduled jobs for the DeepSeek Harness (dsh): agent/command tasks on cron schedules
...
- 🟨 `[不兼容]` [yu-xin-c-dsh-project-wiki](https://github.com/yu-xin-c/dsh-project-wiki) ★4 — Auditable workspace-local project Wiki with a native Web UI for DeepSeek Harness
- 🟨 `[不兼容]` [zhouzhencheng07-dsh-kit](https://github.com/zhouzhencheng07/dsh-kit) ★4 — Page capability kit for DeepSeek Harness (dsh): terminal dock, file tree, source control, skills manager, phon
...
- 🟨 `[不兼容]` [cherrchen-deepseek-harness-electron](https://github.com/cherrchen/deepseek-harness-electron) ★3 — DeepSeek Harness 非官方跨平台桌面封装，预装 Details Host 与 Git 插件；与上游同步〔📦〕
...
- 🟨 `[不兼容]` [memorax-ai-dsh-webui-studio](https://github.com/memorax-ai/dsh-webui-studio) ★3 — DeepSeek WebUI Studio for interactive, visual-first client plugin development
...
- 🟨 `[不兼容]` [soolaugust-deepseek-harness-cli](https://github.com/soolaugust/deepseek-harness-cli) ★3 — Interactive terminal agent (fork of DeepSeek Harness): Claude Code-style REPL — full-screen UI, markdown rende〔📦〕
...
- 🟨 `[不兼容]` [stoneface10-dsh-codex-connect-plus](https://github.com/stoneface10/dsh-codex-connect-plus) ★3 — Use your ChatGPT subscription in DeepSeek Harness to access Codex models and generate or edit images with gpt-
...
- 🟨 `[不兼容]` [21hbguo-dsh-session-batch-manager](https://github.com/21hbguo/dsh-session-batch-manager) ★2 — DeepSeek Harness (DSH) Web GUI 插件：批量选择会话进行归档、恢复与删除的管理面板
...
- 🟨 `[不兼容]` [boheastill-phone-eye](https://github.com/boheastill/phone-eye) ★2 — Let your AI agent see and operate a real Android phone — vision + UI-tree fusion over adb, for any MCP client
...
- 🟨 `[不兼容]` [daboge-beach-dsh-skin-studio](https://github.com/daboge-beach/dsh-skin-studio) ★2 — 🎨 DeepSeek Harness skin studio — built-in curated skins + a community skin gallery〔📦〕
...
- 🟨 `[不兼容]` [heekei-dsh-usage](https://github.com/heekei/dsh-usage) ★2 — DSH 供应商用量查询插件 — 实时查询 Kimi/智谱/DeepSeek/火山方舟等套餐与余额, 在输入框下方显示用量读数条
...
- 🟨 `[不兼容]` [ipromise2021-dsh-omc-tui](https://github.com/ipromise2021/dsh-omc-tui) ★2 — DeepSeek Harness (DSH) 原生全功能终端交互界面 · Claude Code-styled Terminal TUI & CLI for DeepSeek Harness
...
- 🟨 `[不兼容]` [liangyou09-lyshell](https://github.com/liangyou09/lyshell) ★2 — AI-native terminal & SSH client built for vibe coding
- 🟨 `[不兼容]` [lifeopsgo-dsh-capability-toggle-plugin](https://github.com/lifeopsgo/dsh-capability-toggle-plugin) ★2 — 灵活控制 mcp/skill/tool 等启用/禁用，支持会话、项目、全局级别控制
...
- 🟨 `[不兼容]` [motongv-dsh-voice](https://github.com/motongv/dsh-voice) ★2 — Voice input + Edge-TTS read-aloud plugin for DeepSeek Harness
...
- 🟨 `[不兼容]` [shenhuanageshei-dsh-thincoder-suite](https://github.com/shenhuanageshei/dsh-thincoder-suite) ★2 — DSH plugin porting thincoder self-discipline suite: advisor convergent review / engineering mode / escalate /
...
- 🟨 `[不兼容]` [suiyideali-dsh-selection-toolbar](https://github.com/suiyideali/dsh-selection-toolbar) ★2 — 在 DeepSeek Harness 会话里划选文本，选区上方浮现一个小工具栏： 复制 · 引用 · 询问 · 解释 · 翻译 · 总结 · 自定义
...
- 🟨 `[不兼容]` [xingyingyuzhui-dsh-vision-bench](https://github.com/xingyingyuzhui/dsh-vision-bench) ★2 — DSH plugin: Vision bench — Keil debug and Modbus HMI in the session view
...
- 🟨 `[不兼容]` [zrt-ai-lab-dsh-desktop](https://github.com/zrt-ai-lab/dsh-desktop) ★2 — Unofficial Windows desktop build of DeepSeek Harness - one installer, no prerequisites
...
- 🟨 `[不兼容]` [111222cjyq-dsh-branch-visualizer](https://github.com/111222cjyq/dsh-branch-visualizer) ★1 — Developer Preview: persistent DeepSeek Harness session branch visualizer with navigation, rename, archive, and
...
- 🟨 `[不兼容]` [a903067276-rgb-dsh-simple-memory](https://github.com/a903067276-rgb/dsh-simple-memory) ★1 — Layered markdown memory for DeepSeek Harness (dsh): per-session index injection, one-click memory-flow button,
...
- 🟨 `[不兼容]` [cyjyyd-dsh-ssh-tui](https://github.com/cyjyyd/dsh-ssh-tui) ★1 — SSH-friendly DeepSeek Harness TUI
...
- 🟨 `[不兼容]` [fazhu4-dsh-usage-insights](https://github.com/fazhu4/dsh-usage-insights) ★1 — dsh的本地用量面板，统计 Token、请求、Agent 活动、工具调用和性能
...
- 🟨 `[不兼容]` [luoxunhao-dsh-codex-project](https://github.com/luoxunhao/dsh-codex-project) ★1 — codex风格的项目管理，可以给dsh项目增加附加目录，访问附加目录不需要full-access权限，方便多项目开发，可配合dsh-better-sidebar插件使用
...
- 🟨 `[不兼容]` [saqie803-ponytail](https://github.com/saqie803/ponytail) ★1 — Ship production-ready code with one line of AI-generated output, built for 20 agents and zero ceremony.
...
- 🟨 `[不兼容]` [xia-sc-dsh-git](https://github.com/xia-sc/dsh-git) ★1 — DeepSeek Harness Web GUI 的完整 Git 管理插件：分支切换/fetch/pull/commit/push，可折叠悬浮面板 + 输入框胶囊
...
- 🟨 `[不兼容]` [xingyingyuzhui-dsh-agent-memory](https://github.com/xingyingyuzhui/dsh-agent-memory) ★1 — Claw persistent memory for DeepSeek Harness
...
- 🟨 `[不兼容]` [u9521-dsh-advanced-model-editor](https://github.com/u9521/dsh-advanced-model-editor) ★1 — DSH WebUI plugin for managing custom LLM providers, model parameters, thinking budgets, and request settings.
- 🟨 `[不兼容]` [hacxy-dsh-live2d](https://github.com/hacxy/dsh-live2d) ★1 — 一个用于DeepSeek Harness网页界面的Live2D宠物伴侣插件
...
- `[未定位]` **21hbguo-dsh-mobile-ui** — 占位待复核，判定暂不展示
- `[未定位]` **3361805598-gif-dsh-md-annotator** — 占位待复核，判定暂不展示
- `[未定位]` **534119219-chicheng-quickinput** — 占位待复核，判定暂不展示
- `[未定位]` **aa2246740-dsh-files-panel** — 占位待复核，判定暂不展示
...
- `[未定位]` **alingalingling-ui-status-label** — 占位待复核，判定暂不展示
...
- `[未定位]` **awa-123-cw-dsh-webui-perf** — 占位待复核，判定暂不展示
- `[未定位]` **awnlight-talon-ui** — 占位待复核，判定暂不展示
- `[未定位]` **baobaolaodie-dsh-tui-vscode** — 占位待复核，判定暂不展示
- `[未定位]` **benhuhuan-dhs-tuicode** — 占位待复核，判定暂不展示
- `[未定位]` **blaczz-dsh-deck-builder** — 占位待复核，判定暂不展示
- `[未定位]` **blueriverlhr-dsh-better-webui** — 占位待复核，判定暂不展示
- `[未定位]` **boboozeng-dsh-annotation-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **bramblexu-dsh-annotate** — 占位待复核，判定暂不展示
- `[未定位]` **brittanistrehlowll-oss-dsh-quota-panel** — 占位待复核，判定暂不展示
- `[未定位]` **bruce-hmz-dsh-skin-center** — 占位待复核，判定暂不展示
- `[未定位]` **buhuikongpan-dsh-win-gitbash** — 占位待复核，判定暂不展示
- `[未定位]` **bupianlizhugui-dsh-genshin-skin** — 占位待复核，判定暂不展示
...
- `[未定位]` **chajiuqqq-dsh-claude-theme** — 占位待复核，判定暂不展示
- `[未定位]` **chen-001-dsh-grok-tui** — 占位待复核，判定暂不展示
...
- `[未定位]` **chujianyun-deepseek-harness-skin** — 占位待复核，判定暂不展示
- `[未定位]` **cinnamobot-dsh-nothing-skin** — 占位待复核，判定暂不展示
- `[未定位]` **cnseniorious000-dsh-generative-ui** — 占位待复核，判定暂不展示
- `[未定位]` **cogine-ai-dsh-claude-tui** — 占位待复核，判定暂不展示
- `[未定位]` **crack-time-dsh-web-ui-skin** — 占位待复核，判定暂不展示
- `[未定位]` **cyjyyd-dsh-ssh-tui** — 占位待复核，判定暂不展示
- `[未定位]` **daboge-beach-dsh-skin-studio** — 占位待复核，判定暂不展示
- `[未定位]` **danielou1208-deepseek-harness-tui** — 占位待复核，判定暂不展示
...
- `[未定位]` **detongz-dsh-client-ui-obsidian-memory** — 占位待复核，判定暂不展示
- `[未定位]` **difimim-dsh-voice-input** — 占位待复核，判定暂不展示
- `[未定位]` **difimim-dsh-voice-input-npm** — 占位待复核，判定暂不展示
- `[未定位]` **diguike-book-deepseek-harness** — 占位待复核，判定暂不展示
...
- `[未定位]` **dqsjqian-agent-guild** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-tui-dsh-tui** — 占位待复核，判定暂不展示
- `[未定位]` **du-u-uck-dsh-transparent-ui-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **dycode7-deepseek-harness-tui** — 占位待复核，判定暂不展示
- `[未定位]` **enernitylune-dsh-luvian-ui-wallpaper** — 占位待复核，判定暂不展示
- `[未定位]` **equinox7379-dsh-config-watch** — 占位待复核，判定暂不展示
...
- `[未定位]` **equinox7379-dsh-update-radar** — 占位待复核，判定暂不展示
- `[未定位]` **euuuuuuzer-dsh-loop-dock** — 占位待复核，判定暂不展示
- `[未定位]` **exoticknight-dsh-theme-eink-retro** — 占位待复核，判定暂不展示
- `[未定位]` **fan56-dsh-tui-pi** — 占位待复核，判定暂不展示
- `[未定位]` **feiertu-dsh-input-rewriter** — 占位待复核，判定暂不展示
- `[未定位]` **festoney8-deepseek-harness-gui** — 占位待复核，判定暂不展示
- `[未定位]` **flametn7-dsh-tui-browser-use** — 占位待复核，判定暂不展示
- `[未定位]` **flaricy-maid-atelier-ui-bundle** — 占位待复核，判定暂不展示
...
- `[未定位]` **gameswu-dsh-plugin-vscode-sidebar** — 占位待复核，判定暂不展示
- `[未定位]` **ghbhiee-dsh-plugin-tui** — 占位待复核，判定暂不展示
- `[未定位]` **gmh13552-dsh-longrun-suite** — 占位待复核，判定暂不展示
- `[未定位]` **gxinxing-deepseek-harness-tui** — 占位待复核，判定暂不展示
...
- `[未定位]` **huanlinoto-dsh-plugin-ya-workspace-sidebar** — 占位待复核，判定暂不展示
- `[未定位]` **huiliyi37-dsh-tianshu-tui** — 占位待复核，判定暂不展示
...
- `[未定位]` **jiezeng2004-design-dsh-requirements-alignment** — 占位待复核，判定暂不展示
...
- `[未定位]` **kenz1117-dsh-ui-usage-billing** — 占位待复核，判定暂不展示
...
- `[未定位]` **le-soleil-se-couche-dsh-skin-claude-code** — 占位待复核，判定暂不展示
- `[未定位]` **lehhair-dsh-home-ui** — 占位待复核，判定暂不展示
...
- `[未定位]` **lhh010-dsh-input-history** — 占位待复核，判定暂不展示
- `[未定位]` **lhh010-dsh-paste-input** — 占位待复核，判定暂不展示
- `[未定位]` **lhh010-dsh-ui-progress** — 占位待复核，判定暂不展示
- `[未定位]` **lhh010-dsh-ui-whale** — 占位待复核，判定暂不展示
- `[未定位]` **lhy723-dsh-neu-theme** — 占位待复核，判定暂不展示
- `[未定位]` **liang7878-deepseek-harness-tui** — 占位待复核，判定暂不展示
- `[未定位]` **lihang-lh-dsh-task-panel** — 占位待复核，判定暂不展示
- `[未定位]` **lijian-ui-dsh-im-gateway** — 占位待复核，判定暂不展示
...
- `[未定位]` **lqhl-dsh-pi-tui** — 占位待复核，判定暂不展示
- `[未定位]` **lrplrplrp-dsh-sidebar-gdhighlight** — 占位待复核，判定暂不展示
- `[未定位]` **luis1232023-dsh-workspace-enhance** — 占位待复核，判定暂不展示
...
- `[未定位]` **mytianyi0712-dsh-tui-plugin-ohmypi** — 占位待复核，判定暂不展示
- `[未定位]` **nagasakisoyo-ui-dsh-llm-deepseek-vision** — 占位待复核，判定暂不展示
- `[未定位]` **nagi-ovo-dsh-music-tui** — 占位待复核，判定暂不展示
- `[未定位]` **nagi-ovo-dsh-visualize** — 占位待复核，判定暂不展示
- `[未定位]` **nexlineai-dsh-tui** — 占位待复核，判定暂不展示
- `[未定位]` **nexsjournal-dsh-customui-plugin** — 占位待复核，判定暂不展示
...
- `[未定位]` **omdsh-dev-dsh-better-sidebar** — 占位待复核，判定暂不展示
- `[未定位]` **omdsh-dev-dsh-input-history** — 占位待复核，判定暂不展示
- `[未定位]` **omdsh-dev-dsh-paste-input** — 占位待复核，判定暂不展示
- `[未定位]` **omdsh-dev-dsh-tui** — 占位待复核，判定暂不展示
- `[未定位]` **omdsh-dev-dsh-ui-progress** — 占位待复核，判定暂不展示
- `[未定位]` **omdsh-dev-dsh-ui-whale** — 占位待复核，判定暂不展示
...
- `[未定位]` **pany0593-dsh-ui-notifications** — 占位待复核，判定暂不展示
- `[未定位]` **papachong-deepseek-harness-tui** — 占位待复核，判定暂不展示
- `[未定位]` **pengyue-polaron-deepseek-harness-genui** — 占位待复核，判定暂不展示
- `[未定位]` **pgmi-builds-dashr** — 占位待复核，判定暂不展示
- `[未定位]` **pgnqukezrdxmhjso-dsh-ui-fortifier** — 占位待复核，判定暂不展示
- `[未定位]` **pixellover1433-dsh-better-sidebar-lite** — 占位待复核，判定暂不展示
- `[未定位]` **princefrogdida-ux-dsh-vision-suite** — 占位待复核，判定暂不展示
...
- `[未定位]` **siruignaw-sys-dsh-tool-bandit-search** — 占位待复核，判定暂不展示
- `[未定位]` **six6strings-dsh-client-ui-thinking-stats** — 占位待复核，判定暂不展示
- `[未定位]` **sixsixla-deepseek-harness-ui-schema-fix** — 占位待复核，判定暂不展示
- `[未定位]` **skyhancloud-dsh-client-ui-quote** — 占位待复核，判定暂不展示
- `[未定位]` **skylarking-dsh-sidebar** — 占位待复核，判定暂不展示
- `[未定位]` **slicenferqin-dsh-whale-tui** — 占位待复核，判定暂不展示
...
- `[未定位]` **stars2022-dsh-ui-concise** — 占位待复核，判定暂不展示
- `[未定位]` **statem-li-dsh-webui** — 占位待复核，判定暂不展示
- `[未定位]` **stayhear0722-dev-deepseek-harness-skin** — 占位待复核，判定暂不展示
- `[未定位]` **suibbinggan-dsh-cn-plugin-center** — 占位待复核，判定暂不展示
- `[未定位]` **suimi8-dsh-test-runner** — 占位待复核，判定暂不展示
- `[未定位]` **suixin04-dsh-session-migrator** — 占位待复核，判定暂不展示
- `[未定位]` **suiyideali-dsh-selection-toolbar** — 占位待复核，判定暂不展示
- `[未定位]` **sunyuhuirong-deepseek-account** — 占位待复核，判定暂不展示
- `[未定位]` **sunyuhuirong-fsviewer** — 占位待复核，判定暂不展示
- `[未定位]` **suzike-freestyle-dsh-theme** — 占位待复核，判定暂不展示
- `[未定位]` **tangbut1-dsh-plugin-voice-input** — 占位待复核，判定暂不展示
...
- `[未定位]` **tomowang-dsh-tui** — 占位待复核，判定暂不展示
- `[未定位]` **toolclub-dsh-agent-team-gui** — 占位待复核，判定暂不展示
- `[未定位]` **tuhuikao-dsh-wlk-video-edit** — 占位待复核，判定暂不展示
- `[未定位]` **turtle1999-turtle-ui** — 占位待复核，判定暂不展示
- `[未定位]` **tzy168-dsh-web-theme-packs** — 占位待复核，判定暂不展示
- `[未定位]` **uluckystar-dsh-client-ui-side-tasks** — 占位待复核，判定暂不展示
- `[未定位]` **unintendedz-dsh-ui-enhancements** — 占位待复核，判定暂不展示
- `[未定位]` **v-quest123456-dsh-smart-input** — 占位待复核，判定暂不展示
...
- `[未定位]` **weibaohui-dsh-plugin-scheduled-items** — 占位待复核，判定暂不展示
- `[未定位]` **weijianguizhou-dsh-task-notify** — 占位待复核，判定暂不展示
- `[未定位]` **wenheguo2-dsh-delegation-suite** — 占位待复核，判定暂不展示
- `[未定位]` **whyihaveyou-dsh-suite** — 占位待复核，判定暂不展示
- `[未定位]` **wj2514939573-ui-dsh-security-gate** — 占位待复核，判定暂不展示
...
- `[未定位]` **xingyingyuzhui-dsh-agent-delegate** — 占位待复核，判定暂不展示
- `[未定位]` **xingyingyuzhui-dsh-agent-gate** — 占位待复核，判定暂不展示
- `[未定位]` **xingyingyuzhui-dsh-agent-identity** — 占位待复核，判定暂不展示
- `[未定位]` **xingyingyuzhui-dsh-agent-memory** — 占位待复核，判定暂不展示
- `[未定位]` **xingyingyuzhui-dsh-agent-registry** — 占位待复核，判定暂不展示
- `[未定位]` **xingyingyuzhui-dsh-session-permissions** — 占位待复核，判定暂不展示
- `[未定位]` **xingyingyuzhui-dsh-vision-bench** — 占位待复核，判定暂不展示
- `[未定位]` **xinspark-dsh-basic-right-sidebar** — 占位待复核，判定暂不展示
- `[未定位]` **xmanrui-dsh-dingtalk** — 占位待复核，判定暂不展示
- `[未定位]` **xmanrui-dsh-feishu** — 占位待复核，判定暂不展示
- `[未定位]` **xmanrui-dsh-weixin** — 占位待复核，判定暂不展示
- `[未定位]` **xmoon-dsh-pi-tui** — 占位待复核，判定暂不展示
- `[未定位]` **xusuyang030218-dsh-skill-ui** — 占位待复核，判定暂不展示
- `[未定位]` **yamibyfajsstsgsjdw-ui-deepseek-harness-tui** — 占位待复核，判定暂不展示
- `[未定位]` **yancf666-dsh-usage-panel** — 占位待复核，判定暂不展示
- `[未定位]` **yeruizhi-dsh-ask-user-timeout** — 占位待复核，判定暂不展示
- `[未定位]` **yjh051108-dsh-routing-suite** — 占位待复核，判定暂不展示
- `[未定位]` **ynnmuraii-dsh-plugin-chat-annotations** — 占位待复核，判定暂不展示
- `[未定位]` **youwenkui-dsh-openclaw-enterprise-team** — 占位待复核，判定暂不展示
- `[未定位]` **youzhoujimrliu-dsh-ui-wallpaper** — 占位待复核，判定暂不展示
- `[未定位]` **yuanqiyibiansheng-deepseek-harness-gui** — 占位待复核，判定暂不展示
- `[未定位]` **yuanzehui313-dsh-workspace-enhance** — 占位待复核，判定暂不展示
- `[未定位]` **yuikizj2026-dsh-slot-homestead** — 占位待复核，判定暂不展示
- `[未定位]` **yuuz12-dsh-webui-auth** — 占位待复核，判定暂不展示
- `[未定位]` **yxsj245-dsh-basics-panel** — 占位待复核，判定暂不展示
- `[未定位]` **yyyyyylor-dsh-asuka-school-theme** — 占位待复核，判定暂不展示
- `[未定位]` **yzke-dsh-icon-theme** — 占位待复核，判定暂不展示
- `[未定位]` **zengrz-dsh-matrix-theme** — 占位待复核，判定暂不展示
- `[未定位]` **zenquiem-dsh-security-suite** — 占位待复核，判定暂不展示
...
- `[未定位]` **zhangweiii-dsh-tui** — 占位待复核，判定暂不展示
- `[未定位]` **zhuifengqug-pixel-skin** — 占位待复核，判定暂不展示
- `[未定位]` **zhuiyueya-dsh-im-gateway** — 占位待复核，判定暂不展示
...
- `[未定位]` **zrui-c-openclaw-plugin-dsh** — 占位待复核，判定暂不展示
- `[未定位]` **ztyhehe-dsh-better-sidebar-svn** — 占位待复核，判定暂不展示
...
- 🟨 `[不兼容]` [a1303845406-dsh-comfy-video-studio](https://github.com/a1303845406/dsh-comfy-video-studio) ★0 — DeepSeek Harness 的 ComfyUI MiniMax H3 视频工作台
...
- 🟨 `[不兼容]` [jonah791-dsh-panel](https://github.com/jonah791/dsh-panel) ★0 — 独立实时前端面板：宿主托管自包含 HTML + HTTP API，零官方 client 依赖
...
- 🟨 `[不兼容]` [loommii-dsh-input-history](https://github.com/loommii/dsh-input-history) ★0 — DeepSeek Harness Web 输入历史插件:↑/↓ 终端式翻查本会话消息,/history 可搜索面板;纯客户端,按会话隔离
...
- 🟨 `[不兼容]` [qing3a-dsh-event-auditor](https://github.com/qing3a/dsh-event-auditor) ★0 — DeepSeek Harness 事件流审计面板插件：观察事件类型/分发模式/计数/最近事件，帮助插件作者理解 harness 内部
...
- 🟨 `[不兼容]` [yeruizhi-dsh-ask-user-timeout](https://github.com/yeruizhi/dsh-ask-user-timeout) ★0 — Guard plugin for DeepSeek Harness: bounded lifetime for ask_user_question — stops the no-tab-renders-question
...
- `[未定位]` **dujunxi1993-dsh-swiftui** — 占位待复核，判定暂不展示
- `[未定位]` **junt184-dsh-ui-background** — 占位待复核，判定暂不展示
- `[未定位]` **max-null-dsh-sidebar-preview-select** — 占位待复核，判定暂不展示
- `[未定位]` **openma-ai-deepseek-harness-tui** — 占位待复核，判定暂不展示
- `[未定位]` **orangearefruit-deepseek-harness-desktop** — 占位待复核，判定暂不展示
- `[未定位]` **rookiecom-dsh-client-ui-side-chat** — 占位待复核，判定暂不展示
- `[未定位]` **stushansusu-dsh-miku-skin** — 占位待复核，判定暂不展示
- `[未定位]` **woodwhite0ets-dshtui-by-woodwhite** — 占位待复核，判定暂不展示
- `[未定位]` **xcodebuild-dsh-plugin-read-image-free** — 占位待复核，判定暂不展示
...
- `[未定位]` **dsh-custom-css** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-remote-web-ui** — 占位待复核，判定暂不展示
- `[未定位]` **ex-setting** — 占位待复核，判定暂不展示 〔PR〕
- `[未定位]` **dsh-claude-theme** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-tui-app** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-live-stats** — 占位待复核，判定暂不展示 〔PR〕
