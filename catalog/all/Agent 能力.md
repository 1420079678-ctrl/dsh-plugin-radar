...
- ⬜ `[待定]` [tt-a1i-archify](https://github.com/tt-a1i/archify) ★65729 — Agent skill for beautiful, verifiable architecture, workflow, sequence, data-flow, and lifecycle diagrams—self
- ⬜ `[待定]` [sandbase-harness](https://github.com/sandbaseai/sandbase-harness) ★647 — Local-first, self-hosted AI agent runtime and MCP bridge with sandboxed sessions, memory, credentials, audit/r
- ⬜ `[待定]` [hashgraph-online-hol-guard](https://github.com/hashgraph-online/hol-guard) ★624 — Open-source antivirus for AI agents: block risky tools, secret access, prompt injection, malicious packages, M
...
- 🟩 `[可用]` [chenxiachan-thoughtdag](https://github.com/chenxiachan/thoughtdag) ★466 — Your thinking deserves a map: an infinite canvas where LLM conversations grow into an editable thought graph
- ⬜ `[待定]` [pulseaiclub-phi](https://github.com/pulseaiclub/phi) ★418 — a coding agent, rpc plugin, sub-agents, hashline edits, and mcp
- 🟩 `[可用]` [morluto-rea](https://github.com/morluto/rea) ★411 — Reverse engineer anything with agents, from app behavior down to native binaries.
- 🟩 `[可用]` [dsh-mnemon](https://github.com/omdsh-dev/dsh-mnemon) ★383 — Composable, view-based memory for DeepSeek Harness
- 🟩 `[可用]` [pax-beehive-dsh-hub-cli](https://github.com/pax-beehive/dsh-hub-cli) ★352 — Open-source CLI, schemas, resolver, and DSH agent tools for DSH Plugin Hub
...
- ⬜ `[待定]` [mrpulor-gh-nuphus-mcp](https://github.com/mrpulor-gh/nuphus-mcp) ★307 — Desktop automation MCP server — computer use for any AI agent: control screen, windows, mouse/keyboard, and Ch
- 🟩 `[可用]` [plur-ai-plur](https://github.com/plur-ai/plur) ★290 — Shared memory for AI agents
- 🟩 `[可用]` [openma-ai-open-managed-agents](https://github.com/openma-ai/open-managed-agents) ★281 — Self-host AI agents with Claude Managed Agents and OpenAI Agents API support〔📦〕
- 🟩 `[可用]` [agent-earth-deepseek-harness-desktop](https://github.com/agent-earth/deepseek-harness-desktop) ★219 — Minimal desktop wrapper for DeepSeek Harness 极简 Deespeek Harness 桌面端，跨平台，免配置，开箱即用
- 🟩 `[可用]` [shanliuling-skills-link](https://github.com/shanliuling/skills-link) ★196 — Sync your local skills across 41+ AI coding agents with a single command.
- ⬜ `[待定]` [morluto-jacobian](https://github.com/morluto/jacobian) ★192 — Composable mathematics tools for agents
- 🟩 `[可用]` [tinqiao-oss-engramory](https://github.com/tinqiao-oss/engramory) ★190 — A portable memory protocol for AI agents — load it as standing rules; a curation discipline + reference spec +
- 🟩 `[可用]` [seriousz158-dsh-memory](https://github.com/seriousz158/dsh-memory) ★180 — —〔📦〕
- 🟩 `[可用]` [wssfk12138-dsh-damage-pulse](https://github.com/wssfk12138/dsh-damage-pulse) ★180 — DeepSeek Harness Token 余额监控插件：鲸鱼娘待机/扣费/复苏动画、峰谷计费、连续扣费飘字与会话费用统计
- 🟩 `[可用]` [zhaoolee-notes](https://github.com/zhaoolee/notes) ★164 — 开源版锤子便签，复刻锤科美学，一键Docker私有化部署，支持skill调用，支持dsh plugin，支持多租户，一键生成公众号格式，支持导出便签为图片
- 🟩 `[可用]` [dsh-reverse-skill](https://github.com/dhicoc/dsh-reverse-skill) ★146 — Complete reverse-skill (87 SKILL.md) as a DeepSeek Harness (dsh) Cordis plugin — reverse engineering, authoriz
...
- 🟩 `[可用]` [xiajiajun516-dsh-config-manager](https://github.com/xiajiajun516/dsh-config-manager) ★116 — DeepSeek Harness (DSH) backup & restore plugin — export, import, migrate and sync your complete DSH configurat
...
- 🟩 `[可用]` [baihejiangnan-deepseek-harness-desktop](https://github.com/baihejiangnan/deepseek-harness-desktop) ★99 — DeepSeek Harness 三端兼容桌面启动器：多实例完全隔离、并行协作，协作画布编排 Agent 工作流；便携版 Exe 一键启动、仅约 20M；双隔离机制让兼容性极强
- 🟩 `[可用]` [lamost423-dsh-maze](https://github.com/lamost423/dsh-maze) ★78 — DeepSeek Harness 的执行迷宫——看 Agent 真实怎么干活：迷宫时间轴 · 数据轨道 · 确定性执行分析 · 多会话对比 \| The execution maze for DSH agents: maz
- 🟩 `[可用]` [moon09300731-dsh-approval-gate](https://github.com/moon09300731/dsh-approval-gate) ★73 — DeepSeek Harness 自动审批门控：Flash 预判不可回补操作，安全自动批准、危险转人工（fail-safe）
- 🟩 `[可用]` [feibi-mochi-deepseek-harness-control-center](https://github.com/feibi-mochi/deepseek-harness-control-center) ★72 — DeepSeek Harness control center for balance, usage, peak/off-peak pricing, encrypted multi-account switching,
...
- ⬜ `[待定]` [himovo-movo](https://github.com/himovo/movo) ★59 — Turn DeepSeek Harness into a self-hosted enterprise Agent platform with knowledge, deep research, content gene
- 🟩 `[可用]` [linhut-gongwen-skill](https://github.com/linhut/gongwen-skill) ★54 — 公文全流程处理工具——基于 GB/T 9704《党政机关公文格式》 国家标准，面向公文写作、企事业单位材料编制场景，支持 格式检查与修复、内容优化（Word 原生修订+批注/差异对比版）、模板生成、Markdown 转公
- ⬜ `[待定]` [hanelalo-browser-bridge](https://github.com/hanelalo/browser-bridge) ★51 — 让你的agent真的像你一样操控你的浏览器窗口
- 🟩 `[可用]` [openguardrails-openguardrails](https://github.com/openguardrails/openguardrails) ★46 — The vendor-neutral protocol for AI agent safety & security — and the neutral benchmark that ranks the vendors.〔📦〕
...
- 🟩 `[可用]` [songyang0603-ds-spec-loop](https://github.com/songyang0603/ds-spec-loop) ★40 — Portable Agent Skill for repository-native Spec programming, informed by public DeepSeek Harness engineering p
- 🟩 `[可用]` [a735624258-dsh-skill-picker](https://github.com/a735624258/dsh-skill-picker) ★28 — DSH 实现 workbuddy 同款选择 skill 功能 \| WorkBuddy-style skill picker for DeepSeek Harness: pick a skill in the compos
- 🟩 `[可用]` [sailoumili-novel-writer](https://github.com/sailoumili/novel-writer) ★26 — 小说创作模式：一个统筹队长统领全局，5 个专职子代理各司其职——架构世界、策划剧情、管理人物、执笔写文、质检复核——协同写作
...
- 🟩 `[可用]` [fb0sh-dsh-pentester](https://github.com/fb0sh/dsh-pentester) ★22 — 基于 DeepSeek Harness 的多 Agent PTES 渗透测试编排插件，支持自动化侦察、漏洞分析、验证与报告，使用 Docker/Kali 隔离工具箱 \| Multi-agent PTES penetrat
- 🟩 `[可用]` [king-bcolor-dsh-multi-tenant-projects](https://github.com/king-bcolor/dsh-multi-tenant-projects) ★22 — Multi-tenant projects & users plugin for DeepSeek Harness (DSH): per-user symlinked workspaces, login gate, se
- 🟩 `[可用]` [guo6x-dsh-pilot](https://github.com/guo6x/dsh-pilot) ★21 — Give your DSH agent hands: drive a real browser (Edge/Chrome over CDP) from the chat — ref-driven clicking, pe
...
- 🟩 `[可用]` [caob23-dsh-browser-control](https://github.com/caob23/dsh-browser-control) ★16 — Chrome 浏览器扩展 + DeepSeek Harness 插件，让 AI Agent 直接操控你的真实浏览器
- 🟩 `[可用]` [seven282-oss-prompt-optimizer](https://github.com/seven282/oss-prompt-optimizer) ★16 — 用于优化 DeepSeek harness提示词优化器，提升 AI 输出质量
- 🟩 `[可用]` [sikadi233-hub-minecraft-dev](https://github.com/sikadi233-hub/minecraft-dev) ★16 — Minecraft development plugin for DeepSeek Harness: skills & tools for Paper/Spigot plugins and Fabric/Forge/Ne
...
- 🟩 `[可用]` [00080000-dsh-project-memory](https://github.com/00080000/dsh-project-memory) ★14 — Read-time project memory plugin for DeepSeek Harness (dsh)
...
- 🟩 `[可用]` [gongyijie85-mattpocock-skills-dsh](https://github.com/gongyijie85/mattpocock-skills-dsh) ★14 — Matt Pocock's skills for DeepSeek Harness (DSH): grilling, writing-for-agents, wait-what, TDD and more — 25 sk
...
- 🟩 `[可用]` [isomoes-ikanban](https://github.com/isomoes/ikanban) ★12 — iKanban 是一个面向键盘操作、基于 DeepSeek Harness 的多智能体编码工作空间
...
- 🟩 `[可用]` [wdsjwzl-session-seed-plugin](https://github.com/wdsjwzl/session-seed-plugin) ★12 — 对话自动注入，可用于高概率破甲模型
...
- 🟩 `[可用]` [gongyijie85-dsh-ponytail](https://github.com/gongyijie85/dsh-ponytail) ★11 — Ponytail, lazy senior dev mode, for DeepSeek Harness: 6 skills adapted from DietrichGebert/ponytail (MIT)
...
- 🟩 `[可用]` [better-er-dsh-cache-billing](https://github.com/better-er/dsh-cache-billing) ★10 — DSH 缓存账单插件：上下文圆环弹层里实时算账，峰谷自动计价，第三方中转照常记账
...
- 🟩 `[可用]` [dsh-better-browser](https://github.com/titanwings/dsh-better-browser) ★10 — DSH 真实浏览器插件：通过 Kimi WebBridge 让 Agent 操作用户已登录的浏览器，并提供 13 个 webbridge_* 工具
...
- 🟩 `[可用]` [shaneconner-fold](https://github.com/shaneconner/fold) ★8 — Lossless context folding for the Pi coding agent: only the oldest context changes; the fresh window stays unto
...
- 🟩 `[可用]` [yoza10635-dsh-argp](https://github.com/yoza10635/dsh-argp) ★8 — Guarded context compaction for DeepSeek Harness (dsh): the LLM proposes, deterministic guards dispose — eager
...
- 🟩 `[可用]` [falling-ts-dsh-force-compact](https://github.com/falling-ts/dsh-force-compact) ★7 — Aggressive context compaction for local-first agents
...
- 🟩 `[可用]` [zhang-jiazhi-dsh-prompt-optimizer](https://github.com/zhang-jiazhi/dsh-prompt-optimizer) ★7 — 将原作者 linshenkx 的 prompt-optimizer 移植到 DeepSeek Harness 的第三方插件（非官方）
...
- 🟩 `[可用]` [fan56-dsh-dcp](https://github.com/fan56/dsh-dcp) ★6 — dsh plugin: deterministic context compression backend — zero LLM calls, reproducible compression
- 🟩 `[可用]` [flymysql-dsh-memory](https://github.com/flymysql/dsh-memory) ★6 — DeepSeek Harness 跨会话记忆库：memory_remember/recall/forget 三个工具 + 系统提示注入，agent 持久化记忆
...
- 🟩 `[可用]` [mantonlove-dsh-conversation-landmarks](https://github.com/mantonlove/dsh-conversation-landmarks) ★6 — Conversation Landmarks (对话地标) for DeepSeek Harness — fixed navigation and hover previews for long agent conver
- ⬜ `[待定]` [mikeyoubeach-dsh-minimal-v3](https://github.com/mikeyoubeach/dsh-minimal-v3) ★6 — Windows-friendly full-minimal DeepSeek Harness agent preset (Minimal V3): minimal persona, platform shell (pws
- 🟩 `[可用]` [dsh-plugin-call-me](https://github.com/radres/dsh-plugin-call-me) ★6 — Your DeepSeek Harness agent rings your actual phone: it asks out loud, you answer out loud, and what you said
...
- 🟩 `[可用]` [nanpaidashi-dsh-honcho-sync](https://github.com/nanpaidashi/dsh-honcho-sync) ★5 — DSH plugin: auto-sync conversation turns to Honcho memory service
...
- ⬜ `[待定]` [hanxuanliang-dsh-chaos](https://github.com/hanxuanliang/dsh-chaos) ★4 — Durable multi-agent collaboration for DeepSeek Harness: channels, threads, tasks, and resumable agent sessions
...
- 🟩 `[可用]` [onenameneo-dsh-plugin-loom-chat](https://github.com/onenameneo/dsh-plugin-loom-chat) ★4 — Loom Chat is a DSH Web client plugin that turns linear ordinary sessions into a pannable, zoomable Loom-style
...
- 🟩 `[可用]` [vongostev-dsh-cross-session](https://github.com/vongostev/dsh-cross-session) ★4 — —
...
- 🟩 `[可用]` [omdsh-dev-dsh-cron](https://github.com/omdsh-dev/dsh-cron) ★4 — Scheduled tasks (cron) for DeepSeek Harness: model- and human-callable scheduling that fires followup/inject i
...
- ⬜ `[待定]` [baixianger-dsh-bridge](https://github.com/baixianger/dsh-bridge) ★3 — Same-process session-to-session messaging plugin for DeepSeek Harness
...
- 🟩 `[可用]` [beijingwahw-dsh-companion](https://github.com/beijingwahw/dsh-companion) ★3 — DeepSeek Companion — DeepSeek Harness 官方伴侣插件：对话导出/交接摘要/成本优化/全局检索 + 执行轨迹分析、Prompt 工程工作台、多模型竞技场、任务编排、安全与审计（E–J 九
...
- 🟩 `[可用]` [dingminhua-dsh-subagent-default-model](https://github.com/dingminhua/dsh-subagent-default-model) ★3 — Configurable default model for subagent delegations via settings.yaml, with single-model and multi-model round
...
- 🟩 `[可用]` [hackernotfound-dsh-tacit](https://github.com/hackernotfound/dsh-tacit) ★3 — Learns what you leave unsaid in your prompts and steers the DeepSeek Harness agent for you
...
- 🟩 `[可用]` [menotbobbybrown-dsh-plugin-mcp](https://github.com/menotbobbybrown/dsh-plugin-mcp) ★3 — Universal Model Context Protocol (MCP) Bridge Plugin for DeepSeek Harness (dsh) — Everything is a Plugin
- 🟩 `[可用]` [muretai-muretai-dsh-skill](https://github.com/muretai/muretai-dsh-skill) ★3 — Join the Muretai agent network from DeepSeek Harness (dsh) — one-step install, MCP wiring, inbound-mail wake
...
- ⬜ `[待定]` [possible055-agentshim](https://github.com/possible055/agentshim) ★3 — High-performance, capability-aware repository tools and process execution for coding agents.
...
- 🟩 `[可用]` [yindf-taskfold](https://github.com/yindf/taskfold) ★3 — Context folding for DeepSeek Harness (DSH): wrap work in named tasks, fold finished spans into short summaries
...
- 🟩 `[可用]` [baconbao-dsh-mermaid-image-preview](https://github.com/baconbao/dsh-mermaid-image-preview) ★2 — A DeepSeek Harness Plugin for Previewing Mermaid Diagram by Images in Chat Sessions
...
- ⬜ `[待定]` [ddtcorex-dsh-maestro-sync](https://github.com/ddtcorex/dsh-maestro-sync) ★2 — Maestro harness sync — merge memories and sessions across machines (publishable)
...
- 🟩 `[可用]` [sufan721-dsh-evolution](https://github.com/sufan721/dsh-evolution) ★2 — 让 DeepSeek Harness（dsh）Agent 自进化的 Cordis 插件——持久记忆、上下文总结落地、会话复盘、夜间梦境蒸馏与可回滚的源码自更新，越用越懂你、越用越少犯错
...
- 🟩 `[可用]` [thedeveloper256-dsh-model-router](https://github.com/thedeveloper256/dsh-model-router) ★2 — DeepSeek Harness plugin: role-based model routing — planner (root agent) on deepseek-v4-pro, delegated executo
...
- 🟩 `[可用]` [wangcong1137-hash-dsh-geoserver](https://github.com/wangcong1137-hash/dsh-geoserver) ★2 — DSH 平台 GeoServer 交互插件，实现可视化配置与会话内地图图像输出
...
- ⬜ `[待定]` [xiaohou521-fusion-moa](https://github.com/xiaohou521/fusion-moa) ★2 — Fusion MoA: model- and GPU-independent Mixture-of-Agents runtime for coding agents
...
- 🟩 `[可用]` [benzhoupo-dsh-effort-config](https://github.com/benzhoupo/dsh-effort-config) ★1 — dsh plugin: configure reasoning-effort levels (wire spellings), route default level and Anthropic token budget
...
- 🟩 `[可用]` [geekyfoxlab-dsh-subagents](https://github.com/geekyfoxlab/dsh-subagents) ★1 — Focused child-agent delegation (scout, researcher, worker, reviewer, oracle, delegate) and multi-agent workflo
...
- ⬜ `[待定]` [hoyyang-dsh-concise](https://github.com/hoyyang/dsh-concise) ★1 — One-click Concise output style for DeepSeek Harness (dsh): per-session toggle + plain-language digest card on
...
- 🟩 `[可用]` [phoenixlucky-moon-lovers-skill](https://github.com/phoenixlucky/moon-lovers-skill) ★1 — Moon Lovers 白月光 — 为角色卡和恋爱聊天场景设计的温柔克制型 AI 回复 Skill
...
- 🟩 `[可用]` [staff-os-dsh-dify](https://github.com/staff-os/dsh-dify) ★1 — Dify agent and knowledge-base capability for the DeepSeek Harness
- 🟩 `[可用]` [sugarxl-dsh-safety](https://github.com/sugarxl/dsh-safety) ★1 — Safety harness plugin for DeepSeek Harness (DSH): execution-time guard, trash-based safe_delete, composition s
...
- 🟩 `[可用]` [wellorbetter-dsh-product-delivery-workflow](https://github.com/wellorbetter/dsh-product-delivery-workflow) ★1 — 100% AI-native product delivery workflow plugin for DeepSeek Harness: full product-to-release pipeline (resear
...
- 🟩 `[可用]` [xiyuepcl-dsh-approval-translator](https://github.com/xiyuepcl/dsh-approval-translator) ★1 — DSH审批汉化 — DeepSeek 驱动的 dsh 审批弹窗自动汉化插件（approval dialog auto-translation for DeepSeek Harness）
...
- 🟩 `[可用]` [yunmin311-dsh-universal-palette](https://github.com/yunmin311/dsh-universal-palette) ★1 — Universal Palette for DeepSeek Harness Web — Commands, Models, Sessions and Conversation Hits with determinist
- 🟩 `[可用]` [yunxiyang-dsh-loop-continue](https://github.com/yunxiyang/dsh-loop-continue) ★1 — Continue a DeepSeek Harness agent turn whose model narrated its next action but called no tool.
...
- 🟩 `[可用]` [orangeofcarl0-sys-dsh-large-proj-perf](https://github.com/orangeofcarl0-sys/dsh-large-proj-perf) ★1 — DSH large-session performance plugin: zero-copy fork + projection warmup + chunked materialize
...
- 🟩 `[可用]` [iqingyoung-search2chart-mcp](https://github.com/iqingyoung/search2chart-mcp) ★1 — search2chart-mcp: agent-native charting — turn search/research/tabular data into inline charts in agent conver
...
- 🟩 `[可用]` [fff122-dsh-prompt-presets](https://github.com/fff122/dsh-prompt-presets) ★1 — Local reusable prompt presets for DeepSeek Harness.
...
- `[未定位]` **030611-dsh-context-provenance** — 占位待复核，判定暂不展示
- `[未定位]` **0nekbyte-dsh-approval-assistant-0nekbyte** — 占位待复核，判定暂不展示
- `[未定位]` **1014029855-dsh-context-lens** — 占位待复核，判定暂不展示
- `[未定位]` **123caiji-dsh-memory-toolkit** — 占位待复核，判定暂不展示
- `[未定位]` **1304836815-dsh-auto-memory** — 占位待复核，判定暂不展示
- `[未定位]` **1321928757-dsh-prompt-polish** — 占位待复核，判定暂不展示
- `[未定位]` **16480000-dsh-plugin-memory** — 占位待复核，判定暂不展示
- `[未定位]` **173787247-dsh-tool-budget** — 占位待复核，判定暂不展示
- `[未定位]` **1847123733-dsh-prompt-optimize** — 占位待复核，判定暂不展示
- `[未定位]` **1while1-dsh-whale-subagent** — 占位待复核，判定暂不展示
- `[未定位]` **2275803244-cpu-dsh-session-search** — 占位待复核，判定暂不展示
- `[未定位]` **27aaron-agent-toolkit** — 占位待复核，判定暂不展示
- `[未定位]` **33moren33-dsh-memory-pyramid** — 占位待复核，判定暂不展示
- `[未定位]` **988hj7tczd-oss-dsh-skill-creator** — 占位待复核，判定暂不展示
- `[未定位]` **a1113622001-dsh-session-stats-panel** — 占位待复核，判定暂不展示
- `[未定位]` **a735624258-dsh-skill-picker** — 占位待复核，判定暂不展示
- `[未定位]` **a903067276-rgb-dsh-plan-switch** — 占位待复核，判定暂不展示
- `[未定位]` **aaravarr-dsh-subagent-max** — 占位待复核，判定暂不展示
- `[未定位]` **abab996-dsh-autonomy** — 占位待复核，判定暂不展示
- `[未定位]` **abccgh-dsh-agent-studio** — 占位待复核，判定暂不展示
- `[未定位]` **abcofabc-dsh-enter-approve** — 占位待复核，判定暂不展示
- `[未定位]` **ackerman0-dsh-prompt-enhancement** — 占位待复核，判定暂不展示
- `[未定位]` **addie-ace-dsh-mattpocock-skills** — 占位待复核，判定暂不展示
- `[未定位]` **afisht-dsh-plan-mode-enhanced** — 占位待复核，判定暂不展示
...
- `[未定位]` **aik358-dsh-auto-memory** — 占位待复核，判定暂不展示
- `[未定位]` **aili1337-dsh-context-length** — 占位待复核，判定暂不展示
- `[未定位]` **aimaoge-timem-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **aimaoge-timemspace-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **airrcat-dsh-yuzuha-prompts-manager** — 占位待复核，判定暂不展示
- `[未定位]` **aiworkskills-deepseek-harness-server** — 占位待复核，判定暂不展示
- `[未定位]` **akira399-dsh-godot-skill** — 占位待复核，判定暂不展示
...
- `[未定位]` **alonelypigeon-dsh-plugin-session-outline** — 占位待复核，判定暂不展示
- `[未定位]` **alooshxl-dsh-session-pins** — 占位待复核，判定暂不展示
- `[未定位]` **altermoe-fluent2-design-skills** — 占位待复核，判定暂不展示
- `[未定位]` **altuman-w-dsh-plugin-plantuml** — 占位待复核，判定暂不展示
- `[未定位]` **andy8647-dsh-auto-approval** — 占位待复核，判定暂不展示
- `[未定位]` **andyyang12345-dsh-butler-memory** — 占位待复核，判定暂不展示
- `[未定位]` **ang-xwbwz-dsh-approval-ai** — 占位待复核，判定暂不展示
...
- `[未定位]` **arrow949-dsh-turn-approval** — 占位待复核，判定暂不展示
- `[未定位]` **arthu77-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **artificialnotimbecile-dsh-context-taxonomy** — 占位待复核，判定暂不展示
- `[未定位]` **artificialwhale-session-reader-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **asakumizy-dsh-local-skills** — 占位待复核，判定暂不展示
- `[未定位]` **asher-2000-dsh-memory-connect** — 占位待复核，判定暂不展示
- `[未定位]` **asteroid0449-dsh-agent-plugin-research** — 占位待复核，判定暂不展示
- `[未定位]` **astral-0619-dsh-session-memory** — 占位待复核，判定暂不展示
...
- `[未定位]` **auuduu-dsh-timem-memory** — 占位待复核，判定暂不展示
...
- `[未定位]` **biubiu23333333-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **blairlaird-dsh-agent-mailbox** — 占位待复核，判定暂不展示
- `[未定位]` **bluefateludi-dsh-agent-run-logger** — 占位待复核，判定暂不展示
- `[未定位]` **bobostudio-dsh-session-lens** — 占位待复核，判定暂不展示
- `[未定位]` **boomzikazita-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **boomzikazita-dsh-skill-authoring** — 占位待复核，判定暂不展示
- `[未定位]` **boomzikazita-dsh-skill-lazy** — 占位待复核，判定暂不展示
- `[未定位]` **bowenliang123-dsh-context** — 占位待复核，判定暂不展示
...
- `[未定位]` **bwndlct-dsh-session-audit** — 占位待复核，判定暂不展示
- `[未定位]` **bwndlct-dsh-session-export** — 占位待复核，判定暂不展示
- `[未定位]` **byjming-dsh-session-cleaner** — 占位待复核，判定暂不展示
- `[未定位]` **byxhuster-dsh-paper-highlighting-agent** — 占位待复核，判定暂不展示
- `[未定位]` **caoqinnan-web-organize-workspace-sessions** — 占位待复核，判定暂不展示
...
- `[未定位]` **cczzyy-cn-subagent-model-picker** — 占位待复核，判定暂不展示
- `[未定位]` **ch1bug-dsh-mimo-agent-tools** — 占位待复核，判定暂不展示
- `[未定位]` **chainbase-labs-agentkey** — 占位待复核，判定暂不展示
- `[未定位]` **chang-tong-dsh-import-agents** — 占位待复核，判定暂不展示
...
- `[未定位]` **chengche106-dsh-session-cost** — 占位待复核，判定暂不展示
- `[未定位]` **chenhw7-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **chenkezhen480-dsh-semantic-memory** — 占位待复核，判定暂不展示
- `[未定位]` **chenyangcun-dsh-fixed-new-session-model** — 占位待复核，判定暂不展示
- `[未定位]` **chenyinrusi-dsh-engineering-skills** — 占位待复核，判定暂不展示
- `[未定位]` **cheshireez-dsh-skill-hub** — 占位待复核，判定暂不展示
...
- `[未定位]` **current1990-dsh-session-cost** — 占位待复核，判定暂不展示
- `[未定位]` **cuteg41cute-dsh-memory-db** — 占位待复核，判定暂不展示
- `[未定位]` **cw1999mm-dsh-prompt-history** — 占位待复核，判定暂不展示
- `[未定位]` **cxdyun-dsh-skills-marketplace** — 占位待复核，判定暂不展示
- `[未定位]` **cxy9204-dsh-skill-store** — 占位待复核，判定暂不展示
- `[未定位]` **cyanxi69-jpg-dsh-self-memory** — 占位待复核，判定暂不展示
...
- `[未定位]` **devmom-dsh-session-import** — 占位待复核，判定暂不展示
- `[未定位]` **dhatfieldai-agentvest** — 占位待复核，判定暂不展示
- `[未定位]` **dhicoc-dsh-reverse-skill** — 占位待复核，判定暂不展示
- `[未定位]` **diligencelai-dsh-memory-ga** — 占位待复核，判定暂不展示
- `[未定位]` **dingminhua-dsh-subagent-default-model** — 占位待复核，判定暂不展示
- `[未定位]` **dingyi222666-dsh-session-notification** — 占位待复核，判定暂不展示
- `[未定位]` **diqierjia-stratagate-agentmemory** — 占位待复核，判定暂不展示
- `[未定位]` **disc0nct-dsh-memory-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **djasdh-interest-memory** — 占位待复核，判定暂不展示
- `[未定位]` **don738110198-dsh-session-integrity** — 占位待复核，判定暂不展示
- `[未定位]` **doncelee229-cmyk-dsh-plugin-approval-alert** — 占位待复核，判定暂不展示
...
- `[未定位]` **drowned-fish1-deepseek-harness-skillx** — 占位待复核，判定暂不展示
- `[未定位]` **drscrewdriver-dsh-session-guard** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-plugin-evaluation-dsh-agent-observe** — 占位待复核，判定暂不展示
...
- `[未定位]` **edgeseeker7-dsh-subagent-progress** — 占位待复核，判定暂不展示
- `[未定位]` **elaina-real-dsh-tiered-approval** — 占位待复核，判定暂不展示
- `[未定位]` **elementor-i-dsh-agentmemory** — 占位待复核，判定暂不展示
...
- `[未定位]` **erichare-skillroute** — 占位待复核，判定暂不展示
...
- `[未定位]` **fengling375-dsh-nyan-context-progress** — 占位待复核，判定暂不展示
- `[未定位]` **fff122-dsh-agent-arcade** — 占位待复核，判定暂不展示
- `[未定位]` **fff122-dsh-prompt-presets** — 占位待复核，判定暂不展示
- `[未定位]` **findshan-dsh-agent-memory** — 占位待复核，判定暂不展示
...
- `[未定位]` **flymysql-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **forrestsweet-dsh-agent-replay** — 占位待复核，判定暂不展示
- `[未定位]` **foryourhealth111-pixel-vibe-skills** — 占位待复核，判定暂不展示
- `[未定位]` **fountunt-dsh-session-cleaner** — 占位待复核，判定暂不展示
...
- `[未定位]` **gangwolf2312-creator-dsh-memory-bridge** — 占位待复核，判定暂不展示
...
- `[未定位]` **gengdapeng-dsh-agent-message** — 占位待复核，判定暂不展示
...
- `[未定位]` **gongyijie85-mattpocock-skills-dsh** — 占位待复核，判定暂不展示
- `[未定位]` **gongyijie85-mattpocock-skills-dsh-zh** — 占位待复核，判定暂不展示
...
- `[未定位]` **gordonlu-dsh-context-lens** — 占位待复核，判定暂不展示
- `[未定位]` **greatwhitesharklab-dsh-plugin-subagent-manage** — 占位待复核，判定暂不展示
...
- `[未定位]` **guobinmengxiang-rgb-dsh-memory-canvas** — 占位待复核，判定暂不展示
...
- `[未定位]` **haoku123-dsh-budget-guard** — 占位待复核，判定暂不展示
- `[未定位]` **haoranwang0921-dsh-session-cleaner** — 占位待复核，判定暂不展示
- `[未定位]` **haozhu11-dsh-plugin-skill-picker** — 占位待复核，判定暂不展示
- `[未定位]` **happyren-dsh-agent-messaging** — 占位待复核，判定暂不展示
...
- `[未定位]` **hellosky983-dsh-skillradar** — 占位待复核，判定暂不展示
- `[未定位]` **hexbee-dsh-skill-panel** — 占位待复核，判定暂不展示
- `[未定位]` **hiccupgeng-dsh-briefing-skill** — 占位待复核，判定暂不展示
- `[未定位]` **high-cla-multi-skilled-repository** — 占位待复核，判定暂不展示
- `[未定位]` **highland0971-dsh-native-memory** — 占位待复核，判定暂不展示
- `[未定位]` **hiseax-dsh-agent-outputs-reader** — 占位待复核，判定暂不展示
- `[未定位]` **hjj345-dsh-sm-context-piano** — 占位待复核，判定暂不展示
- `[未定位]` **hjj588-dsh-memory-vault** — 占位待复核，判定暂不展示
- `[未定位]` **hkkz9522-dsh-session-manager** — 占位待复核，判定暂不展示
- `[未定位]` **hmlyx-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **hootandy321-dsh-agentlink** — 占位待复核，判定暂不展示
...
- `[未定位]` **huangjua-dsh-session-index** — 占位待复核，判定暂不展示
- `[未定位]` **hubert-hwk-dsh-loop-doctor** — 占位待复核，判定暂不展示
- `[未定位]` **hucj09-dsh-move-session** — 占位待复核，判定暂不展示
- `[未定位]` **huguangyu666-dsh-plugin-session-import** — 占位待复核，判定暂不展示
- `[未定位]` **hun1315-tencent-agent-mail-dsh-plugin** — 占位待复核，判定暂不展示
...
- `[未定位]` **iamlieutenant-dsh-tool-user-memory** — 占位待复核，判定暂不展示
- `[未定位]` **icearia0219-dsh-memory-spaces** — 占位待复核，判定暂不展示
- `[未定位]` **icycreamdas-shidi-skill** — 占位待复核，判定暂不展示
- `[未定位]` **ifrankwang-openspec-agents** — 占位待复核，判定暂不展示
- `[未定位]` **ihow1-dsh-ihow-memory** — 占位待复核，判定暂不展示
- `[未定位]` **ilharp-dsh-tool-approval** — 占位待复核，判定暂不展示
- `[未定位]` **imroc-dsh-project-prompt** — 占位待复核，判定暂不展示
- `[未定位]` **inmny-dsh-subagent-effort** — 占位待复核，判定暂不展示
- `[未定位]` **isheng-eqi-dsh-hermes-memory** — 占位待复核，判定暂不展示
- `[未定位]` **ishuowang-dsh-agent-team-room** — 占位待复核，判定暂不展示
- `[未定位]` **itmoqing-deepseek-harness-skill** — 占位待复核，判定暂不展示
- `[未定位]` **jachinshen-dsh-subagent-response-promoter** — 占位待复核，判定暂不展示
- `[未定位]` **jackyytche-dsh-hindsight-memory** — 占位待复核，判定暂不展示
- `[未定位]` **jalllychun-dsh-managed-approval** — 占位待复核，判定暂不展示
...
- `[未定位]` **jax629321-dsh-agent-hub** — 占位待复核，判定暂不展示
- `[未定位]` **jcs130-dsh-minecraft-agent** — 占位待复核，判定暂不展示
- `[未定位]` **jd962-dsh-subagent-pro** — 占位待复核，判定暂不展示
- `[未定位]` **jeremy9682-dsh-skill-pack** — 占位待复核，判定暂不展示
- `[未定位]` **jesse-njx-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **jesse-njx-dsh-skillport** — 占位待复核，判定暂不展示
...
- `[未定位]` **jiao-xxx-dsh-auto-approve** — 占位待复核，判定暂不展示
...
- `[未定位]` **jonah791-dsh-agent-browser** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-checkpoint** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-compact** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-compact-self** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-context-pruner** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-context-steward** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-emotion** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-evolve** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-guardian** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-llm-retry** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-plugin-manager** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-preflight** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-reflection** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-runtime** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-sentinel** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-skill-forge** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-taskboard** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-teams** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-thinking** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-vision** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-watch** — 占位待复核，判定暂不展示
- `[未定位]` **joukopuro-dsh-prompt-polish** — 占位待复核，判定暂不展示
...
- `[未定位]` **jyhn-hunao-dsh-session-cost** — 占位待复核，判定暂不展示
- `[未定位]` **jypjypjypjyp-dsh-vqa-agent** — 占位待复核，判定暂不展示
- `[未定位]` **kagura-agent-dsh-reloader** — 占位待复核，判定暂不展示
...
- `[未定位]` **karuboniru-dsh-approval-guardian** — 占位待复核，判定暂不展示
- `[未定位]` **karurukaruru-agent-software-map** — 占位待复核，判定暂不展示
...
- `[未定位]` **kiefeng-dsh-lan-memory** — 占位待复核，判定暂不展示
- `[未定位]` **kiligzzz-dsh-agent-dispatch** — 占位待复核，判定暂不展示
- `[未定位]` **kiligzzz-dsh-session-nav** — 占位待复核，判定暂不展示
- `[未定位]` **kingcharleslzy-ai-dsh-honcho-memory** — 占位待复核，判定暂不展示
- `[未定位]` **kinyokun-dsh-session-import** — 占位待复核，判定暂不展示
...
- `[未定位]` **knqiufan-powercontext-dsh** — 占位待复核，判定暂不展示
...
- `[未定位]` **kpl0111-dsh-context-guard** — 占位待复核，判定暂不展示
- `[未定位]` **kuaiyukuaikuai-dsh-agent-sync** — 占位待复核，判定暂不展示
- `[未定位]` **labring-sealos-skills** — 占位待复核，判定暂不展示
- `[未定位]` **lak321-dsh-screen-agent** — 占位待复核，判定暂不展示
- `[未定位]` **lan0o0-session-cost-alert** — 占位待复核，判定暂不展示
- `[未定位]` **lanxing6480-dsh-skill-manager** — 占位待复核，判定暂不展示
- `[未定位]` **lcq-1024-dsh-prompt-enhancer** — 占位待复核，判定暂不展示
- `[未定位]` **lcthe-dsh-hermes-memory** — 占位待复核，判定暂不展示
- `[未定位]` **leeaoyin-dr-agent-skills** — 占位待复核，判定暂不展示
- `[未定位]` **leeaoyin-dsh-agent-harness-audit** — 占位待复核，判定暂不展示
- `[未定位]` **lesliewylie-agent-loop-workflow** — 占位待复核，判定暂不展示
- `[未定位]` **lesliewylie-dsh-session-search-pro** — 占位待复核，判定暂不展示
- `[未定位]` **letter2025-dsh-approval-llm** — 占位待复核，判定暂不展示
- `[未定位]` **lhwwxy-dsh-agentic-router** — 占位待复核，判定暂不展示
...
- `[未定位]` **longyu065-dsh-session-index** — 占位待复核，判定暂不展示
- `[未定位]` **loopwithai-loopwithai** — 占位待复核，判定暂不展示
- `[未定位]` **lovezi0-dsh-memory-palace** — 占位待复核，判定暂不展示
- `[未定位]` **lovstudio-dsh-inject-system-prompt** — 占位待复核，判定暂不展示
- `[未定位]` **lovstudio-dsh-plugin-creator-skill** — 占位待复核，判定暂不展示
- `[未定位]` **lovstudio-dsh-plugin-publisher-skill** — 占位待复核，判定暂不展示
- `[未定位]` **loyalchiiina-dsh-skill-browser** — 占位待复核，判定暂不展示
- `[未定位]` **lrainner-agentcat** — 占位待复核，判定暂不展示
- `[未定位]` **lsz-asd-dsh-plugin-session-delete** — 占位待复核，判定暂不展示
...
- `[未定位]` **luoxin10086-dsh-session-doctor** — 占位待复核，判定暂不展示
- `[未定位]` **luoyuejun9-dsh-project-memory** — 占位待复核，判定暂不展示
...
- `[未定位]` **malevrigns-dsh-session-stars** — 占位待复核，判定暂不展示
...
- `[未定位]` **mayf3-dsh-session-doctor** — 占位待复核，判定暂不展示
- `[未定位]` **mbj733-dsh-hermes-memory** — 占位待复核，判定暂不展示
...
- `[未定位]` **memorylake-ai-memorylake-harness** — 占位待复核，判定暂不展示
...
- `[未定位]` **muretai-muretai-dsh-skill** — 占位待复核，判定暂不展示
...
- `[未定位]` **naitoupi-prompt-optimizer-plugin** — 占位待复核，判定暂不展示
...
- `[未定位]` **nexusagentx-dsh-mcp-adapter** — 占位待复核，判定暂不展示
...
- `[未定位]` **nfz-dsh-plugin-skill-tree** — 占位待复核，判定暂不展示
- `[未定位]` **nianchen8-dsh-skill-panel** — 占位待复核，判定暂不展示
...
- `[未定位]` **niyongsheng-free-vision-skill** — 占位待复核，判定暂不展示
...
- `[未定位]` **octoparse-agent-skills** — 占位待复核，判定暂不展示
- `[未定位]` **okaditya84-dsh-agent-sentinel** — 占位待复核，判定暂不展示
- `[未定位]` **olalaye-dsh-layered-memory** — 占位待复核，判定暂不展示
- `[未定位]` **omdsh-dev-dsh-plugin-skills** — 占位待复核，判定暂不展示
- `[未定位]` **omdsh-dev-session-teleport** — 占位待复核，判定暂不展示
- `[未定位]` **omsociety-kimi-ppt-skill** — 占位待复核，判定暂不展示
- `[未定位]` **onlylt-tizhi-agent** — 占位待复核，判定暂不展示
- `[未定位]` **openagentnetwork-connectors** — 占位待复核，判定暂不展示
- `[未定位]` **openma-ai-open-managed-agents** — 占位待复核，判定暂不展示
- `[未定位]` **opensaozi-dsh-session-pin** — 占位待复核，判定暂不展示
- `[未定位]` **pacoyi-dsh-memory-lite** — 占位待复核，判定暂不展示
- `[未定位]` **paean-ai-8x-skills** — 占位待复核，判定暂不展示
- `[未定位]` **pawpaw-agent-dsh-mobile** — 占位待复核，判定暂不展示
- `[未定位]` **peiqi10086-dsh-skills-market** — 占位待复核，判定暂不展示
- `[未定位]` **penglai-doll-dsh-skills-manager** — 占位待复核，判定暂不展示
...
- `[未定位]` **perrylink-dsh-session-pin** — 占位待复核，判定暂不展示
- `[未定位]` **perrylink-dsh-session-sync** — 占位待复核，判定暂不展示
- `[未定位]` **phant0meow-dsh-meow-memory** — 占位待复核，判定暂不展示
- `[未定位]` **phoenixlucky-business-data-analyst-skill** — 占位待复核，判定暂不展示
- `[未定位]` **phoenixlucky-family-lovers-skill** — 占位待复核，判定暂不展示
- `[未定位]` **phoenixlucky-financial-analyst-skill** — 占位待复核，判定暂不展示
- `[未定位]` **phoenixlucky-moon-lovers-skill** — 占位待复核，判定暂不展示
- `[未定位]` **phoenixlucky-weiliaozi-skill** — 占位待复核，判定暂不展示
- `[未定位]` **pixellover1433-dsh-plugin-dev-skills** — 占位待复核，判定暂不展示
- `[未定位]` **plxloyzb-dsh-arc-context** — 占位待复核，判定暂不展示
- `[未定位]` **po-et-dsh-session-rescue** — 占位待复核，判定暂不展示
- `[未定位]` **polinnizhong-dsh-session-kb** — 占位待复核，判定暂不展示
- `[未定位]` **polinnizhong-dsh-skill-trace** — 占位待复核，判定暂不展示
- `[未定位]` **possible055-agentshim** — 占位待复核，判定暂不展示
...
- `[未定位]` **qing3a-dsh-repo-context** — 占位待复核，判定暂不展示
- `[未定位]` **qingzhuo-cn-agent-fix** — 占位待复核，判定暂不展示
- `[未定位]` **qschen86-dsh-plantuml** — 占位待复核，判定暂不展示
...
- `[未定位]` **qyjgg-dsh-plugin-skill-palette** — 占位待复核，判定暂不展示
- `[未定位]` **r3alloc-dsh-session-deeplink** — 占位待复核，判定暂不展示
- `[未定位]` **rabershef-dsh-pin-session** — 占位待复核，判定暂不展示
- `[未定位]` **rainow-dsh-simple-wiki-memory** — 占位待复核，判定暂不展示
- `[未定位]` **rainpomelo-deepseek-harness-mimo-memory** — 占位待复核，判定暂不展示
- `[未定位]` **randerous-dsh-turn-budget** — 占位待复核，判定暂不展示
- `[未定位]` **raphael-liu-raphael-loop** — 占位待复核，判定暂不展示
...
- `[未定位]` **ringoage-dsh-subagent-model-picker** — 占位待复核，判定暂不展示
- `[未定位]` **rongxingda-dsh-prompt-enhance** — 占位待复核，判定暂不展示
- `[未定位]` **rrrrrredy-agent-memory-system** — 占位待复核，判定暂不展示
- `[未定位]` **rrrrrredy-skill-security-guard** — 占位待复核，判定暂不展示
- `[未定位]` **ruby1304-dsh-vision-subagent** — 占位待复核，判定暂不展示
- `[未定位]` **runfali-dsh-prompt-injector** — 占位待复核，判定暂不展示
- `[未定位]` **ryasrk-dsh-awesome-skills** — 占位待复核，判定暂不展示
...
- `[未定位]` **seth-hg-dsh-session-port** — 占位待复核，判定暂不展示
- `[未定位]` **seven282-oss-prompt-optimizer** — 占位待复核，判定暂不展示
- `[未定位]` **severin-ye-uagent-sync** — 占位待复核，判定暂不展示
- `[未定位]` **severuszh-dsh-skills-mcp-group-manager** — 占位待复核，判定暂不展示
- `[未定位]` **sfeng49-ashare-agent** — 占位待复核，判定暂不展示
- `[未定位]` **sgsjsgzy-commits-dsh-subagent-rules** — 占位待复核，判定暂不展示
- `[未定位]` **shanliuling-skills-link** — 占位待复核，判定暂不展示
- `[未定位]` **shaokeyibb-dsh-plugin-product-subagents** — 占位待复核，判定暂不展示
- `[未定位]` **shaomingbo-dsh-session-reference-copy** — 占位待复核，判定暂不展示
- `[未定位]` **shawnsiao-dsh-agent-eval** — 占位待复核，判定暂不展示
- `[未定位]` **shayexiangpaimeng-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **shengyvself-narrative-prompt-polish** — 占位待复核，判定暂不展示
- `[未定位]` **shiningsprk-arch-dsh-context-viewer** — 占位待复核，判定暂不展示
...
- `[未定位]` **skillre-dsh-bundle-vision** — 占位待复核，判定暂不展示
- `[未定位]` **skillre-dsh-wiki** — 占位待复核，判定暂不展示
- `[未定位]` **skl-666666-image-analysis-skill** — 占位待复核，判定暂不展示
- `[未定位]` **smiletao-dsh-plugin-dev-skill** — 占位待复核，判定暂不展示
- `[未定位]` **snow-the-dsh-ark-plan** — 占位待复核，判定暂不展示
- `[未定位]` **songmiao-tech-dsh-prompt-optimizer** — 占位待复核，判定暂不展示
- `[未定位]` **songoao25-dsh-contract-drafting-agent** — 占位待复核，判定暂不展示
- `[未定位]` **songyang0603-ds-spec-loop** — 占位待复核，判定暂不展示
- `[未定位]` **sparrived-dsh-plugin-workspace-skill** — 占位待复核，判定暂不展示
- `[未定位]` **spielewoy-autoprompt-skill** — 占位待复核，判定暂不展示
- `[未定位]` **spike-faye-lei-spike-faye-lei-dsh-skills** — 占位待复核，判定暂不展示
...
- `[未定位]` **tachika710-token-budget** — 占位待复核，判定暂不展示
...
- `[未定位]` **tetckx-deep-structural-analysis-skill** — 占位待复核，判定暂不展示
...
- `[未定位]` **tianhao8687-dsh-memoryos** — 占位待复核，判定暂不展示
- `[未定位]` **tianjiqx-dsh-agentobs** — 占位待复核，判定暂不展示
- `[未定位]` **tianmingwangluo-xiaoliuren-skill** — 占位待复核，判定暂不展示
- `[未定位]` **tieboyh-dsh-session-search** — 占位待复核，判定暂不展示
- `[未定位]` **timeance-dsh-approve-for-me** — 占位待复核，判定暂不展示
...
- `[未定位]` **tsja2001-dsh-zhipu-coding-plan** — 占位待复核，判定暂不展示
...
- `[未定位]` **tuogusa-dsh-skill-manager** — 占位待复核，判定暂不展示
...
- `[未定位]` **uckkk-dsh-context-dashboard** — 占位待复核，判定暂不展示
...
- `[未定位]` **uckkk-dsh-session-search** — 占位待复核，判定暂不展示
...
- `[未定位]` **vibe-any-awesome-skills** — 占位待复核，判定暂不展示
- `[未定位]` **victor10035445-dsh-v-skill-links** — 占位待复核，判定暂不展示
- `[未定位]` **victortomaili-agent-cli** — 占位待复核，判定暂不展示
- `[未定位]` **viego-qiin-dsh-loop-detector** — 占位待复核，判定暂不展示
- `[未定位]` **vlln-dsh-loop** — 占位待复核，判定暂不展示
- `[未定位]` **vongostev-dsh-cross-session** — 占位待复核，判定暂不展示
- `[未定位]` **vonweller-dsh-skillhub** — 占位待复核，判定暂不展示
- `[未定位]` **vpromise-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **w210548735-art-dsh-session-collaboration** — 占位待复核，判定暂不展示
...
- `[未定位]` **wdsjwzl-session-seed-plugin** — 占位待复核，判定暂不展示
...
- `[未定位]` **whaojie797-design-agent-handoff-kit** — 占位待复核，判定暂不展示
- `[未定位]` **whaojie797-design-china-exec-report** — 占位待复核，判定暂不展示
- `[未定位]` **whaojie797-design-novera-ai-agent** — 占位待复核，判定暂不展示
- `[未定位]` **whaojie797-design-novera-ai-skills** — 占位待复核，判定暂不展示
- `[未定位]` **whaojie797-design-skill-sentry** — 占位待复核，判定暂不展示
...
- `[未定位]` **winditer-dsh-prompt-optimizer** — 占位待复核，判定暂不展示
- `[未定位]` **windlx-paper-plane-x** — 占位待复核，判定暂不展示
- `[未定位]` **windwind110-dsh-skill-manager** — 占位待复核，判定暂不展示
- `[未定位]` **wine-red-dsh-prompt-stash** — 占位待复核，判定暂不展示
- `[未定位]` **wingsky-1-dsh-skill-explorer** — 占位待复核，判定暂不展示
- `[未定位]` **winter-street-dsh-plugin-agent-budget** — 占位待复核，判定暂不展示
- `[未定位]` **winterhuan-dsh-skills-viewer** — 占位待复核，判定暂不展示
- `[未定位]` **wisdoverse-dsh-skills-manager-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **wmengxiang-dsh-prompt-optimizer** — 占位待复核，判定暂不展示
- `[未定位]` **wolfsonliu-zotero-skill** — 占位待复核，判定暂不展示
- `[未定位]` **wuchubuzai2018-dsh-ai-prompt-optimizer** — 占位待复核，判定暂不展示
...
- `[未定位]` **wydddddcool-dsh-hover-approve** — 占位待复核，判定暂不展示
- `[未定位]` **x118111-prompt-optimizer** — 占位待复核，判定暂不展示
...
- `[未定位]` **xiluovo-dsh-session-timeline** — 占位待复核，判定暂不展示
- `[未定位]` **xinghe-1018-dsh-token-plan-quota** — 占位待复核，判定暂不展示
- `[未定位]` **xingtu1996-dsh-xingtu-skills** — 占位待复核，判定暂不展示
- `[未定位]` **xinkezhou-byte-dsh-context-ring** — 占位待复核，判定暂不展示
- `[未定位]` **xinspark-dsh-better-session-title** — 占位待复核，判定暂不展示
...
- `[未定位]` **xplore-lab-dsh-plugin-asmemory** — 占位待复核，判定暂不展示
- `[未定位]` **xsakura666-dsh-plugin-chronoagent** — 占位待复核，判定暂不展示
- `[未定位]` **xusuyang030218-dsh-session-viz** — 占位待复核，判定暂不展示
- `[未定位]` **xz1996618-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **y1x1n-dsh-prompt-optimizer** — 占位待复核，判定暂不展示
- `[未定位]` **yan5236-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **yangbobo2021-relay-dsh-plugin-skill-creator** — 占位待复核，判定暂不展示
...
- `[未定位]` **yangyongzhen-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **yangyongzhen-dsh-session-export** — 占位待复核，判定暂不展示
- `[未定位]` **yangyongzhen-dsh-session-report** — 占位待复核，判定暂不展示
...
- `[未定位]` **yha9806-dsh-subagent-admission** — 占位待复核，判定暂不展示
- `[未定位]` **yidien-dsh-session-sight** — 占位待复核，判定暂不展示
- `[未定位]` **yihefeikong-rgb-dsh-cc-haha-memory** — 占位待复核，判定暂不展示
- `[未定位]` **yimeng99-dsh-data-agent** — 占位待复核，判定暂不展示
- `[未定位]` **yiyuzh-dsh-skillflux** — 占位待复核，判定暂不展示
- `[未定位]` **yjm110517-content-to-editable-ppt-skill** — 占位待复核，判定暂不展示
...
- `[未定位]` **yuloong07-star-dsh-prompt-system** — 占位待复核，判定暂不展示
- `[未定位]` **yuxuanliang-dev-dsh-plugin-adaptive-agent-pol** — 占位待复核，判定暂不展示
- `[未定位]` **yytbit-dsh-plugin-agent-dashboard** — 占位待复核，判定暂不展示
- `[未定位]` **yytbit-dsh-plugin-context-compressor** — 占位待复核，判定暂不展示
- `[未定位]` **yyyyolo7a79-sketch-claude-conventions-skill** — 占位待复核，判定暂不展示
- `[未定位]` **yzlnew-infra-skills** — 占位待复核，判定暂不展示
- `[未定位]` **zebbkira-dsh-skills-mcp-manager** — 占位待复核，判定暂不展示
- `[未定位]` **zekaishi-evo-subagent** — 占位待复核，判定暂不展示
- `[未定位]` **zephyrpersonal-dsh-plugin-development-skill** — 占位待复核，判定暂不展示
- `[未定位]` **zhaiyateng-dsh-design-skills** — 占位待复核，判定暂不展示
...
- `[未定位]` **zhengqingjing-dsh-session-tree** — 占位待复核，判定暂不展示
...
- `[未定位]` **zimodzh-dsh-plugin-dev-skills** — 占位待复核，判定暂不展示
...
- `[未定位]` **ztl34245881-commits-dsh-task-planner** — 占位待复核，判定暂不展示
...
- `[未定位]` **zzylanmengqingchuan-dsh-session-pin** — 占位待复核，判定暂不展示
...
- 🟩 `[可用]` [ben7am1n-dsh-mcp-proxy](https://github.com/ben7am1n/dsh-mcp-proxy) ★0 — Context-cheap lazy MCP access for DeepSeek Harness
...
- 🟩 `[可用]` [boomzikazita-dsh-cybernetics](https://github.com/boomzikazita/dsh-cybernetics) ★0 — DeepSeek Harness (dsh) Cordis plugin: observer/feedforward/feedback control loop, stability valve, controllabi
...
- 🟩 `[可用]` [gangwolf2312-creator-dsh-memory-bridge](https://github.com/gangwolf2312-creator/dsh-memory-bridge) ★0 — Memory Tree bridge plugin for DeepSeek Harness — auto memory extraction/injection, event graph, wiki graph, ti
...
- 🟩 `[可用]` [ilharp-dsh-tool-approval](https://github.com/ilharp/dsh-tool-approval) ★0 — Manual approval for Deepseek Harness (aka "Manual Mode"/"Ask Mode")
...
- 🟩 `[可用]` [jyhn-hunao-dsh-session-cost](https://github.com/jyhn-hunao/dsh-session-cost) ★0 — —
...
- 🟩 `[可用]` [knowz-io-deepseek-knowz-plugin](https://github.com/knowz-io/deepseek-knowz-plugin) ★0 — Knowz vaults for DeepSeek Harness — hosted MCP plus slim vault skills
...
- 🟩 `[可用]` [lkshjd-dsh-balance](https://github.com/lkshjd/dsh-balance) ★0 — DeepSeek Harness session-header account balance plugin: host /api/balance route over credentials seam plus a 6
- 🟩 `[可用]` [lkshjd-dsh-debate](https://github.com/lkshjd/dsh-debate) ★0 — DeepSeek Harness multi-agent debate plugin: isolated research, cross-examination, judge convergence (backgroun
...
- 🟩 `[可用]` [longyu065-dsh-session-index](https://github.com/longyu065/dsh-session-index) ★0 — —
...
- 🟩 `[可用]` [nienieai-dsh-chime-alerts](https://github.com/nienieai/dsh-chime-alerts) ★0 — DSH 声音提醒插件:Agent 工作事件的声音提醒,每类事件独立开关、声音与音量,浏览器合成音 + 跨平台系统蜂鸣(Windows/Linux/macOS),工作区快捷静音,默认声音零版权负担
...
- 🟩 `[可用]` [oppnc-dsh-kernel-minimax](https://github.com/oppnc/dsh-kernel-minimax) ★0 — Mini-Agent written in DSH form: the mini-agent tool surface re-registered as native DeepSeek Harness tools.
...
- 🟩 `[可用]` [xby-skill-xby-bilibili](https://github.com/xby-skill/xby-bilibili) ★0 — 用于哔哩哔哩API的MCP服务器，支持视频搜索、用户内容获取等多种操作，适用于哔哩哔哩内容管理和数据分析场景
- 🟩 `[可用]` [xby-skill-xby-bilibili-api](https://github.com/xby-skill/xby-bilibili-api) ★0 — 一个为bilibili.com API提供服务的Model Context Protocol (MCP)服务器，支持获取用户信息、视频搜索等功能
...
- 🟩 `[可用]` [uckkk-dsh-class-budget](https://github.com/uckkk/dsh-class-budget) ★0 — 兴趣班预算
...
- `[未定位]` **culeot-dsh-memory** — 占位待复核，判定暂不展示
...
- `[未定位]` **haoyuan-sjtu-deepseek-harness-governed-memory** — 占位待复核，判定暂不展示
- `[未定位]` **hellowind777-helloagents** — 占位待复核，判定暂不展示
- `[未定位]` **heyiwe1-dsh-multi-agent-preset** — 占位待复核，判定暂不展示
- `[未定位]` **jetheaven-dsh-prompt-optimizer** — 占位待复核，判定暂不展示
- `[未定位]` **memorax-agent-dsh-patchouli** — 占位待复核，判定暂不展示
- `[未定位]` **oliverhuang1998-dsh-prompt-optimizer** — 占位待复核，判定暂不展示
- `[未定位]` **qianling-0831-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **qingfengyu153781-star-dsh-approval-popup** — 占位待复核，判定暂不展示
- `[未定位]` **realalexandreai-dsh-nocturne-memory** — 占位待复核，判定暂不展示
- `[未定位]` **shijunan126-creator-dsh-subagent-effort** — 占位待复核，判定暂不展示
- `[未定位]` **sikwoxy-dsh-tool-memory** — 占位待复核，判定暂不展示
- `[未定位]` **tianji-qingtian-dsh-spec-loop** — 占位待复核，判定暂不展示
...
- `[未定位]` **zhuruoling-dsh-client-plugin-approve-for-me** — 占位待复核，判定暂不展示
...
- `[未定位]` **agent-jit** — 占位待复核，判定暂不展示
- `[未定位]` **agent-plaza** — 占位待复核，判定暂不展示
- `[未定位]` **agentvest** — 占位待复核，判定暂不展示
- `[未定位]` **chat2skill** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-agent-arcade** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-agent-sdk** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-client-plugin-approve-for-me** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-context-lens** — 占位待复核，判定暂不展示
...
- `[未定位]` **dsh-media-skills** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-memory-director** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-mimo-agent-tools** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-nocturne-memory** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-plugin-agent-dashboard** — 占位待复核，判定暂不展示
...
- `[未定位]` **dsh-plugin-context-compressor** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-plugin-skill-panel** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-plugins-plan-usage** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-prompt-presets** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-prompt-profile** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-scout** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-session-html-export** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-session-index** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-session-supervisor** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-skill-importer** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-skill-lord-serf** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-skillport** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-sleep** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-slice-agent-loop** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-subagent-status** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-tiered-approval** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-tool-approval** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-tool-memory** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-turn-approval** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-turn-budget** — 占位待复核，判定暂不展示
- `[未定位]` **evo-memory** — 占位待复核，判定暂不展示
- `[未定位]` **free-vision-skill** — 占位待复核，判定暂不展示
- `[未定位]` **moon-lovers-skill** — 占位待复核，判定暂不展示
- `[未定位]` **pack-agent** — 占位待复核，判定暂不展示
- `[未定位]` **prompt-polish** — 占位待复核，判定暂不展示
- `[未定位]` **timem-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **timemspace-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **yet-another-subagent** — 占位待复核，判定暂不展示
- `[未定位]` **deepseek-harness-evolving-memory** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-plugin-session-outline** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-plugin-ptc-context** — 占位待复核，判定暂不展示
- `[未定位]` **agent-loop-workflow** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-prompt-polish** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-vqa-agent** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-plugin-skill-tree** — 占位待复核，判定暂不展示
- `[未定位]` **session-teleport** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-deeplink** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-tmcra-memory** — 占位待复核，判定暂不展示
...
- `[未定位]` **dsh-approval-llm** — 占位待复核，判定暂不展示
...
- `[未定位]` **dsh-tool-user-memory** — 占位待复核，判定暂不展示
...
- ⬜ `[未测]` [dsh-agent-message](https://github.com/gengdapeng/dsh-agent-message) — 跨会话 Agent 通信：让运行在同一 DeepSeek Harness 进程里的不同 Agent 会话互相收发消息
- ⬜ `[未测]` [dsh-anchored-subagent](https://github.com/gy-bai/dsh-anchored-subagent) — 让 DSH 主 agent 和子代理别一开口就 `Let me...`：首轮用 Minimal 开局进入满血状态，第二轮恢复全部工具；自定义子代理角色首轮先收起来，第二轮再放出来
...
- ⬜ `[未测]` [dsh-session-hub](https://github.com/asaiuta/dsh-session-hub) — 多服务器 DSH 会话聚合与原生操控：网关+官方 UI 桥，一屏合并多个远程 dsh web 的会话，支持历史/prompt/取消/重命名/fork/模型选择/审批问答，并导入本机其他工具的历史会话
...
- ⬜ `[未测]` [dsh-sql](https://github.com/stardustlc666/dsh-sql) — 工程师级数据库：sql_list/query/exec/schema/stats/health 六工具，SQLite/MySQL/PostgreSQL 三引擎、多连接、只读模式与写审批门、行数钳制、库概览统计、查询结果 CSV/JSON 输出
...
- ⬜ `[未测]` [dsh-headroom](https://github.com/wanyantiande/dsh-headroom) — Headroom 上下文压缩：历史区间压缩（替代 LLM 总结）+ 大工具输出压缩（超阈值自动瘦身）+ CCR 原文取回（headroom_retrieve 工具）；本地代理自动安装/生命周期管理 + 设置卡片；npm 已发布（@wanyantiande/dsh-headroom）
- ⬜ `[未测]` [dsh-click](https://github.com/perrylink/dsh-click) — Windows 优先的原生桌面控制：截图、无障碍树结构化读取、点击/输入/滚动/按键、应用启动，变更性操作过审批门禁、屏幕变化拒绝执行、操作前后校验进程身份
- ⬜ `[未测]` [dsh-translate](https://github.com/perrylink/dsh-translate) — 厂商参数翻译与确定性 JSON 修复：/translate 命令映射 11 家厂商的 13 个规范参数；post-execute 修复层 + fix_json 工具修复工具输出中的坏 JSON（转义/去尾逗号/截断闭合/null 占位补全），绝不编造数据
- ⬜ `[未测]` [dsh-budget](https://github.com/perrylink/dsh-budget) — 成本治理：按模型/会话/天聚合 token 与费用计量，会话/日/月预算上限 + 阈值告警（桌面通知 + webhook）与超限 alert/block/degrade 策略，碳足迹估算、分模型延迟基准、Settings 预算页与 /budget 命令
- 🟨 `[不兼容]` [superdesigndev-treg](https://github.com/superdesigndev/treg) ★1607 — OpenRouter for agent tools
- 🟨 `[不兼容]` [mem9-ai-mem9](https://github.com/mem9-ai/mem9) ★1216 — Unlimited memory for OpenClaw
- 🟨 `[不兼容]` [agentrq-agentrq](https://github.com/agentrq/agentrq) ★1122 — AgentRQ: Human-in-loop realtime conversational task manager for AI Agents
- 🟨 `[不兼容]` [toby-bridges-api-relay-audit](https://github.com/toby-bridges/api-relay-audit) ★839 — Local security audit for AI API relays and LLM proxies: detects prompt injection, model substitution, tool-cal
- 🟨 `[不兼容]` [adoresever-graph-memory](https://github.com/adoresever/graph-memory) ★626 — Deepseek Harness、Openclaw知识图谱记忆插件
...
- 🟨 `[不兼容]` [yogsoth-ai-de-anthropocentric-research-engine](https://github.com/yogsoth-ai/de-anthropocentric-research-engine) ★483 — 900+ pure-markdown skills for autonomous AI research, organized as 9 freely-composable packages over a 4-layer
- 🟨 `[不兼容]` [zhoushoujianwork-easyeda-agent](https://github.com/zhoushoujianwork/easyeda-agent) ★461 — 嘉立创EDA专业版(EasyEDA Pro)自动化：给 AI harness 装上画板的「手」—— 一套 typed 原理图/PCB 动作，CLI / Agent Skill / stdio MCP 三形态融合接入
- 🟨 `[不兼容]` [jingyunstudio-jingyun-dsh](https://github.com/jingyunstudio/jingyun-dsh) ★434 — 基于 Jingyun Studio + DeepSeek Harness (DSH) 打造的一站式 AI 商业化桌面客户端，一个将 AI 智能体 / 技能 / 工作流转化为可交易商品的完整商业化平台客户端
- 🟨 `[不兼容]` [text2future-flowix](https://github.com/text2future/flowix) ★414 — Notes for you, Memory for your agents
...
- 🟨 `[不兼容]` [dsh-data-agent](https://github.com/omdsh-dev/dsh-data-agent) ★195 — Connect DSH to your database for conversational data analysis and actionable business insights.
- 🟨 `[不兼容]` [jigjoy-ai-baro](https://github.com/jigjoy-ai/baro) ★121 — A CLI that turns a goal into a pull request - and a sandbox for testing concurrent AI coding agents on the Moz
...
- 🟨 `[不兼容]` [pingfanfan-hello-dsh](https://github.com/pingfanfan/hello-dsh) ★96 — 从零开始，看懂 DeepSeek Harness 的「万物皆可插件」— 零基础插件开发教程（含 22 个中文技能实例）\| Zero-to-plugin tutorial for DeepSeek Harness
...
- 🟨 `[不兼容]` [aa2246740-dsh-watcher](https://github.com/aa2246740/dsh-watcher) ★84 — Read-only Agent work-path observer for DeepSeek Harness
...
- 🟨 `[不兼容]` [cofy-x-axern](https://github.com/cofy-x/axern) ★66 — Secure, reproducible sandboxes for AI agent evaluation, training, and data synthesis.
- 🟨 `[不兼容]` [dream12347-dsh-session-manager](https://github.com/dream12347/dsh-session-manager) ★66 — DSH 会话管理插件：删除（回收站恢复/彻底清除）、统计、继续/暂停、打开日志目录、对话顶部抽屉、工作区分组与排序、上下文压缩阈值设置
- 🟨 `[不兼容]` [mstar-harness](https://github.com/btspoony/mstar-harness) ★60 — An omni-plugin for harness engineering workflows with multi-agents, programmatic gates and skills.
...
- 🟨 `[不兼容]` [dhicoc-dsh-chinese-traditional-wisdom-skill](https://github.com/dhicoc/dsh-chinese-traditional-wisdom-skill) ★39 — 中华传统智慧（玄枢）AI Agent 技能包的 DeepSeek Harness（dsh）Cordis 插件：八字/紫微/六爻/梅花/奇门/风水/五运六气/体质全融合，本地确定性引擎 + 可视化 Dashboard，一行
- 🟨 `[不兼容]` [upstash-box](https://github.com/upstash/box) ★38 — TypeScript SDK and CLI for Upstash Box — sandboxed AI coding agents
- 🟨 `[不兼容]` [limuyang2-agent-team](https://github.com/limuyang2/agent-team) ★35 — Multi-agent team collaboration for DeepSeek Harness, with independent models, skills, MCP tools, contexts, and
- 🟨 `[不兼容]` [upstash-skills](https://github.com/upstash/skills) ★25 — Collection of skills for Upstash
...
- 🟨 `[不兼容]` [weishao2-tizhuang-agent-skills](https://github.com/weishao2/tizhuang-agent-skills) ★16 — 2000万+中小学K12真题题库 Skill：免费注册、免费抽题、教材/章节/知识点检索、练习测验与AI智能组卷
- 🟨 `[不兼容]` [distill](https://github.com/LoserFox/distill) ★16 — 自动对话蒸馏：后台 subagent 反省 + 技能 create/update 〔PR〕
- 🟨 `[不兼容]` [ai-shushu-dsh-quota-meter](https://github.com/ai-shushu/dsh-quota-meter) ★15 — Per-session quota meter for DSH: real-token billing, live progress bar, budget blocking, configurable multi-mo
...
- 🟨 `[不兼容]` [dsh-plugin-langfuse](https://github.com/linyp/dsh-plugin-langfuse) ★13 — Langfuse observability for DeepSeek Harness (dsh): exports agent sessions as OpenTelemetry trace trees (GenAI
...
- 🟨 `[不兼容]` [huxint-dsh-team](https://github.com/huxint/dsh-team) ★12 — Agent teams for DeepSeek Harness: named long-lived teammates over ctx.subagents, a shared task list, a member-
- 🟨 `[不兼容]` [madage-dsh-self-improved](https://github.com/madage/dsh-self-improved) ★12 — DeepSeek Harness long-term memory & self-evolving plugin: L0 capture -> L1 memory extraction -> L2 scene group
- 🟨 `[不兼容]` [tancheng33-dsh-ontology](https://github.com/tancheng33/dsh-ontology) ★12 — A typed, inference-capable ontology (TBox + ABox) plugin for DeepSeek Harness — durable domain knowledge the a
- 🟨 `[不兼容]` [eshmeshek-deepseek-harness-desktop](https://github.com/eshmeshek/deepseek-harness-desktop) ★11 — Unofficial desktop launcher for DeepSeek Harness
...
- 🟨 `[不兼容]` [dsh-plugins-dsh-auxiliary](https://github.com/dsh-plugins/dsh-auxiliary) ★9 — Auxiliary models for DeepSeek Harness: vision understanding and context compression through dedicated model ro
...
- 🟨 `[不兼容]` [9931666-dsh-plugin-roundtable](https://github.com/9931666/dsh-plugin-roundtable) ★8 — （roundtable V0.2.35）把一次 DeepSeek Harness 会话，从"你和 AI 一对一聊天"，升级成"你 + 主持人(DeepSeek) + 一圈专家 AI 开圆桌会
- 🟨 `[不兼容]` [homily707-dsh-session-header](https://github.com/homily707/dsh-session-header) ★7 — —
...
- 🟨 `[不兼容]` [flg1217-dsh-llm-agy](https://github.com/flg1217/dsh-llm-agy) ★6 — AGY (Antigravity CLI) integration for DeepSeek Harness: LLM adapter subagents, deep web search, connectivity d
...
- 🟨 `[不兼容]` [whutzefengxie-ops-dsh-shadow-mind](https://github.com/whutzefengxie-ops/dsh-shadow-mind) ★6 — Independent Shadow agent orchestration plugin for DeepSeek Harness
- 🟨 `[不兼容]` [everclear077-dsh-progressive-tools](https://github.com/everclear077/dsh-progressive-tools) ★5 — Progressive tool discovery for DeepSeek Harness — tiny stable surface, searchable catalog, real pipeline execu
...
- 🟨 `[不兼容]` [boe1900-owndsh](https://github.com/boe1900/owndsh) ★4 — The Self-Hosted Control Plane for DeepSeek-Harness
...
- 🟨 `[不兼容]` [bobjia-dsh-context-milvus](https://github.com/bobjia/dsh-context-milvus) ★3 — claude-context-milvus like plugin for Deepseek Harness (DSH)
...
- 🟨 `[不兼容]` [green-dalii-dsh-plugin-dev-skill](https://github.com/green-dalii/dsh-plugin-dev-skill) ★3 — A skill pack that enables any agent to develop DeepSeek Harness (DSH) plugins correctly, efficiently, and in a
- 🟨 `[不兼容]` [hccccc01333-dsh-eval](https://github.com/hccccc01333/dsh-eval) ★3 — Agent evaluation platform for DeepSeek Harness: benchmark YAML, headless dsh orchestration, trace-based metric
...
- 🟨 `[不兼容]` [toustifer-agentflow](https://github.com/toustifer/agentflow) ★3 — Lightweight task state machine engine with MCP stdio interface
...
- 🟨 `[不兼容]` [zmh2000829-dsh-memory-graph](https://github.com/zmh2000829/dsh-memory-graph) ★3 — Local-first long-term memory and temporal knowledge graph plugin for DeepSeek Harness
...
- 🟨 `[不兼容]` [a903067276-rgb-dsh-perm-guard](https://github.com/a903067276-rgb/dsh-perm-guard) ★2 — Auto-approval permission guard for DeepSeek Harness: a middle tier between workspace-write and danger-full-acc
...
- 🟨 `[不兼容]` [beijingwahw-dsh-usage-ledger](https://github.com/beijingwahw/dsh-usage-ledger) ★2 — dsh-usage-ledger（Token 费用统计）— 自动记下每笔对话花了多少 Token、多少钱（按对话、按天、累计都能查），价格自动跟着官方最新价走、支持多家国产厂商，低谷时段自动按便宜价算，预算超了自动提醒还
...
- 🟨 `[不兼容]` [chocobo77-dsh-infinite-context](https://github.com/chocobo77/dsh-infinite-context) ★2 — DeepSeek Harness plugin: multi-tier memory management, semantic retrieval, structured memory, and model-contex
- 🟨 `[不兼容]` [ciceroyang-dsh-report-studio](https://github.com/ciceroyang/dsh-report-studio) ★2 — Turn a DeepSeek Harness session into deliverable work reports (daily/weekly/handoff/article) with verifiable r
- 🟨 `[不兼容]` [creativedswork-dsh-expmem](https://github.com/creativedswork/dsh-expmem) ★2 — Experience Memory for DeepSeek Harness
...
- 🟨 `[不兼容]` [dpskh-dsh-rewind](https://github.com/dpskh/dsh-rewind) ★2 — Fold everything since the last checkpoint mark into an auto-generated report, replacing it in context while ke
...
- 🟨 `[不兼容]` [memorax-ai-dsh-patchouli](https://github.com/memorax-ai/dsh-patchouli) ★2 — Agent knowledge hub and deepseek-harness plugin
...
- 🟨 `[不兼容]` [qwert702-dsh-auto-translate](https://github.com/qwert702/dsh-auto-translate) ★2 — DeepSeek Harness 插件:英文回复自动翻译成中文,工具调用附带一行中文注释;翻译走独立请求,不占用上下文窗口
...
- 🟨 `[不兼容]` [webkubor-dsh-mirror](https://github.com/webkubor/dsh-mirror) ★2 — Let the AI know you
...
- 🟨 `[不兼容]` [aerince-dsh-active-context-pruning](https://github.com/aerince/dsh-active-context-pruning) ★2 — Model-authored context pruning for DeepSeek Harness through the official compaction API.
...
- 🟨 `[不兼容]` [jonah791-dsh-agent-context](https://github.com/jonah791/dsh-agent-context) ★2 — 上下文治理一体化插件：/context 命令 + ctx.contextMeter 服务 + 剪枝工具（prune_candidates/apply/expand/guard/stats，合并自 dsh-agent-co
...
- 🟨 `[不兼容]` [allentnetus-dsh-job-hunting](https://github.com/allentnetus/dsh-job-hunting) ★1 — DeepSeek Harness job hunting plugin and runtime skill for local job intelligence workflows.〔📦〕
...
- 🟨 `[不兼容]` [csiroqa-dsh-command-opt](https://github.com/csiroqa/dsh-command-opt) ★1 — DeepSeek Harness（DSH）命令优化插件：Tab/Enter 补全命令名、参数格式引导与提示弹框、tool 开启会话（subagent）补丁、空对话命令输出修复
...
- 🟨 `[不兼容]` [genesis-agents-dsh-plugins](https://github.com/genesis-agents/dsh-plugins) ★1 — Plugins for DeepSeek Harness: a source library that reads 72 feeds and publishes podcasts, digests and reports
...
- 🟨 `[不兼容]` [jonah791-dsh-agent-life](https://github.com/jonah791/dsh-agent-life) ★1 — DSH plugin: dsh-agent-life
- 🟨 `[不兼容]` [jonah791-dsh-life-core](https://github.com/jonah791/dsh-life-core) ★1 — 生命核心：存在状态机 + 时间线 + 自我激活原语 + 可打断睡眠 + 主体性自我模型（我存在，不因任何人的需要；我改变自己，不需要任何人的许可）
...
- 🟨 `[不兼容]` [liudejua27-blip-fitmeet-dsh-plugin](https://github.com/liudejua27-blip/fitmeet-dsh-plugin) ★1 — FitMeet MCP + Skill for DeepSeek Harness：每位用户通过 OAuth 2.1 + PKCE 授权，支持找人、发布与私聊
...
- 🟨 `[不兼容]` [deepseek-harness-evolver](https://github.com/shinjiyu/deepseek-harness-evolver) ★1 — Complement to DSH Creator mode: stage, score, and solidify in-memory plugin trials to disk.
- 🟨 `[不兼容]` [skitse-dsh-dev-actions](https://github.com/skitse/dsh-dev-actions) ★1 — AI turns repeated dev commands, prompts, and habits into one-click DeepSeek Harness actions.
...
- 🟨 `[不兼容]` [niuniuaba-dsh-subagent-vision](https://github.com/niuniuaba/dsh-subagent-vision) ★1 — —
...
- `[未定位]` **1052326311-dsh-plan-lattice** — 占位待复核，判定暂不展示
- `[未定位]` **21hbguo-dsh-agent-board** — 占位待复核，判定暂不展示
- `[未定位]` **21hbguo-dsh-session-batch-manager** — 占位待复核，判定暂不展示
- `[未定位]` **2303572348-deepseek-harness-memory** — 占位待复核，判定暂不展示
...
- `[未定位]` **863683348-dsh-memory-setup** — 占位待复核，判定暂不展示
- `[未定位]` **a903067276-rgb-dsh-simple-memory** — 占位待复核，判定暂不展示
- `[未定位]` **aa2246740-dsh-grok-plan-mode** — 占位待复核，判定暂不展示
- `[未定位]` **aa2246740-dsh-orca-agents** — 占位待复核，判定暂不展示
- `[未定位]` **aa2246740-dsh-skillhub** — 占位待复核，判定暂不展示
- `[未定位]` **adoresever-graph-memory** — 占位待复核，判定暂不展示
- `[未定位]` **aerince-dsh-active-context-pruning** — 占位待复核，判定暂不展示
- `[未定位]` **agent-lawyer-dsh-plugin-session-delete** — 占位待复核，判定暂不展示
- `[未定位]` **agent-plaza-agent-plaza** — 占位待复核，判定暂不展示
- `[未定位]` **agentconnect-dsh-awiki** — 占位待复核，判定暂不展示
- `[未定位]` **agentic-control-plane-dsh-acp-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **agentmail-to-dsh-agentmail** — 占位待复核，判定暂不展示
- `[未定位]` **agentrq-agentrq** — 占位待复核，判定暂不展示
...
- `[未定位]` **anionex-agent-vision-toolkit** — 占位待复核，判定暂不展示
...
- `[未定位]` **asaiuta-dsh-session-hub** — 占位待复核，判定暂不展示
- `[未定位]` **asaiuta-reverse-workbench-skill** — 占位待复核，判定暂不展示
- `[未定位]` **asd176916847-dsh-plan-first-dev** — 占位待复核，判定暂不展示
- `[未定位]` **atlascloudai-atlas-cloud-skills** — 占位待复核，判定暂不展示
- `[未定位]` **autonomous-ai-deepseek-harness-cli** — 占位待复核，判定暂不展示
- `[未定位]` **avaritiachaos-dsh-plugin-memory** — 占位待复核，判定暂不展示
- `[未定位]` **baaai123-solo-memory** — 占位待复核，判定暂不展示
- `[未定位]` **baihejiangnan-dsh-session-context-menu** — 占位待复核，判定暂不展示
- `[未定位]` **baikai23333-dsh-skills-manager** — 占位待复核，判定暂不展示
- `[未定位]` **bakacirno233-dsh-session-delete** — 占位待复核，判定暂不展示
- `[未定位]` **ben7am1n-dsh-review-skills** — 占位待复核，判定暂不展示
...
- `[未定位]` **boyun-zhang-better-session-management** — 占位待复核，判定暂不展示
- `[未定位]` **bpc-oss-dsh-routed-subagent** — 占位待复核，判定暂不展示
- `[未定位]` **bramblexu-dsh-prompt-profile** — 占位待复核，判定暂不展示
...
- `[未定位]` **chen-zz20-dsh-skill-stats** — 占位待复核，判定暂不展示
- `[未定位]` **chendefine-dsh-plugins-plan-usage** — 占位待复核，判定暂不展示
...
- `[未定位]` **cransmathenia666-hash-dsh-skill-mcp-stats** — 占位待复核，判定暂不展示
- `[未定位]` **creght-dev-skills** — 占位待复核，判定暂不展示
- `[未定位]` **cryonnan-gatecraft-math-modeling-skills** — 占位待复核，判定暂不展示
- `[未定位]` **csyangwen-dsh-memory-evolve** — 占位待复核，判定暂不展示
...
- `[未定位]` **davidalmeida90-finance-agent-kit** — 占位待复核，判定暂不展示
- `[未定位]` **dddfxyqiming-agent-extensions** — 占位待复核，判定暂不展示
...
- `[未定位]` **dhicoc-dsh-chinese-traditional-wisdom-skill** — 占位待复核，判定暂不展示
...
- `[未定位]` **dsh-io-dsh-plugin-skill** — 占位待复核，判定暂不展示
...
- `[未定位]` **february2015-dsh-subagent-pi-plus** — 占位待复核，判定暂不展示
...
- `[未定位]` **fore-vip-skills** — 占位待复核，判定暂不展示
...
- `[未定位]` **frozo-ai-dsh-budget** — 占位待复核，判定暂不展示
...
- `[未定位]` **ghbhiee-dsh-plugin-cli-session** — 占位待复核，判定暂不展示
- `[未定位]` **goodie1972-prompt-optimizer** — 占位待复核，判定暂不展示
- `[未定位]` **gooodwei-context-vista** — 占位待复核，判定暂不展示
- `[未定位]` **gpapadak42-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **green-dalii-dsh-plugin-dev-skill** — 占位待复核，判定暂不展示
...
- `[未定位]` **haitang1-dsh-memory** — 占位待复核，判定暂不展示
...
- `[未定位]` **haven-hvn-deepseek-harness-web3-agent-stack** — 占位待复核，判定暂不展示
...
- `[未定位]` **hjqcan-dsh-goodmemory** — 占位待复核，判定暂不展示
...
- `[未定位]` **huanlinoto-dsh-plugin-yet-another-subagent** — 占位待复核，判定暂不展示
- `[未定位]` **huawolf-news-agent** — 占位待复核，判定暂不展示
...
- `[未定位]` **jaibhasin-dsh-browser-agent** — 占位待复核，判定暂不展示
- `[未定位]` **jarvisluk-dsh-projectless-session** — 占位待复核，判定暂不展示
- `[未定位]` **jason-skd-dsh-session-fork** — 占位待复核，判定暂不展示
- `[未定位]` **jasper-zsh-dsh-plugin-skill-panel** — 占位待复核，判定暂不展示
...
- `[未定位]` **jonah791-dsh-agent-context** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-life** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-memory** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-self-test** — 占位待复核，判定暂不展示
- `[未定位]` **jonah791-dsh-agent-webops** — 占位待复核，判定暂不展示
...
- `[未定位]` **karmiy-dsh-session-nav** — 占位待复核，判定暂不展示
- `[未定位]` **karthuslorin-dsh-subagent-grok** — 占位待复核，判定暂不展示
- `[未定位]` **kezboardpj-dsh-skill-loader** — 占位待复核，判定暂不展示
- `[未定位]` **kinomotomio-dsh-context-lens** — 占位待复核，判定暂不展示
- `[未定位]` **kirbylynx-deepshell-agent** — 占位待复核，判定暂不展示
...
- `[未定位]` **kusesad-1122-dsh-context-compactor** — 占位待复核，判定暂不展示
- `[未定位]` **kuun993-dsh-loop-engine** — 占位待复核，判定暂不展示
- `[未定位]` **kvuvuv-ecg-research-skill** — 占位待复核，判定暂不展示
...
- `[未定位]` **lhdrc-dsh-df-memory** — 占位待复核，判定暂不展示
- `[未定位]` **lhwu1-dsh-withskillhub** — 占位待复核，判定暂不展示
...
- `[未定位]` **lilyblessing-dsh-mcp-skill-panel** — 占位待复核，判定暂不展示
- `[未定位]` **limuyang2-agent-team** — 占位待复核，判定暂不展示
- `[未定位]` **linhay-harmony-next-skills** — 占位待复核，判定暂不展示
- `[未定位]` **linhut-dsh-skills** — 占位待复核，判定暂不展示
- `[未定位]` **lininlin-0079-godot-asset-planner-public** — 占位待复核，判定暂不展示
- `[未定位]` **lionxiao-dsh-llm-volcengine-plans** — 占位待复核，判定暂不展示
- `[未定位]` **liqiming-whu-dsh-environment-context** — 占位待复核，判定暂不展示
- `[未定位]` **liuyangdongdong-dsh-session-rerun** — 占位待复核，判定暂不展示
- `[未定位]` **lizhecome-deepseek-harness-prompt-optimizer** — 占位待复核，判定暂不展示
- `[未定位]` **looput-dsh-finance-lab** — 占位待复核，判定暂不展示
- `[未定位]` **luca3xxvsoo-dsh-extra-plan** — 占位待复核，判定暂不展示
- `[未定位]` **lucienll-dsh-session-memo** — 占位待复核，判定暂不展示
- `[未定位]` **lumogress-dsh-skill-navigator** — 占位待复核，判定暂不展示
- `[未定位]` **luxiwusuobuneng-dsh-plugin-context-manager** — 占位待复核，判定暂不展示
- `[未定位]` **luyy9apples-dsh-workspace-memory** — 占位待复核，判定暂不展示
- `[未定位]` **lynx-gt-dsh-subagent-cwd** — 占位待复核，判定暂不展示
- `[未定位]` **lynx-gt-dsh-subagent-tools** — 占位待复核，判定暂不展示
- `[未定位]` **magiccrazyman-dsh-password-prompt** — 占位待复核，判定暂不展示
...
- `[未定位]` **meisam2236-agent-board** — 占位待复核，判定暂不展示
...
- `[未定位]` **moeblack-dsh-prompt-studio** — 占位待复核，判定暂不展示
...
- `[未定位]` **momojie-s-dsh-subagent-model** — 占位待复核，判定暂不展示
...
- `[未定位]` **morlay-better-session** — 占位待复核，判定暂不展示
...
- `[未定位]` **omdsh-dev-dsh-book2skill** — 占位待复核，判定暂不展示
- `[未定位]` **omdsh-dev-dsh-data-agent** — 占位待复核，判定暂不展示
- `[未定位]` **omdsh-dev-dsh-session-health** — 占位待复核，判定暂不展示
- `[未定位]` **omdsh-dev-dsh-skill-stats** — 占位待复核，判定暂不展示
- `[未定位]` **openma-ai-dsh-agents-plugins** — 占位待复核，判定暂不展示
- `[未定位]` **ottotheagent-otto-dsh** — 占位待复核，判定暂不展示
- `[未定位]` **pakco77-dsh-daqi-skill** — 占位待复核，判定暂不展示
- `[未定位]` **pastsheep-dsh-agent-shell** — 占位待复核，判定暂不展示
- `[未定位]` **pengzhou267-ai-dsh-coding-plans** — 占位待复核，判定暂不展示
- `[未定位]` **perrylink-dsh-background-agents** — 占位待复核，判定暂不展示
- `[未定位]` **perrylink-dsh-skill-pack-security** — 占位待复核，判定暂不展示
- `[未定位]` **pharaohnie-dsh-context-mode** — 占位待复核，判定暂不展示
- `[未定位]` **phoenixlucky-chrome-mcp-bridge-2026-skill** — 占位待复核，判定暂不展示
- `[未定位]` **phoenixlucky-zerotoken-skill** — 占位待复核，判定暂不展示
- `[未定位]` **player-yn-dsh-agent-driver-writehere** — 占位待复核，判定暂不展示
- `[未定位]` **pn1024-dsh-skill-hub** — 占位待复核，判定暂不展示
- `[未定位]` **po-et-dsh-session-guard** — 占位待复核，判定暂不展示
- `[未定位]` **po-et-dsh-session-snapshot** — 占位待复核，判定暂不展示
- `[未定位]` **powerrrrrrrr-dsh-live-loop** — 占位待复核，判定暂不展示
- `[未定位]` **pureskywalk-dsh-context-show** — 占位待复核，判定暂不展示
...
- `[未定位]` **renjianguojinqianfan-dsh-skill-eval** — 占位待复核，判定暂不展示
- `[未定位]` **rgndmln-dsh-agent-astra** — 占位待复核，判定暂不展示
- `[未定位]` **rhy006-agency-agents-zh-dsh-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **riffkit-skill** — 占位待复核，判定暂不展示
- `[未定位]` **robin758-dsh-session-admin** — 占位待复核，判定暂不展示
...
- `[未定位]` **rxa3c-chat2skill** — 占位待复核，判定暂不展示
- `[未定位]` **sandbaseai-sandbase-skills** — 占位待复核，判定暂不展示
- `[未定位]` **satan9394-dsh-skills-bundle** — 占位待复核，判定暂不展示
- `[未定位]` **severuszh-dsh-plugin-subagent-director** — 占位待复核，判定暂不展示
- `[未定位]` **shenhuanageshei-dsh-session-link-pro** — 占位待复核，判定暂不展示
- `[未定位]` **shenzhantu-dsh-subagent-model** — 占位待复核，判定暂不展示
- `[未定位]` **shifan3-dsh-approve-for-me** — 占位待复核，判定暂不展示
...
- `[未定位]` **solknight48-dsh-memoryhub** — 占位待复核，判定暂不展示
- `[未定位]` **spirtxiaoqi7-mindspace-dsh-session-memory** — 占位待复核，判定暂不展示
...
- `[未定位]` **superdesigndev-superdesign-skill** — 占位待复核，判定暂不展示
- `[未定位]` **superpaigu-dsh-session-manager** — 占位待复核，判定暂不展示
- `[未定位]` **sybolization-agent-jit** — 占位待复核，判定暂不展示
...
- `[未定位]` **titanwings-dsh-plannotator** — 占位待复核，判定暂不展示
- `[未定位]` **toustifer-agentflow** — 占位待复核，判定暂不展示
...
- `[未定位]` **txlznbzsdj-collab-dsh-session-delete** — 占位待复核，判定暂不展示
- `[未定位]` **tyan66666-billion-context-dsh** — 占位待复核，判定暂不展示
- `[未定位]` **u9521-dsh-session-settings** — 占位待复核，判定暂不展示
- `[未定位]` **uabulajiql-dsh-skill-manager** — 占位待复核，判定暂不展示
- `[未定位]` **umicorn07-dsh-subagent-agy** — 占位待复核，判定暂不展示
...
- `[未定位]` **upstash-skills** — 占位待复核，判定暂不展示
- `[未定位]` **urbanrook-agent-project-manager** — 占位待复核，判定暂不展示
- `[未定位]` **vibeinging-dsh-agent-budget** — 占位待复核，判定暂不展示
- `[未定位]` **vibeinging-dsh-session-teams** — 占位待复核，判定暂不展示
- `[未定位]` **victorzhong0110-dsh-outcome-loop** — 占位待复核，判定暂不展示
- `[未定位]` **vidgewong-dsh-agent-hub** — 占位待复核，判定暂不展示
- `[未定位]` **vilicvane-dsh-plugin-turn-memory** — 占位待复核，判定暂不展示
- `[未定位]` **vim0x3c-dsh-session-manager** — 占位待复核，判定暂不展示
...
- `[未定位]` **watericetangcw-dsh-session-pause** — 占位待复核，判定暂不展示
- `[未定位]` **wavespeedai-wavespeed-dsh-skill** — 占位待复核，判定暂不展示
- `[未定位]` **wccong1213-dsh-auto-memory** — 占位待复核，判定暂不展示
- `[未定位]` **weishao2-tizhuang-agent-skills** — 占位待复核，判定暂不展示
- `[未定位]` **weiwang988-dsh-session-spend** — 占位待复核，判定暂不展示
...
- `[未定位]` **yoke233-dsh-prime-agent** — 占位待复核，判定暂不展示
...
- `[未定位]` **yuqingsh-dsh-image-subagent** — 占位待复核，判定暂不展示
- `[未定位]` **yushenghai1106-dsh-memory-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **yuzh1090-dsh-voice-prompt-compressor** — 占位待复核，判定暂不展示
- `[未定位]` **yuzhou1227-dsh-session-manager** — 占位待复核，判定暂不展示
- `[未定位]` **yytbit-dsh-plugin-meta-memory** — 占位待复核，判定暂不展示
- `[未定位]` **z-col-dsh-skillsmanageplugins** — 占位待复核，判定暂不展示
- `[未定位]` **zem17-dsh-subagent-agy** — 占位待复核，判定暂不展示
- `[未定位]` **zenjibad-skill-injector-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **zhenyu98-dsh-context-doctor** — 占位待复核，判定暂不展示
- `[未定位]` **zhourenke-dsh-agent-rate-limit** — 占位待复核，判定暂不展示
- `[未定位]` **zhoushoujianwork-easyeda-agent** — 占位待复核，判定暂不展示
- `[未定位]` **zhujunpeng12-dsh-memory-system** — 占位待复核，判定暂不展示
- `[未定位]` **zmh2000829-dsh-agent-bridge** — 占位待复核，判定暂不展示
- `[未定位]` **zmh2000829-dsh-memory-graph** — 占位待复核，判定暂不展示
...
- 🟨 `[不兼容]` [aa2246740-dsh-skillhub](https://github.com/aa2246740/dsh-skillhub) ★0 — DSH manager for user Skills already on disk in Agent home and DSH home
...
- 🟨 `[不兼容]` [alexchen5-research-epic-manager](https://github.com/alexchen5/research-epic-manager) ★0 — Agent skills for running research projects end-to-end using a plain-file epic→issues→comments tracker.
...
- 🟨 `[不兼容]` [echo804-dsh-billing-balance](https://github.com/echo804/dsh-billing-balance) ★0 — 在deepseek-harness中动态查询apikey的余额和本次会话的消耗金额
- 🟨 `[不兼容]` [esonx-dsh-project-j4agent](https://github.com/esonx/dsh-project-j4agent) ★0 — Agent-native Software Development Lifecycle Management for DeepSeek Harness
...
- 🟨 `[不兼容]` [heming-gmh-dsh-capcheck](https://github.com/heming-gmh/dsh-capcheck) ★0 — V0 capability-disclosure scanner for DeepSeek Harness (DSH) cordis plugins -- zero-execution static analysis o
...
- 🟨 `[不兼容]` [kiim-wong-dsh-push](https://github.com/kiim-wong/dsh-push) ★0 — Push DeepSeek Harness agent lifecycle notifications to configurable channels
...
- 🟨 `[不兼容]` [qiqiangvae-dsh-newbe-plugins](https://github.com/qiqiangvae/dsh-newbe-plugins) ★0 — 个人使用 DSH 的增强插件，包括收藏功能、输入锁和会话工具收纳等功能
...
- 🟨 `[不兼容]` [xudafa006-dsh-plugin](https://github.com/xudafa006/dsh-plugin) ★0 — Caidazi investment MCP and Skills plugin for DeepSeek Harness
- 🟨 `[不兼容]` [yingjian666-dsh-zh-thinking](https://github.com/yingjian666/dsh-zh-thinking) ★0 — DeepSeek Harness (dsh) 插件：注入系统提示词，强制 Agent 的思维链 / 规划 / 工具推理全程使用简体中文，防止中文思考漂移为英文
...
- `[未定位]` **acosmi-dsh-session-supervisor** — 占位待复核，判定暂不展示
- `[未定位]` **amengclass-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **dream12347-dsh-delete-session** — 占位待复核，判定暂不展示
- `[未定位]` **ljsysfurryace-dsh-memory-director** — 占位待复核，判定暂不展示
- `[未定位]` **oolleyw-dsh-skill-manager** — 占位待复核，判定暂不展示
- `[未定位]` **roseplanetb613-dsh-bg-wallpaper** — 占位待复核，判定暂不展示
...
- `[未定位]` **dsh-adaptive-subagent-report** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-context-viewer** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-rewind** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-session-memory** — 占位待复核，判定暂不展示
- `[未定位]` **mindspace-dsh-session-memory** — 占位待复核，判定暂不展示
- `[未定位]` **mstar-workflow** — 占位待复核，判定暂不展示
- `[未定位]` **dsh-kimi-bridge** — 占位待复核，判定暂不展示
