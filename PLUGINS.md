...
| dsh-autotier | [PerryLink/dsh-autotier](https://github.com/PerryLink/dsh-autotier) | 自动模型分层路由：一条用户指令进、一条分层决策出——复杂意图由强档规划、廉价档执行，确定性护栏 + TTL 升级回退，无需手动切换模型；`dsh.bundle.patch` 可安装，1024 商店渠道 `dsh1024 plugin --profile web add dsh-autotier` | 待测 |
...
| dsh-restart | [zhengjy01/dsh-restart](https://github.com/zhengjy01/dsh-restart) | DeepSeek Harness 一键重启：网页重启按钮 + `dsh_restart` Agent 工具，独立 helper 等端口释放后以相同命令拉起新宿主、页面自动重连，新宿主启动失败时恢复控制台显示启动错误；npm `@zhengjunyao/dsh-restart` 0.1.1，MIT | 待测 |
...
| dsh-split-screen | [YEYEYEYESHIFU/dsh-split-screen](https://github.com/YEYEYEYESHIFU/dsh-split-screen) | Web UI 分屏工作区：拖拽会话到面板边缘即可分屏，每个面板都是完整原生会话（流式回复、排队消息、面板内 ask_user/审批卡片、面板内 Chat/Trajectory/插件视图标签切换），Alt+T 切换紧凑 TUI 输入（沿用原生输入卡的字号/行高/对齐几何），Alt+Shift+方向键切换面板焦点；需 DSH 0.1.5+，npm 已发布 | 待测 |
